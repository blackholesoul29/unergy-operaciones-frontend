...
const eq = (got, exp, msg) => {
  ...
  console.log((pass ? '✅' : '❌') + ` ${msg} → ${JSON.stringify(got)}${pass ? '' : ` (esperado ${JSON.stringify(exp)})`}`)
  if (!pass) ok = false
}

// ── Fixture: el caso real de Tecni-plast (Margaritas 1 firmada, 2 muda) ──────
const oferta = (extra) => ({
  id: 1, estado: 'oferta', tipo: 'compra_energia', cliente_id: 7,
  cliente_razon_social: 'TECNI-PLAST S.A.S.', planta_nombre: 'Margaritas 1',
  codigo_seguimiento: 'OP.COM No.0103-6-2026', resultado: 'pendiente',
  seguimientos: 0, fecha_oferta: null, fecha_ultima_respuesta: null,
  alerta: false, dias_sin_respuesta: 0, updated_at: '2026-08-01T10:00:00-05:00',
  ficha: { energia_promedio_kwh_mes: 170000, fuentes: {} }, ...extra,
})

const OFERTAS = [
  oferta({ id: 1, estado: 'firmado', planta_nombre: 'Margaritas 1' }),
  oferta({ id: 2, estado: 'oferta', planta_nombre: 'Margaritas 2', fecha_oferta: '2026-06-01',
           seguimientos: 6, alerta: true, dias_sin_respuesta: 42 }),
  oferta({ id: 3, estado: 'declinado', planta_nombre: 'Los Apóstoles', tipo: 'servicios_operacionales',
           resultado: 'declinado', ficha: { energia_promedio_kwh_mes: null, fuentes: {} } }),
  oferta({ id: 4, estado: 'operando', planta_nombre: 'San Pelayo',
...
eq(grupos.firmado.map((o) => o.id), [1], 'firmado')
eq(grupos.oferta.map((o) => o.id), [2], 'oferta')
eq(grupos.operando.map((o) => o.id), [4], 'operando')
eq(grupos.cerradas.map((o) => o.id), [3], 'declinado cae en Cerradas')
eq(M.COLUMNAS.find((c) => c.value === 'cerradas').alSoltar, 'declinado',
...
eq(M.agruparPorColumna([oferta({ id: 9, estado: 'etapa_nueva' })]).oportunidad.map((o) => o.id), [9],
...
eq(M.filtrar(OFERTAS, { tipos: ['servicios_operacionales'] }).map((o) => o.id), [3], 'filtro por tipo')
eq(M.filtrar(OFERTAS, { etapas: ['oferta', 'firmado'] }).map((o) => o.id), [1, 2], 'filtro por etapa')
eq(M.filtrar(OFERTAS, { soloAlerta: true }).map((o) => o.id), [2], 'solo con alerta')
eq(M.filtrar(OFERTAS, { soloSinRespuesta: true }).map((o) => o.id), [2], 'solo sin respuesta')
...
eq(M.sinRespuesta({ fecha_oferta: '2026-06-01', fecha_ultima_respuesta: null }), true, 'enviada sin respuesta')
eq(M.sinRespuesta({ fecha_oferta: '2026-06-01', fecha_ultima_respuesta: '2026-06-10' }), false, 'ya respondió')
...
eq(M.alarmante({ seguimientos: 9, fecha_ultima_respuesta: '2026-07-01' }), false, 'respondió: no alarma')

// ── El mes que vive dentro del código ────────────────────────────────────────
eq(M.mesDelCodigo({ codigo_seguimiento: 'OP.COM No.0103-6-2026' }), 'jun 2026', 'mes del código')
eq(M.mesDelCodigo({ codigo_seguimiento: 'OP.REP No.0087-12-2025' }), 'dic 2025', 'mes de dos dígitos')
eq(M.mesDelCodigo({ codigo_seguimiento: 'sin formato' }), null, 'código sin formato → null')
eq(M.mesDelCodigo({}), null, 'sin código → null')
eq(M.mesDelCodigo({ codigo_seguimiento: 'OP.COM No.0103-13-2026' }), null, 'mes 13 no existe → null')
...
eq(M.puedeFirmarPPA({ tipo: 'servicios_operacionales' }), false, 'servicios NO firma PPA (el backend da 422)')
eq(M.puedeFirmarPPA({ tipo: 'compra_energia', ppa_contrato_id: 5 }), false, 'ya tiene contrato')

eq(M.validarFirma({ fecha_inicio: '2026-02-12', fecha_fin: '2032-12-31', modo_precio: 'unica', tarifa_base: 300 }),
   [], 'tarifa única válida')
eq(M.validarFirma({ fecha_inicio: '2026-02-12', fecha_fin: '2025-01-01', modo_precio: 'unica', tarifa_base: 300 }),
   ['La fecha de fin es anterior a la de inicio.'], 'fin antes del inicio')
eq(M.validarFirma({ fecha_inicio: '2026-01-01', fecha_fin: '2026-12-31', modo_precio: 'unica' }),
   ['Falta la tarifa ($/kWh).'], 'sin precio no se firma')
eq(M.validarFirma({ fecha_inicio: '2026-01-01', fecha_fin: '2027-12-31', modo_precio: 'tabla',
                    precios_anuales: [{ anio: 2026, precio: 320 }, { anio: 2026, precio: 305 }] }),
   ['La tabla de precios tiene años repetidos.'], 'años repetidos')
eq(M.validarFirma({ fecha_inicio: '2026-01-01', fecha_fin: '2026-12-31', modo_precio: 'unica',
                    tarifa_base: 300, periodo_indexacion_base: '2025-13' }),
   ['El mes base de indexación debe ser YYYY-MM (por ejemplo 2025-10).'], 'mes base inválido')
...
eq(M.tarifasMensualesQueGenera({
  fecha_inicio: '2026-02-12', fecha_fin: '2027-06-30', modo_precio: 'tabla',
  precios_anuales: [{ anio: 2026, precio: 320 }, { anio: 2027, precio: 305 }],
}), 17, 'la tabla anual se expande a 17 filas mensuales')
eq(M.tarifasMensualesQueGenera({
  fecha_inicio: '2026-02-12', fecha_fin: '2027-06-30', modo_precio: 'tabla',
  precios_anuales: [{ anio: 2025, precio: 320 }, { anio: 2026, precio: 320 }],
}), 11, 'un año fuera del periodo no genera filas')
eq(M.tarifasMensualesQueGenera({ modo_precio: 'unica', tarifa_base: 300 }), 0, 'tarifa única no genera tabla')
...
eq(M.TIPOS_OFERTA.map((t) => t.value),
