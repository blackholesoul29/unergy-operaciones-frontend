  ...
  contratos: [
    {
      id: 1,
      nombre_interno: 'C1',
      comprador_nombre: 'Comp',
      n_plantas: 2,
      estado_cumplimiento: 'no_cumple',
      requiere_bolsa: true,
      total_anual_mwh: 24,
      total_min_anual_mwh: 12,
      bolsa_anual_mwh: 3,
      ...
      proyectos: [
        {
          id: 10,
          nombre: 'A',
          pct_despacho_rep: 0.5,
          meses: Array.from({ length: 12 }, (_, i) => ({ month: i + 1, valor_mwh: 1 })),
        },
        {
          id: 11,
          nombre: 'B',
...
ok(
  'proyectos en outline level 1',
  rowLevels.some((l) => l === 1),
)
// Total general usa SUM de filas de contrato
ok(
  'total general SUM',
  formulaCells.some((f) => f.kind === 'total_mes' && /^SUM\(/.test(f.f)),
)
// Total anual de contrato = SUM(Ene:Dic) de su propia fila
ok(
  'total anual contrato SUM',
  formulaCells.some((f) => f.kind === 'contrato_total'),
)
...
ok(
  'contrato_mes fórmula referencia filas correctas (proyectos)',
  /B5:B6/.test(contractMesFormula.f) || /SUM\(B5:B6\)/.test(contractMesFormula.f),
)
