  ...
  <div class="mf-root">
      ...
      <div class="mf-chips">
        ...
        <button
          ...
          :style="
            filtro === e.id
              ? {
                  background: e.color_hex || '#915BD8',
                  borderColor: e.color_hex || '#915BD8',
                  color: '#fff',
                }
              : {}
          "
    ...
    <main class="mf-list">
      <div v-if="loading" class="mf-state">
        <i class="pi pi-spin pi-spinner" /> Cargando fallas…
      </div>
        ...
        <button v-for="f in filtradas" :key="f.id" class="mf-card" @click="openDetail(f)">
          ...
          <div class="mf-card-main">
            ...
            <div class="mf-card-foot">
              <span class="mf-prio" :style="{ color: f.prioridad?.color_hex || '#6b5a8a' }">{{
                f.prioridad?.etiqueta
              }}</span>
              <span class="mf-time">{{ relativeTime(f.fecha_identificacion) }}</span>
              <span v-if="f.asignado_a" class="mf-assignee" :title="f.asignado_a.nombre">{{
