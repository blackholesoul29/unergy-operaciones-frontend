      ...
      <div v-if="open" class="is-root">
        ...
        <main class="is-chart">
          <div v-if="loading" class="is-state">
            <i class="pi pi-spin pi-spinner" /> <span>Cargando inversores…</span>
          </div>
        ...
        <footer class="is-foot">
          ...
          <span v-if="actualizado" class="is-foot-upd"
            ><i class="pi pi-clock" /> {{ actualizado }}</span
          >
        </footer>
      </div>
    </Transition>
...
async function cargar(forzar = false) {
  ...
  try {
    const { data } = await api.get(
      `/generacion-solar/monitoring/${props.proyectoId}/inverters-power`,
    )
