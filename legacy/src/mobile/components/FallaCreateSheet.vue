...
async function agregarInv() {
  ...
  try {
    ...
    invError.value = e.response?.data?.detail || 'No se pudo agregar'
  }
}
async function prefillMinigranja() {
  ...
  for (let i = 0; i < tipica.length; i++) {
    try {
      ...
      invError.value = e.response?.data?.detail || 'Error creando inversores'
      break
    }
  }
  await cargarInversores()
...
async function submit() {
  ...
  try {
    ...
    if (f.nota.trim()) {
      try {
        await api.post(`/fallas/${nueva.id}/seguimientos`, { nota: f.nota.trim() })
      } catch {
        /* no crítico */
      }
    }
