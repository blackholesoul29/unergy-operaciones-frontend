        ...
        <div class="td-sheet">
          ...
          <div class="td-body">
            ...
            <div class="td-info-row">
              <span class="td-badge-estado" :style="estadoStyle(fa.estado)">{{
                fa.estado?.etiqueta
              }}</span>
            ...
            <div class="td-proj">
              <i class="pi pi-bolt" /> {{ fa.proyecto?.nombre_comercial || '—' }}
            </div>
            ...
            <div class="td-section">
              <div class="td-section-head">
                ...
                <span v-if="archivos.length" class="td-foto-count"
                  >{{ archivos.length }} foto{{ archivos.length !== 1 ? 's' : '' }}</span
                >
              </div>
              ...
              <div class="td-foto-btns">
                <button
                  class="td-foto-btn td-foto-btn--cam"
                  @click="$refs.inputCamara.click()"
                  :disabled="uploading"
                >
                ...
                <button
                  class="td-foto-btn td-foto-btn--gal"
              ...
              <p v-else-if="!uploading" class="td-fotos-empty">
                Las fotos se guardan en Google Drive, organizadas por falla.
              </p>
            </div>
            ...
            <div class="td-section">
              ...
              <div class="td-nota-add">
                <textarea
                  v-model="nota"
                  rows="2"
                  class="td-textarea td-textarea--sm"
                  placeholder="Agregar nota o actualización…"
                />
                <button
                  class="td-nota-send"
                  :disabled="addingSeg || !nota.trim()"
                  @click="agregarNota"
                >
                  <i v-if="addingSeg" class="pi pi-spin pi-spinner" /><i
                    v-else
                    class="pi pi-send"
                  />
                </button>
              ...
              <div
                v-for="s in (fa.seguimientos || []).slice().reverse()"
                :key="s.id"
                class="td-seg"
              >
...
.td-btn-cerrar:disabled,
.td-btn-reabrir:disabled {
  opacity: 0.5;
}
...
.tsheet-enter-active .td-sheet,
.tsheet-leave-active .td-sheet {
...
.tsheet-enter-from .td-sheet,
.tsheet-leave-to .td-sheet {
