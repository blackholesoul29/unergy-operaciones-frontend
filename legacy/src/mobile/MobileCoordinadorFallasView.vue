<template>
  <div class="cf-root">
    ...
    <div class="cf-filters">
      ...
      <div class="cf-chips">
        <button
          :class="['cf-fchip', filtro === 'sin_asignar' && 'cf-fchip--on cf-fchip--warn']"
        ...
        <button v-for="f in filtradas" :key="f.id" class="cf-card" @click="openDetail(f)">
          ...
          <div class="cf-card-main">
            ...
            <div class="cf-card-foot">
              <span class="cf-prio" :style="{ color: f.prioridad?.color_hex || '#6b5a8a' }">{{
              ...
              <span
                v-if="f.asignado_a"
                class="cf-assignee cf-assignee--set"
                :title="f.asignado_a.nombre"
              >
...
const sinAsignar = computed(
  () => fallas.value.filter((f) => !f.asignado_a && !f.estado?.es_estado_final).length,
)
