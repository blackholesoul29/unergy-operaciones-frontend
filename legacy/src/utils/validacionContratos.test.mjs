...
assert(
  toIsoDate(new Date(2026, 0, 5)) === '2026-01-05',
  'toIsoDate de Date usa componentes locales (sin correr día)',
)
...
assert(
  checkDateOverlap('2026-07-01', '2026-12-31', base).length === 0,
  'Sin solapamiento (rangos disjuntos)',
)
assert(
  checkDateOverlap('2026-06-30', '2026-12-31', base).length === 1,
  'Solapamiento en un solo día (borde inclusivo)',
)
assert(checkDateOverlap('2026-05-01', '2026-08-01', base).length === 1, 'Solapamiento parcial')
assert(
  checkDateOverlap('2025-01-01', '2027-01-01', base).length === 1,
  'Solapamiento total (envuelve al existente)',
)
assert(checkDateOverlap('2026-01-01', '2026-06-30', base).length === 1, 'Fechas iguales solapan')

// Fecha fin abierta (null) = vigente indefinido → cruza con cualquier futuro.
assert(
  checkDateOverlap('2030-01-01', null, [{ id: 9, fecha_inicio: '2020-01-01', fecha_fin: null }])
    .length === 1,
...
assert(
  conflictosAtribucion(
    { proyecto_id: 10, fecha_inicio: '2026-03-01', fecha_fin: '2026-04-01' },
    universo,
  ).length === 1,
...
assert(
  conflictosAtribucion(
    { id: 1, proyecto_id: 10, fecha_inicio: '2026-03-01', fecha_fin: '2026-04-01' },
    universo,
  ).length === 0,
  'Se excluye a sí mismo por id al editar',
)
assert(
  conflictosAtribucion({ proyecto_id: 10, fecha_inicio: '2026-03-01', fecha_fin: '2026-04-01' }, [
    universo[2],
  ]).length === 0,
...
assert(
  conflictosAtribucion(
    { proyecto_id: null, fecha_inicio: '2026-03-01', fecha_fin: '2026-04-01' },
    universo,
  ).length === 0,
...
assert(
  repl.length === 1 && repl[0].id === 1,
  'Reemplazo: encuentra el contrato del mismo SIC que se cruza',
)
assert(
  findReplacementContract({ codigo_sic_contrato: null, fecha_inicio: '2026-06-01' }, porSic)
    .length === 0,
...
assert(
  validateContractUniqueness({ ...nuevoDup, fecha_fin: '2027-01-01' }, existentes).length === 0,
  'Distinta fecha fin → no es duplicado exacto',
)
assert(
  validateContractUniqueness({ ...nuevoDup, id: 1 }, existentes).length === 0,
...
assert(
  finEfectivoIso({ fecha_fin: '2030-03-31', fecha_fin_efectiva: '2026-02-06' }) === '2026-02-06',
  'finEfectivoIso prefiere la ventana efectiva sobre la cruda',
)
assert(
  finEfectivoIso({ fecha_fin: '2030-03-31' }) === '2030-03-31',
...
assert(
  conflictosAtribucion({ proyecto_id: 12, fecha_inicio: '2026-02-07', fecha_fin: '2039-12-31' }, [
...
assert(
  conflictosAtribucion({ proyecto_id: 12, fecha_inicio: '2026-02-07', fecha_fin: '2039-12-31' }, [
    { ...filaSuperadaLaReserva, fecha_fin_efectiva: undefined },
  ]).length === 1,
  'Sin efectiva (backend viejo) se conserva el comportamiento anterior (fallback a cruda)',
)
assert(
  conflictosAtribucion({ proyecto_id: 12, fecha_inicio: '2026-01-01', fecha_fin: '2039-12-31' }, [
    filaSuperadaLaReserva,
  ]).length === 1,
...
assert(
  conflictosAtribucion({ proyecto_id: 12, fecha_inicio: '2026-01-01', fecha_fin: '2039-12-31' }, [
    { ...filaSuperadaLaReserva, es_duplicado: true },
  ]).length === 0,
  'Una fila ya marcada compra en bolsa no vuelve a generar conflicto',
)
assert(
  conflictosAtribucion({ proyecto_id: 12, fecha_inicio: '2026-01-01', fecha_fin: '2039-12-31' }, [
    { ...filaSuperadaLaReserva, tipo_solicitud: 'terminacion' },
  ]).length === 0,
