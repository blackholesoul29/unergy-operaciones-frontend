...
;(function (global) {
  ...
  function diagnosticar(fases) {
    var conAlerta = fases.filter(function (x) {
      return x.alerta
    })
    var sinDatos = fases.filter(function (x) {
      return !x.valido
    })
    ...
    return {
      nivel: 'alerta',
      texto:
        'POSIBLE ERROR DE CABLEADO DE CTs — ' +
        hints.join(' · ') +
  ...
  function renderFasorial(container, datos, opciones) {
    ...
    fases.forEach(function (fx) {
      ...
      s.push(
        '<line x1="' +
          ...
          '" stroke="' +
          c.v +
          '" stroke-width="3.5" marker-end="url(#fzArr)"/>',
      )
    ...
    s.push(
      txt(
        cx,
        H - 14,
        (datos.meter ? 'Medidor ' + esc(String(datos.meter)) + ' · ' : '') +
          esc(fecha) +
          (opciones.marca ? ' · ' + esc(opciones.marca) : ''),
  ...
  function caja(x, y, texto, color) {
    ...
    return (
      ...
      '" height="' +
      h +
      '" rx="5" fill="#0b0f1a" stroke="' +
      color +
      '" stroke-width="1" opacity="0.95"/>' +
      txt(bx + w / 2, by + 15, esc(texto), {
        size: 11,
        fill: color,
        anchor: 'middle',
        weight: 600,
