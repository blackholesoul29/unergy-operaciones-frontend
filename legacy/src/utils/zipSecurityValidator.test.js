...
describe('validateZipEntries — lo que acepta', () => {
  it('acepta las extensiones permitidas', () => {
    const res = validateZipEntries(fakeZip([['pago_1/cuenta_cobro.pdf'], ['pago_1/factura.jpg']]), DOCS)
  ...
  it('ignora las entradas de directorio', () => {
    expect(validateZipEntries(fakeZip([['pago_1/', true], ['pago_1/cuenta.pdf']]), DOCS).valid).toBe(true)
  })
  ...
  it('respeta una allowlist personalizada', () => {
    const res = validateZipEntries(fakeZip([['data.csv'], ['libro.xlsx']]), { allowedExtensions: ['csv', 'xlsx'] })
    expect(res.valid).toBe(true)
  })
...
describe('validateZipEntries — lo que rechaza', () => {
  ...
  it('rechaza una extensión fuera de la allowlist', () => {
    ...
    expect(res.valid).toBe(false)
    expect(res.errors.some((e) => e.code === 'DISALLOWED_EXTENSION' && /exe/.test(e.path))).toBe(true)
  })
