...
export const CONCEPTS = {
  mant: 'Mantenimiento',
  iva_mant: 'IVA mantenimiento',
  int: 'Servicio de internet',
  iva_int: 'IVA internet',
  arr: 'Arriendo',
  iva_arr: 'IVA arriendo',
  poliza: 'Póliza todo riesgo y lucrocesante',
  iva_poliza: 'IVA póliza',
  serv_pub: 'Servicios públicos - consumo de energía',
  admin: 'Administración',
  iva_admin: 'IVA administración',
  ...
  arr_cc: 'Arriendo Cuenta de Cobro',
  arr_fact: 'Arriendo Factura Electrónica',
}
...
export const nameTokenSet = (s) =>
  new Set(
    expandirAbreviaturas(s)
      .split(' ')
...
export function extractMandate(text, filename = '') {
  ...
  let mandante = '',
    nit = ''
  const mm = flat.match(
    /de mandatario,?\s*y\s+(.+?),\s*con NIT\.?\s*([\d.\-\s]+?),?\s*en calidad de mandante/i,
  )
  if (mm) {
    mandante = mm[1].trim()
    nit = mm[2].replace(/\s/g, '')
  }
  ...
  if (!proj) {
    const m = filename.match(/-Costos-(.+?)-[^-]+\.pdf/i)
    proj = m ? m[1] : filename
  }
  ...
  const map = [
    ['MANTENIMIENTO', 'mant'],
    ['IVA MANTENIMIENTO', 'iva_mant'],
    ['SERVICIO DE INTERNET', 'int'],
    ['IVA INTERNET', 'iva_int'],
    ...
    ['ARRIENDO CUENTA DE COBRO', 'arr_cc'],
    ['ARRIENDO FACTURA', 'arr_fact'],
  ...
  text.split('\n').forEach((line) => {
    ...
    const v = parseMandatoNumber(m[2])
...
export function suggestTag(projName, tags, savedMap = {}) {
  ...
  if (remembered && tags.includes(remembered))
  ...
  const cand = tags.filter((t) => {
    const nt = norm(t)
    return tks.some((tk) => nt.includes(tk))
  })
  ...
  if (cand.length > 1) {
    const scored = cand
      .map((t) => {
        const nt = norm(t)
        const sc =
          tks.filter((tk) => nt.includes(tk)).length + (nt.endsWith(tks[tks.length - 1]) ? 0.5 : 0)
        return { t, sc }
      })
      .sort((a, b) => b.sc - a.sc)
...
export function reconciliar(mandato, details, tag) {
  ...
  Object.keys(mandato.vals || {}).forEach((c) => {
    ...
    if (av === 0 && mv > 0) {
      ...
      if (candidatos.length) {
        const best = candidatos.reduce((a, b) =>
          Math.abs(b.debe - mv) < Math.abs(a.debe - mv) ? b : a,
        )
        flags.push({
          lvl: 'warn',
          code: 'OTRA_CUENTA',
          txt: `${CONCEPTS[c]}: en el mandato (${fmt(mv)}) — registrado en el asiento pero en otra cuenta/asociado: cuenta ${best.acc} (${best.accDesc}), asociado "${best.asociado}" (${fmt(best.debe)}).`,
        })
      } else {
        flags.push({
          lvl: 'bad',
          code: 'FALTANTE',
          txt: `${CONCEPTS[c]}: en el mandato (${fmt(mv)}) pero no aparece en el asiento para este mandante/proyecto.`,
        })
      }
      ...
      flags.push({
        lvl: 'bad',
        code: 'DIFERENCIA',
        txt: `${CONCEPTS[c]}: mandato ${fmt(mv)} vs. asiento ${fmt(av)} · diferencia ${fmt(Math.abs(mv - av))}.`,
      })
    else flags.push({ lvl: 'ok', code: 'OK', txt: `${CONCEPTS[c]}: ${fmt(mv)} — coincide.` })
  })
  ...
  Object.keys(sums).forEach((c) => {
    if (!(c in (mandato.vals || {})) && sums[c] > TOL)
      flags.push({
        lvl: 'warn',
        code: 'SOBRANTE',
        txt: `${CONCEPTS[c]}: ${fmt(sums[c])} registrado en el asiento pero no listado en el mandato.`,
      })
  })
  ...
  wrongAcc.forEach((d) => {
    flags.push({
      lvl: 'bad',
      code: 'CUENTA',
      txt: `Posible cuenta equivocada: ${fmt(d.debe)} en cuenta ${d.acc} (${NON_COST_ACCOUNTS[d.acc]}), que no es de costo de mandato.`,
    })
  })
  ...
  if (mandato.total != null) {
    ...
    if (Math.abs(sumConc - mandato.total) > TOL)
      flags.push({
        lvl: 'warn',
        code: 'TOTAL',
        txt: `Total: "valor a pagar" del mandato ${fmt(mandato.total)} vs. suma de costos ${fmt(Math.round(sumConc))}.`,
      })
  }

  const status = flags.some((f) => f.lvl === 'bad')
    ? 'bad'
    : flags.some((f) => f.lvl === 'warn')
      ? 'warn'
      : 'ok'
...
const CONCEPTO_RE =
  /^(INGRESO BRUTO|COMERCIALIZACION|SERVICIOS DESPACHO Y COORDINACION CND|DESPACHO|VENTAS EN BOLSA|COMPRAS EN BOLSA|ENERGIA EN BOLSA|REDISTRIBUCION DE INGRESOS DE ACUERDO AL PROTOCOLO|REDISTRIBUCION|ARRANQUE Y PARADA|SERVICIOS DE ADMINISTRACION SIC|I V A SIC(?: 19)?|CARGO POR CONFIABILIDAD|FAZNI)\s*(?:19\s*)?(?:COP\s*)?(?:GENERADOR|COMERCIALIZADOR)?\s*/
  ...
  for (const grupos of porPlanta.values()) {
    ...
    if (inversionistas.length === 1 && operadores.length) {
      ...
      for (const o of operadores) {
        ...
        for (const [k, v] of Object.entries(o.conceptos))
          base.conceptos[k] = (base.conceptos[k] || 0) + v
      }
      out.push(base)
    } else {
...
export function parseIngresos(rows, accPrefix = INGRESO_ACC_PREFIX) {
  return _gruposIngresos(rows, accPrefix)
    .map((g) => ({
      asociado: g.asociado,
      planta: g.planta,
      valor_contabilidad: g.valor_contabilidad,
    }))
...
export const INGRESO_CONCEPTOS = [
  ...
  {
    key: 'SERVICIOS DESPACHO Y COORDINACION CND',
    rol: 'resta',
    re: /^SERVICIOS DESPACHO Y COORDINACION CND\b/,
  },
  ...
  {
    key: 'SERVICIOS DE ADMINISTRACION SIC',
    rol: 'resta',
    re: /^SERVICIOS DE ADMINISTRACION SIC\b/,
  },
...
export function parseIngresosPorConcepto(rows, accPrefix = INGRESO_ACC_PREFIX) {
  return _gruposIngresos(rows, accPrefix).map((g) => ({
    asociado: g.asociado,
    planta: g.planta,
...
export function matchIngresoConceptos(valsPdf = {}, conceptosContab = {}, tol = TOL_CONCEPTO) {
  return INGRESO_CONCEPTOS.filter(
    (c) => valsPdf[c.key] !== undefined || conceptosContab[c.key] !== undefined,
  ).map((c) => {
    ...
    return {
      ...
      pdf,
      contab,
      dif: pdf !== null && contab !== null ? Math.round(pdf - contab) : null,
      estado,
    }
  })
}
