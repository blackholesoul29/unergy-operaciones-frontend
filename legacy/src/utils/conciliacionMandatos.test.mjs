...
assert(
  parseMandatoNumber('2,011.51') === 2011.51,
  `parseMandatoNumber("2,011.51") = ${parseMandatoNumber('2,011.51')} (esperado 2011.51)`,
)
assert(
  parseMandatoNumber('129') === 129,
  `parseMandatoNumber("129") = ${parseMandatoNumber('129')} (esperado 129)`,
)
assert(
  parseMandatoNumber('$ 2,011,510.00') === 2011510,
  `parseMandatoNumber("$ 2,011,510.00") = ${parseMandatoNumber('$ 2,011,510.00')} (esperado 2011510)`,
)
assert(
  parseMandatoNumber('497,333') === 497333,
  `parseMandatoNumber("497,333") = ${parseMandatoNumber('497,333')} (esperado 497333)`,
)
assert(
  parseMandatoNumber('0.50') === 0.5,
  `parseMandatoNumber("0.50") = ${parseMandatoNumber('0.50')} (esperado 0.5)`,
)
assert(
  parseMandatoNumber('-1,000.50') === -1000.5,
  `parseMandatoNumber("-1,000.50") = ${parseMandatoNumber('-1,000.50')} (esperado -1000.5)`,
)
assert(
  parseMandatoNumber('') === 0 &&
    parseMandatoNumber(null) === 0 &&
    parseMandatoNumber(undefined) === 0,
  'parseMandatoNumber vacío/null/undefined = 0',
)
assert(parseMandatoNumber(2011.51) === 2011.51, 'parseMandatoNumber(number) pasa directo')
// -- Asiento (CO): punto=miles, coma=decimal --
assert(
  parseAsientoNumber('2.011,51') === 2011.51,
  `parseAsientoNumber("2.011,51") = ${parseAsientoNumber('2.011,51')} (esperado 2011.51)`,
)
assert(
  parseAsientoNumber('129.413') === 129413,
  `parseAsientoNumber("129.413") = ${parseAsientoNumber('129.413')} (esperado 129413, son miles)`,
)
assert(
  parseAsientoNumber('129') === 129,
  `parseAsientoNumber("129") = ${parseAsientoNumber('129')} (esperado 129)`,
)
assert(
  parseAsientoNumber('$ 1.234.567,89') === 1234567.89,
  `parseAsientoNumber("$ 1.234.567,89") = ${parseAsientoNumber('$ 1.234.567,89')} (esperado 1234567.89)`,
)
assert(
  parseAsientoNumber('-1.000,50') === -1000.5,
  `parseAsientoNumber("-1.000,50") = ${parseAsientoNumber('-1.000,50')} (esperado -1000.5)`,
)
assert(
  parseAsientoNumber('') === 0 && parseAsientoNumber(null) === 0,
  'parseAsientoNumber vacío/null = 0',
)
...
// -- Mismo valor, distinta fuente: deben coincidir (diferencia = céntimos, no miles) --
{
  ...
  assert(
    Math.abs(mv - av) < 0.001,
    `mismo valor US vs CO: |${mv} - ${av}| ≈ 0 (no diferencia de miles)`,
  )
}
...
const details = [
  {
    asociado: 'STRADA ASOCIADOS S A S',
    acc: '28151002',
    accDesc: '',
    debe: 497333,
    haber: 0,
  ...
  {
    asociado: 'INVERSIONES ESTRADA ARBELAEZ Y CIA S. EN C.',
    acc: '28151002',
    accDesc: '',
    debe: 2655667,
    haber: 0,
...
const resStrada = reconciliar(
  { mandante: 'STRADA ASOCIADOS S.A.S.', vals: { mant: 497333 }, total: 497333 },
  details,
  TAG,
)
assert(
  resStrada.sums.mant === 497333,
  `STRADA: suma mant = ${resStrada.sums.mant} (esperado 497333, NO 3153000)`,
)
...
const resEstrada = reconciliar(
  {
    mandante: 'INVERSIONES ESTRADA ARBELAEZ Y CIA S EN C',
    vals: { mant: 2655667 },
    total: 2655667,
  },
  details,
...
assert(
  resEstrada.sums.mant === 2655667,
  `ESTRADA: suma mant = ${resEstrada.sums.mant} (esperado 2655667)`,
)
...
assert(
  m.vals.mant === 497333 && m.total === 497333,
  `extractMandate mant=${m.vals.mant} total=${m.total}`,
)
...
VALOR A PAGAR $ 2,011,510.00`
  ...
  assert(
    mb.vals.mant === 2011510,
    `regresión: mandato US 2,011,510.00 = ${mb.vals.mant} (esperado 2011510, NO 2.01)`,
  )
  ...
  assert(
    Math.round(resBug.sums.mant) === 2011510,
    `regresión: asiento CO 2.011.510,00 = ${resBug.sums && resBug.sums.mant} (esperado 2011510)`,
  )
  ...
  assert(
    resBug.status === 'ok',
    `regresión: status = ${resBug.status} (esperado ok — valores iguales)`,
  )
}
...
//     avisar "OTRA_CUENTA", no reportarlo como FALTANTE.
{
  ...
  const lineasNestle = [
    {
      asociado: 'SOLENIUM SAS',
      acc: '28151020',
      accDesc: 'COSTOS PARA TERCEROS - ADMINISTRACION DE PROYECTOS',
      debe: 6831500,
      haber: 0,
      etiqueta: 'Mantenimiento Preventivo - Nestle',
      proj: tagNestle,
    },
    {
      asociado: 'SOLENIUM SAS',
      acc: '28151021',
      accDesc: 'IVA ADMINISTRACION DE PROYECTOS - COSTOS PARA TERCEROS',
      debe: 1297985,
      haber: 0,
      etiqueta: 'Mantenimiento Preventivo - Nestle',
      proj: tagNestle,
  ...
  const mNestle = {
    mandante: fiduciaria,
    vals: { mant: 6831500, iva_mant: 1297985 },
    total: 8129485,
  }
  ...
  assert(
    !codes.has('FALTANTE'),
    `Nestlé/Solenium: NO debe reportarse FALTANTE (flags: ${JSON.stringify(resNestle.flags)})`,
  )
  assert(
    codes.has('OTRA_CUENTA'),
    `Nestlé/Solenium: debe reportar OTRA_CUENTA (flags: ${JSON.stringify(resNestle.flags)})`,
  )
  const otra = resNestle.flags.filter((f) => f.code === 'OTRA_CUENTA')
  assert(
    otra.some((f) => f.txt.includes('28151020') && f.txt.includes('SOLENIUM SAS')),
    `OTRA_CUENTA debe mencionar cuenta y asociado real: ${JSON.stringify(otra)}`,
  )
  assert(
    resNestle.status === 'warn',
    `Nestlé/Solenium: status = ${resNestle.status} (esperado warn, no bad — el dinero sí está)`,
  )
}
...
assert(
  pa.details.length === 1 && pa.tags.includes(TAG),
  `parseAsientos detalles=${pa.details.length} tags=${JSON.stringify(pa.tags)}`,
)
assert(
  suggestTag('La Reserva', pa.tags, {}).tag === TAG,
  'suggestTag("La Reserva") -> tag correcto',
)
...
assert(
  plantaDesdeEtiqueta('INGRESO BRUTO BIAC GD NAOS 1 ABRIL 2026 BIA ENERGY') === 'GD NAOS 1',
...
const ingRows = [
  ...
  [
    'CM/1',
    '28150505 INGRESO DE ENERGIA',
    'RODRIGUEZ VELEZ BEATRIZ',
    'INGRESO BRUTO MINIGRANJA SOLAR URUACO ABRIL 2026 TERPEL',
    0,
    3712635.47,
  ],
  [
    ...
    '28150505 INGRESO DE ENERGIA',
    'RODRIGUEZ VELEZ BEATRIZ',
    'COMERCIALIZACIÓN MINIGRANJA SOLAR URUACO ABRIL 2026 TERPEL',
    273321.72,
    0,
  ],
  [
    'CM/1',
    '28151001 FACTURAS DE COMERCIALIZACION',
    'RODRIGUEZ VELEZ BEATRIZ',
    'COMERCIALIZACIÓN MINIGRANJA SOLAR URUACO ABRIL 2026 TERPEL',
    0,
    273321.72,
  ...
  [
    'CM/1',
    '28150501 GANANCIAS POR PARTICIPACION',
...
assert(
  rodri && Math.round(Math.abs(rodri.valor_contabilidad)) === 3439314,
...
const grupos = [
  ...
  {
    asociado: 'RODRIGUEZ VELEZ BEATRIZ',
    planta: 'MINIGRANJA SOLAR URUACO',
    valor_contabilidad: -3439314,
  },
]
const mNaos = matchIngresoContab(
  { mandante: 'GD EL REMOLINO 1 S.A.S. E.S.P.', projName: 'GD NAOS 1' },
  grupos,
)
assert(
  mNaos && mNaos.planta === 'GD NAOS 1',
  `matchIngresoContab NAOS 1 (no 2) = "${mNaos && mNaos.planta}"`,
)
const mUru = matchIngresoContab(
  { mandante: 'Rodríguez Vélez Beatriz', projName: 'Minigranja Solar Uruaco' },
  grupos,
)
assert(
  mUru && mUru.planta === 'MINIGRANJA SOLAR URUACO',
  `matchIngresoContab Uruaco = "${mUru && mUru.planta}"`,
)
...
const DEB = 368513.81
const arrendadores = [
  'EDGARDO JESUS AROCA MENDIOLA',
  'DULM DAYAN AROCA GUTIERREZ',
  'CARLOS ALBERTO AROCA MINDIOLA',
]
const arrLineas = []
...
arrendadores.forEach((p, i) =>
  arrLineas.push({
    ...
    debe: 0,
    haber: 184256.91,
    etiqueta: ['30982', '30978', '30974'][i],
    proj: ESM,
  }),
)

const resArr = reconciliar(
  { mandante: BANC, vals: { arr: 1842569 }, total: 1842569 },
  arrLineas,
  ESM,
)
assert(
  Math.round(resArr.sums.arr) === 1842569,
  `ESMERALDA arriendo (suma de débitos, 5 contratos) = ${resArr.sums && Math.round(resArr.sums.arr)} (esperado 1842569, NO 0, NO 184257)`,
)
assert(
  resArr.lines.every((l) => l.asociado === BANC),
  'ESMERALDA: las líneas conciliadas son solo del mandante (arrendadores excluidos)',
)
assert(
  resArr.status === 'ok',
  `ESMERALDA arriendo: status = ${resArr.status} (esperado ok — reconcilia exacto)`,
)
...
VALOR A PAGAR $ 895,000.00`
  ...
  assert(
    mp.vals.iva_poliza === 95000,
    `BUG1: PDF iva_poliza = ${mp.vals.iva_poliza} (esperado 95000, NO en poliza)`,
  )
  ...
  assert(
    resPol.sums.poliza === 500000 &&
      resPol.sums.iva_poliza === 95000 &&
      resPol.sums.serv_pub === 300000,
    `BUG1: sums poliza/iva/serv = ${resPol.sums.poliza}/${resPol.sums.iva_poliza}/${resPol.sums.serv_pub}`,
  )
  assert(
    resPol.status === 'ok',
    `BUG1: status = ${resPol.status} (esperado ok — Póliza y Serv. Públicos ahora conciliados)`,
  )
}
...
{
  assert(
    expandirAbreviaturas('FIDUCIARIA BANCOLOMBIA PA NESTLE 18254').includes(
  ...
  assert(
    !expandirAbreviaturas('PARQUE INDUSTRIAL').includes('PATRIMONIOS AUTONOMOS'),
  ...
  const resNestle = reconciliar(
    { mandante: MAND_FULL, vals: { mant: 1000000, iva_mant: 190000 }, total: 1190000 },
    nestleLineas,
    TAGN,
  )
  assert(
    resNestle.lines.length === 2,
    `BUG2: líneas conciliadas = ${resNestle.lines.length} (esperado 2, "PA" reconocido)`,
  )
  assert(
    resNestle.sums.mant === 1000000 && resNestle.sums.iva_mant === 190000,
...
VALOR A PAGAR $ 79,705.00`
  ...
  assert(
    md2.vals.arr === 79705,
    `BUG3: dos líneas "ARRIENDO" se suman = ${md2.vals.arr} (esperado 79705, NO 39705 pisado)`,
  )
  assert(
    md.vals.arr_cc === 79705 && md.vals.arr_fact === 79706,
...
VALOR A PAGAR $ 159,411.00`
  ...
  const lrLineas = [
    ...
    },
    {
      ...
      debe: 79706,
      haber: 0,
      etiqueta: 'ARRIENDO FACT 40101',
      proj: TAGLR,
    },
  ]
  const resLR = reconciliar(mLR, lrLineas, TAGLR)
  assert(
    resLR.sums.arr_cc === 79705,
    `BUG4: arr_cc del asiento = ${resLR.sums.arr_cc} (esperado 79705 por etiqueta CC)`,
  )
  assert(
    resLR.sums.arr_fact === 79706,
    `BUG4: arr_fact del asiento = ${resLR.sums.arr_fact} (esperado 79706 por etiqueta FACT)`,
  )
  assert(
    resLR.sums.arr === undefined,
    'BUG4: no queda "arr" genérico cuando la etiqueta reclasifica ambas',
  )
  assert(
    resLR.status === 'ok',
    `BUG4: status = ${resLR.status} (esperado ok — cada factura concilia por separado)`,
  )
  ...
  assert(
    resGen.sums.arr === 100,
    `BUG4: etiqueta sin CC/FACT sigue en 'arr' genérico = ${resGen.sums.arr}`,
  )
}
...
VALOR A PAGAR $ 159,411.00`
  ...
  const lrLineas = [
    {
      ...
      debe: 79705,
      haber: 0,
      etiqueta: 'ARRIENDO CC JULIO LA RESERVA',
      proj: TAGLR,
    },
    {
      asociado: MANDLR,
      ...
      debe: 79706,
      haber: 0,
      etiqueta: 'ARRIENDO FACT JULIO LA RESERVA',
      proj: TAGLR,
    },
  ]
  const resLR = reconciliar(mLR, lrLineas, TAGLR)
  assert(
    resLR.sums.arr_cc === 79705,
    `BUG4b: arr_cc (cuenta 28151025) = ${resLR.sums.arr_cc} (esperado 79705)`,
  )
...
VALOR A PAGAR $ 1,699,529.00`
  ...
  assert(
    mIVA.vals.arr === undefined,
    'BUG4c: no debe quedar "arr" genérico (se habría descartado, no reclasificado)',
  )
  ...
  assert(
    resIVA.sums.arr_cc === 673757,
    `BUG4c: sums.arr_cc = ${resIVA.sums.arr_cc} (esperado 673757)`,
  )
  assert(
    resIVA.sums.arr_fact === 673757,
    `BUG4c: sums.arr_fact = ${resIVA.sums.arr_fact} (esperado 673757)`,
  )
...
VALOR A PAGAR $ 874,490.00`
  ...
  assert(
    resGenIVA.sums.arr === 874490,
    `BUG4d: sums.arr = ${resGenIVA.sums.arr} (esperado 874490, NO dividido en arr_cc)`,
  )
  assert(
    resGenIVA.sums.arr_cc === undefined,
    'BUG4d: no debe aparecer arr_cc cuando el mandato no divide',
  )
...
VALOR A PAGAR $ 238,000.00`
  ...
  assert(
    ma.vals.iva_admin === 38000,
    `BUG5: PDF iva_admin = ${ma.vals.iva_admin} (esperado 38000, NO en admin)`,
  )
  const admLineas = [
    {
      asociado: MANDA,
  ...
  assert(
    resAdm.sums.admin === 200000 && resAdm.sums.iva_admin === 38000,
    `BUG5: sums admin/iva = ${resAdm.sums.admin}/${resAdm.sums.iva_admin}`,
  )
  assert(
    resAdm.status === 'ok',
    `BUG5: status = ${resAdm.status} (esperado ok — Administración ahora conciliada)`,
  )
}
...
//     Debe reconciliar; y fondos distintos (18254 Nestlé, Skandia) NO deben cruzar.
{
  ...
  const lineasSS = [
    {
      asociado: 'PA 17844 SOL DE LA SIERRA',
      acc: '28151009',
      accDesc: '',
      debe: 64706.3,
      haber: 0,
    ...
    {
      asociado: 'FIDUCIARIA BANCOLOMBIA PA NESTLE 18254',
      acc: '28151020',
      accDesc: '',
      debe: 888888,
      haber: 0,
  ...
  const resSS = reconciliar(
    {
      mandante: MAND_SS,
      vals: { int: 64706.3, iva_int: 12294.2, admin: 2681883.45, iva_admin: 509557.86 },
      total: 3268441.81,
    },
    lineasSS,
    TAGSS,
  ...
  assert(
    resSS.lines.length === 4,
    `BUG6: líneas Sol Sierra = ${resSS.lines.length} (esperado 4, "PA 17844 SOL DE LA SIERRA" reconocido; Solenium y Nestlé fuera)`,
  )
  assert(
    Math.round(resSS.sums.admin) === 2681883,
    `BUG6: admin = ${resSS.sums.admin} (esperado 2681883, sin sumar 999999/888888)`,
  )
  assert(
    resSS.status === 'ok',
    `BUG6: status = ${resSS.status} (esperado ok — antes: todo faltante, suma 0)`,
  )
  // Fondo distinto: un mandato Nestlé (18254) NO debe emparejar la línea Sol Sierra.
  const resNo = reconciliar(
    {
      mandante:
        'PATRIMONIOS AUTONOMOS FIDUCIARIA BANCOLOMBIA S A SOCIEDAD FIDUCIARIA - 18254 NESTLE',
      vals: { admin: 888888 },
      total: 888888,
    },
    lineasSS,
  ...
  assert(
    resNo.sums.admin === 888888,
    `BUG6: Nestlé (18254) solo suma su propia línea = ${resNo.sums.admin} (esperado 888888, NO Sol Sierra)`,
  )
}
...
// 14) normalizarCifra — función ÚNICA que detecta miles/decimal en AMBOS formatos.
{
  // Mandato (US): coma=miles, punto=decimal
  assert(
    normalizarCifra('1,234,567') === 1234567,
    `normalizarCifra("1,234,567") = ${normalizarCifra('1,234,567')} (esperado 1234567)`,
  )
  assert(
    normalizarCifra('2,011.51') === 2011.51,
    `normalizarCifra("2,011.51") = ${normalizarCifra('2,011.51')} (esperado 2011.51)`,
  )
  assert(
    normalizarCifra('$ 2,011,510.00') === 2011510,
    `normalizarCifra("$ 2,011,510.00") = ${normalizarCifra('$ 2,011,510.00')} (esperado 2011510)`,
  )
  assert(
    normalizarCifra('497,333') === 497333,
    `normalizarCifra("497,333") = ${normalizarCifra('497,333')} (esperado 497333, miles)`,
  )
  assert(
    normalizarCifra('0.50') === 0.5,
    `normalizarCifra("0.50") = ${normalizarCifra('0.50')} (esperado 0.5)`,
  )
  // Asiento (CO): punto=miles, coma=decimal
  assert(
    normalizarCifra('1.234.567') === 1234567,
  ...
  assert(
    normalizarCifra('129.413') === 129413,
    `normalizarCifra("129.413") = ${normalizarCifra('129.413')} (esperado 129413, miles)`,
  )
  assert(
    normalizarCifra('$ 1.234.567,89') === 1234567.89,
    `normalizarCifra("$ 1.234.567,89") = ${normalizarCifra('$ 1.234.567,89')} (esperado 1234567.89)`,
  )
  assert(
    normalizarCifra('0,50') === 0.5,
    `normalizarCifra("0,50") = ${normalizarCifra('0,50')} (esperado 0.5)`,
  ...
  assert(
    normalizarCifra('2,681,883.45') === 2681883.45,
    `normalizarCifra("2,681,883.45") = ${normalizarCifra('2,681,883.45')} (esperado 2681883.45)`,
  )
  assert(
    normalizarCifra('64,706.30') === 64706.3,
    `normalizarCifra("64,706.30") = ${normalizarCifra('64,706.30')} (esperado 64706.3)`,
  )
  // Sin separador, negativos, número nativo, vacío
  assert(normalizarCifra('129') === 129, `normalizarCifra("129") = ${normalizarCifra('129')}`)
  assert(
    normalizarCifra('-1,000.50') === -1000.5,
    `normalizarCifra("-1,000.50") = ${normalizarCifra('-1,000.50')} (esperado -1000.5)`,
  )
  assert(
    normalizarCifra('-1.000,50') === -1000.5,
    `normalizarCifra("-1.000,50") = ${normalizarCifra('-1.000,50')} (esperado -1000.5)`,
  ...
  assert(
    normalizarCifra('(1.234)') === -1234,
    `normalizarCifra("(1.234)") = ${normalizarCifra('(1.234)')} (esperado -1234, paréntesis=negativo)`,
  )
  ...
  assert(
    Math.abs(normalizarCifra('2,011.51') - normalizarCifra('2.011,51')) < 0.001,
    'US vs CO: mismo valor coincide',
  )
  // El bug original de "Auditoría PDFs": $2,011,510.00 (US) NO debe leerse como 2.011
  assert(
    normalizarCifra('2,011,510.00') !== 2.011,
    'regresión Auditoría: US con comas no se lee como 2.011',
  )
}
...
for (const concepto of [
  ...
  assert(
    plantaDesdeEtiqueta(etq(concepto)) === URU,
...
assert(
  plantaDesdeEtiqueta('INGRESO BRUTO TERPEL 1 MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL') === URU,
  `plantaDesdeEtiqueta quita "TERPEL 1" = "${plantaDesdeEtiqueta('INGRESO BRUTO TERPEL 1 MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL')}"`,
)
assert(
  plantaDesdeEtiqueta('INGRESO BRUTO TERPEL 2 MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL') === URU,
  `plantaDesdeEtiqueta quita "TERPEL 2" = "${plantaDesdeEtiqueta('INGRESO BRUTO TERPEL 2 MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL')}"`,
)
assert(
  plantaDesdeEtiqueta('INGRESO BRUTO UNGG MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL') === URU,
...
assert(
  plantaDesdeEtiqueta('ENERGIA EN BOLSA COP GENERADOR MINIGRANJA SOLAR URUACO JUNIO 2026') === URU,
...
const ingJun = [
  ...
  [
    ...
    AC,
    ASO,
    'INGRESO BRUTO TERPEL 1 MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL',
    0,
    44000000,
  ],
  [
    ...
    AC,
    ASO,
    'CARGO POR CONFIABILIDAD MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL',
    0,
    1500000,
  ],
...
assert(
  ingJunRes[0] && Math.round(Math.abs(ingJunRes[0].valor_contabilidad)) === 44345428,
...
const gSierra = [
  {
    asociado: 'PA 17844 SOL DE LA SIERRA',
    planta: 'MINIGRANJA SOLAR LA PAZ LEYENDA',
    valor_contabilidad: -66407637,
  },
  {
    asociado: 'PA 17844 SOL DE LA SIERRA',
    planta: 'MINIGRANJA SOLAR SAN DIEGO SUR',
    valor_contabilidad: -57570713,
  },
  // Decoy: otro fondo (18254) con el MISMO nombre de planta NO debe robar el match.
  {
    asociado: 'PA 18254 OTRO FONDO',
    planta: 'MINIGRANJA SOLAR LA PAZ LEYENDA',
    valor_contabilidad: -999,
  },
]
const mLPL = matchIngresoContab(
  { mandante: MSIERRA, projName: 'Minigranja Solar La Paz Leyenda' },
  gSierra,
)
assert(
  mLPL &&
    mLPL.asociado === 'PA 17844 SOL DE LA SIERRA' &&
    Math.round(mLPL.valor_contabilidad) === -66407637,
...
const mSDS = matchIngresoContab(
  { mandante: MSIERRA, projName: 'Minigranja Solar San Diego Sur' },
  gSierra,
)
assert(
  mSDS && mSDS.planta === 'MINIGRANJA SOLAR SAN DIEGO SUR',
...
const mOtro = matchIngresoContab(
  {
    mandante: 'PATRIMONIOS AUTONOMOS X - 18254 OTRO FONDO',
    projName: 'Minigranja Solar San Diego Sur',
  },
  gSierra,
)
assert(
  !mOtro,
  `matchIngresoContab fondo 18254 no roba San Diego Sur del 17844 → ${mOtro ? mOtro.asociado : 'SIN MATCH (correcto)'}`,
)
...
assert(
  conceptoDesdeEtiqueta('INGRESO BRUTO MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL') ===
    'INGRESO BRUTO',
  `conceptoDesdeEtiqueta INGRESO BRUTO = "${conceptoDesdeEtiqueta('INGRESO BRUTO MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL')}"`,
)
assert(
  conceptoDesdeEtiqueta('ARRANQUE Y PARADA (COP) MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL') ===
    'ARRANQUE Y PARADA',
...
assert(
  conceptoDesdeEtiqueta('I.V.A. SIC (19%) (COP) (GENERADOR) MINIGRANJA SOLAR URUACO JUNIO 2026') ===
    'I V A SIC',
  `conceptoDesdeEtiqueta I.V.A. SIC = "${conceptoDesdeEtiqueta('I.V.A. SIC (19%) (COP) (GENERADOR) MINIGRANJA SOLAR URUACO JUNIO 2026')}"`,
)
assert(
  conceptoDesdeEtiqueta(
    'SERVICIOS DESPACHO Y COORDINACIÓN CND (COP) (GENERADOR) MINIGRANJA SOLAR URUACO JUNIO 2026',
  ) === 'SERVICIOS DESPACHO Y COORDINACION CND',
...
assert(
  conceptoDesdeEtiqueta(
    'SERVICIOS DE ADMINISTRACIÓN SIC (COP) (GENERADOR) MINIGRANJA SOLAR URUACO JUNIO 2026',
  ) === 'SERVICIOS DE ADMINISTRACION SIC',
...
const ingC = [
  ...
  [
    'CM/6',
    '28150505 INGRESO DE ENERGIA',
    'PA 17844 SOL DE LA SIERRA',
    'INGRESO BRUTO MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL',
    0,
    45786907,
  ],
  [
    ...
    '28150505 INGRESO DE ENERGIA',
    'PA 17844 SOL DE LA SIERRA',
    'ARRANQUE Y PARADA (COP) MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL',
    612972,
    0,
  ],
  [
  ...
  [
    ...
    '28150505 INGRESO DE ENERGIA',
    'PA 17844 SOL DE LA SIERRA',
    'I.V.A. SIC (19%) (COP) (GENERADOR) MINIGRANJA SOLAR URUACO JUNIO 2026',
    26283,
    0,
  ],
]
...
assert(
  gC[0] && Math.round(gC[0].conceptos['INGRESO BRUTO']) === -45786907,
  `parseIngresosPorConcepto INGRESO BRUTO neto = ${gC[0] && Math.round(gC[0].conceptos['INGRESO BRUTO'])} (esperado -45786907)`,
)
assert(
  gC[0] && Math.round(gC[0].conceptos['ARRANQUE Y PARADA']) === 612972,
...
assert(
  gC[0] && Math.round(gC[0].conceptos['I V A SIC']) === 26283,
...
const valsPdf = {
  'INGRESO BRUTO': 45786907,
  'ARRANQUE Y PARADA': 612972,
  'I V A SIC': 26283,
  FAZNI: 5000,
}
const contabConc = {
  'INGRESO BRUTO': -45786907,
  'ARRANQUE Y PARADA': 613500,
  'I V A SIC': 26283,
  COMERCIALIZACION: 1000,
}
const filas = matchIngresoConceptos(valsPdf, contabConc, 200)
const byK = Object.fromEntries(filas.map((f) => [f.concepto, f]))
assert(
  byK['INGRESO BRUTO'] && byK['INGRESO BRUTO'].estado === 'OK',
  `matchIngresoConceptos INGRESO BRUTO estado = ${byK['INGRESO BRUTO'] && byK['INGRESO BRUTO'].estado} (esperado OK)`,
)
assert(
  byK['ARRANQUE Y PARADA'] && byK['ARRANQUE Y PARADA'].estado === 'DIFERENCIA',
  `matchIngresoConceptos ARRANQUE Y PARADA (dif 528>tol) = ${byK['ARRANQUE Y PARADA'] && byK['ARRANQUE Y PARADA'].estado} (esperado DIFERENCIA)`,
)
assert(
  byK['FAZNI'] && byK['FAZNI'].estado === 'FALTA_CONTAB',
  `matchIngresoConceptos FAZNI (en PDF, no en contab) = ${byK['FAZNI'] && byK['FAZNI'].estado} (esperado FALTA_CONTAB)`,
)
assert(
  byK['COMERCIALIZACION'] && byK['COMERCIALIZACION'].estado === 'SOBRA_CONTAB',
  `matchIngresoConceptos COMERCIALIZACION (en contab, no en PDF) = ${byK['COMERCIALIZACION'] && byK['COMERCIALIZACION'].estado} (esperado SOBRA_CONTAB)`,
)
...
assert(
  mIng.vals['INGRESO BRUTO'] === 45786907,
  `extractMandate vals INGRESO BRUTO = ${mIng.vals['INGRESO BRUTO']} (esperado 45786907)`,
)
assert(
  mIng.vals['ARRANQUE Y PARADA'] === 612972,
  `extractMandate vals ARRANQUE Y PARADA = ${mIng.vals['ARRANQUE Y PARADA']} (esperado 612972)`,
)
assert(
  mIng.vals['SERVICIOS DESPACHO Y COORDINACION CND'] === 387327,
  `extractMandate vals SERV DESPACHO CND = ${mIng.vals['SERVICIOS DESPACHO Y COORDINACION CND']} (esperado 387327)`,
)
assert(
  mIng.vals['I V A SIC'] === 26283,
  `extractMandate vals I V A SIC = ${mIng.vals['I V A SIC']} (esperado 26283)`,
)
...
const ingOp = [
  ...
  [
    ...
    '28150505 INGRESO',
    'FONSAR SAS',
    'INGRESO BRUTO GD AGUSTIN 2 JUNIO 2026 XM',
    0,
    58238467,
  ],
  [
    'CM/6',
    '28150505 INGRESO',
    'XM COMPANIA DE EXPERTOS EN MERCADOS',
    'ARRANQUE Y PARADA (COP) GD AGUSTIN 2 JUNIO 2026 XM',
    675783,
    0,
  ],
  [
    ...
    '28150505 INGRESO',
    'XM COMPANIA DE EXPERTOS EN MERCADOS',
    'SERVICIOS DESPACHO Y COORDINACION CND (COP) GD AGUSTIN 2 JUNIO 2026 XM',
    498739,
    0,
  ],
]
...
assert(
  gOp.length === 1,
  `parseIngresos fusiona operador al inversionista: 1 grupo — fue ${gOp.length}`,
)
assert(
  gOp[0] && /FONSAR/.test(gOp[0].asociado),
  `grupo fusionado queda bajo el inversionista = "${gOp[0] && gOp[0].asociado}" (esperado FONSAR)`,
)
assert(
  gOp[0] && Math.round(Math.abs(gOp[0].valor_contabilidad)) === 57063945,
...
assert(
  gcOp.length === 1 &&
    Math.round(gcOp[0].conceptos['ARRANQUE Y PARADA']) === 675783 &&
    Math.round(gcOp[0].conceptos['SERVICIOS DESPACHO Y COORDINACION CND']) === 498739,
...
const ingMI = [
  ...
  [
    'CM/6',
    '28150505 INGRESO',
    'SUNO ACTIVOS SOSTENIBLES S A S',
    'INGRESO BRUTO MINIGRANJA SOLAR URUACO JUNIO 2026 TERPEL',
    0,
    6969547,
  ],
...
assert(
  parseIngresos(ingMI).length === 2,
  `multi-inversionista URUACO no se fusiona: 2 grupos — fue ${parseIngresos(ingMI).length}`,
)
