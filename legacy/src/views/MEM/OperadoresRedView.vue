<template>
  <div class="space-y-4">
    <PageHeader
      ...
      :subtitle="`${operadores.length} operadores · catálogo y correos de contacto para el reporte CGM`"
    >
      <template #actions>
    ...
    <Dialog
      ...
      :header="editingId ? 'Editar Operador de Red' : 'Nuevo Operador de Red'"
      modal
      class="w-full max-w-sm"
    >
      <div class="space-y-4 pt-2">
        <div>
          <label class="mb-1 block text-xs font-medium text-gray-600">Nombre legal *</label>
          <InputText
        ...
        <template v-if="!editingId">
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-600"
              >Correo de contacto (opcional)</label
            >
            <InputText
              v-model="form.contacto_email"
              class="w-full"
              placeholder="Ej: reportes@afinia.com.co"
            />
          </div>
          <div>
            <label class="mb-1 block text-xs font-medium text-gray-600"
              >Nombre del contacto (opcional)</label
            >
            <InputText
      ...
      <template #footer>
        ...
        <Button
          ...
          :loading="saving"
          :disabled="!form.nombre_legal?.trim()"
          @click="guardar"
        />
      </template>
    </Dialog>
    ...
    <Dialog
      ...
      header="Operador parecido ya existe"
      modal
      class="w-full max-w-sm"
    >
      <p class="mb-4 text-sm" style="color: #6b5a8a">
        ...
        (ID {{ duplicadoInfo?.candidato_id }}). Si de verdad es un operador distinto, puedes crearlo
        igual.
      </p>
...
async function guardarForzado() {
  ...
  try {
    if (editingId.value) {
      await api.patch(`/operadores-red/${editingId.value}`, pendingBody.value, {
        params: { forzar: true },
      })
      toast.add({ severity: 'success', summary: 'Operador actualizado', life: 2000 })
    } else {
      const { data } = await api.post('/operadores-red', pendingBody.value, {
        params: { forzar: true },
      })
      await _crearContactoSiAplica(data.id)
      toast.add({ severity: 'success', summary: 'Operador creado', life: 2000 })
