  ...
  <div class="space-y-5">
    ...
    <div class="grid grid-cols-2 gap-4 lg:grid-cols-4">
      ...
      >
        ...
        <p v-if="kpi.sub" class="mt-0.5 text-xs" style="color: #915bd8">{{ kpi.sub }}</p>
      </div>
    </div>

    <!-- Tabs -->
    <div class="flex gap-1 rounded-lg bg-white p-1 shadow-sm" style="border: 1px solid #e8e0f0">
      <button
        v-for="(tab, i) in TABS"
        :key="tab"
        @click="activeTab = i"
        ...
        :style="
          activeTab === i ? { backgroundColor: '#915BD8', color: 'white' } : { color: '#6b5a8a' }
        "
      >
        {{ tab }}
      </button>
    ...
    <div
      ...
      style="border: 1px solid #e8e0f0"
    >
      <h3 class="mb-4 text-sm font-semibold" style="color: #2c2039">
      ...
      <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        >
          <div class="mb-2 flex items-center gap-2">
            <div class="h-3 w-3 rounded-full" :style="{ backgroundColor: phase.color }" />
    ...
    >
      ...
      <div class="mt-4 grid grid-cols-3 gap-4">
        <div class="rounded-lg p-3" style="background: #f8f6fb">
          ...
          <p
            class="text-xs"
            :style="{ color: precipStats.lastAnomaly > 0 ? '#3B82F6' : '#F0C040' }"
          >
        ...
        <Column header="Fecha" style="min-width: 100px">
          <template #body="{ data }">
            <span class="font-mono text-xs"
              >{{ data.year }}-{{ String(data.month).padStart(2, '0') }}</span
            >
          </template>
        </Column>
        <Column field="oni_value" header="ONI" style="min-width: 80px" sortable>
          <template #body="{ data }">
            <span
              ...
              :style="{
                color:
                  data.oni_value > 0.5 ? '#D64455' : data.oni_value < -0.5 ? '#3B82F6' : '#6b5a8a',
              }"
...
async function loadPrecip() {
  try {
    const res = await api.get('/evo/clima/precip', {
      params: { region: precipRegion.value, years: 10 },
    })
