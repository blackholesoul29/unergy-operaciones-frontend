  ...
  <div class="space-y-5">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <Button
          ...
          :disabled="!resumen || !resumen.puede_enviar"
          :loading="enviando"
          v-tooltip.bottom="
            !resumen?.puede_enviar ? 'Quedan fronteras con horas sin fuente por revisar' : null
          "
    ...
    <div class="flex flex-wrap gap-4">
      <div
        v-for="stat in stats"
        :key="stat.label"
        class="flex h-20 min-w-[9rem] flex-1 cursor-pointer flex-col justify-center rounded-xl bg-white p-4 shadow-sm"
    ...
    >
      <div class="mb-3 flex items-start justify-between gap-3">
        <div>
          ...
          <p class="mt-0.5 text-xs" style="color: #9b89b5">
      ...
      <div class="mb-1 grid grid-cols-4 gap-2.5">
        <div class="rounded-lg py-2.5 text-center" style="background: #eef0f4">
          <p class="text-xl font-extrabold" style="color: #52596b">{{ estadoQuoia.en_espera }}</p>
          <p class="mt-0.5 text-[11px] font-semibold" style="color: #52596b">En espera</p>
        </div>
        <div class="rounded-lg py-2.5 text-center" style="background: #e8f5d9">
          <p class="text-xl font-extrabold" style="color: #4d7c0f">{{ estadoQuoia.exitoso }}</p>
          <p class="mt-0.5 text-[11px] font-semibold" style="color: #4d7c0f">Exitoso</p>
        </div>
        <div class="rounded-lg py-2.5 text-center" style="background: #fdf3d0">
          <p class="text-xl font-extrabold" style="color: #92700c">
            {{ estadoQuoia.exitoso_con_alerta }}
          </p>
          <p class="mt-0.5 text-[11px] font-semibold" style="color: #92700c">Con alerta</p>
        </div>
        <div class="rounded-lg py-2.5 text-center" style="background: #fde3e3">
          <p class="text-xl font-extrabold" style="color: #c02626">{{ estadoQuoia.error }}</p>
          <p class="mt-0.5 text-[11px] font-semibold" style="color: #c02626">Error</p>
        </div>
      </div>
      <div
        v-if="estadoQuoia.fallidas.length"
        class="mt-3 pt-3"
        style="border-top: 1px solid #e8e0f0"
      >
        <p class="mb-2 text-xs font-bold" style="color: #c02626">
          ⚠ {{ estadoQuoia.fallidas.length }} con error
        </p>
        <div
          v-for="f in estadoQuoia.fallidas"
          :key="f.frontera_id + f.tipo"
          class="mb-1.5 rounded-lg px-2.5 py-1.5 text-xs"
      ...
      <p
        ...
        class="mt-3 pt-3 text-xs font-semibold"
        style="color: #4d7c0f; border-top: 1px solid #e8e0f0"
      >
    ...
    <TabView v-model:activeIndex="activeTab">
      <TabPanel header="Revisión de hoy">
        ...
        <div v-else-if="!filas.length" class="py-12 text-center" style="color: #9b89b5">
        ...
        <div v-else class="workspace">
          ...
          <div class="detail-pane">
            <p v-if="!seleccion" class="py-16 text-center text-sm" style="color: #9b89b5">
      ...
      <TabPanel header="Historial">
        ...
        <div v-else-if="filasHistorial.length" class="workspace">
          ...
          />
          <div class="detail-pane">
            <p v-if="!seleccionHistorial" class="py-16 text-center text-sm" style="color: #9b89b5">
              Elige una frontera de la lista para ver su detalle.
            </p>
            <ReporteEnergiaDetalleTab
              v-else
        ...
        <p v-else class="py-8 text-center text-sm" style="color: #9b89b5">
      ...
      <TabPanel header="Resumen">
        <div class="mb-4 flex flex-wrap items-end gap-3">
          <div>
            <label class="mb-1 block text-xs font-semibold" style="color: #6b5a8a">Desde</label>
            <Calendar
              v-model="resumenDesde"
              dateFormat="yy-mm-dd"
              class="w-40"
              :maxDate="resumenHasta"
              showIcon
            />
          </div>
          <div>
            <label class="mb-1 block text-xs font-semibold" style="color: #6b5a8a">Hasta</label>
            <Calendar
              v-model="resumenHasta"
              dateFormat="yy-mm-dd"
              class="w-40"
              :minDate="resumenDesde"
              :maxDate="maxFecha"
              showIcon
            />
          </div>
          <Button
            label="Buscar"
            :loading="loadingResumenHistorico"
            @click="cargarResumenHistorico"
          />
        </div>
        ...
        <template v-else-if="resumenHistorico">
          ...
          <section class="mb-6">
            ...
            <div
              ...
              style="border-color: #e8e0f0; height: 220px"
            >
              <Bar :data="chartGen" :options="chartOptionsGen" :plugins="[dataLabelPlugin]" />
            </div>
            <p v-else class="py-8 text-center text-xs" style="color: #9b89b5">
              Sin datos en este rango.
            </p>

            <div v-if="grupoSeleccionadoGen" class="mt-4">
              <div class="mb-2 flex items-center justify-between">
                <p class="flex items-center gap-1.5 text-sm font-bold" style="color: #2c2039">
                  <span
              ...
              <DataTable
                :value="detalleFiltrado('gen')"
                class="resumen-tabla text-sm"
                stripedRows
                rowHover
                paginator
                :rows="10"
                @row-click="(e) => irAFronteraHistorial(e.data.frontera_id)"
              >
                ...
                <Column
                  field="dias_grupo"
                  :header="`Días en ${grupoSeleccionadoGen.toLowerCase()}`"
                  sortable
                  style="width: 150px"
                />
                <Column
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      >
                        <div
                          ...
                          :style="{
                            width: pctDe(data.dias_grupo, data.dias_totales) + '%',
                            background: severidadColor(pctDe(data.dias_grupo, data.dias_totales)),
                          }"
                        />
                      </div>
                      <span class="w-10 text-right text-xs font-bold"
                        >{{ pctDe(data.dias_grupo, data.dias_totales) }}%</span
                      >
                    </div>
                  </template>
                </Column>
                <Column header="Fuente(s) usadas">
                  <template #body="{ data }">
                    <span
                      v-for="d in data.desglose"
                      :key="d.etiqueta"
                      class="mr-1 mb-1 inline-block rounded-full px-2 py-0.5 text-[11px] font-semibold"
                      style="background: #f0ebf6; color: #6b5a8a"
          ...
          <section class="mb-6">
            ...
            <div v-if="grupoSeleccionadoCon" class="mt-4">
              ...
              >
                ...
                <Column
                  field="dias_grupo"
                  :header="`Días en ${grupoSeleccionadoCon.toLowerCase()}`"
                  sortable
                  style="width: 150px"
                />
                <Column
          ...
          <!-- Datos incompletos -->
          <div class="mb-5 rounded-xl border bg-white p-4 shadow-sm" style="border-color: #e8e0f0">
            <p class="mb-1 text-sm font-bold" style="color: #2c2039">
              Datos incompletos de medidores e inversores
            </p>
            <p class="mb-3 text-xs" style="color: #9b89b5">
              Solo Generación — cuántas veces cada fuente llegó incompleta en el rango.
            </p>
            <div
              class="mb-3 flex flex-wrap items-center gap-3 rounded-lg px-3 py-2.5"
              style="background: #faf9fc"
            >
              <template v-for="(c, idx) in resumenHistorico.incompletos_callouts" :key="idx">
                <span
                  ><b class="text-base font-extrabold" style="color: #6e3fb8">{{ c.valor }}</b>
                  <span class="ml-1.5 text-xs" style="color: #6b5a8a">{{ c.etiqueta }}</span></span
                >
                <span
                  v-if="idx < resumenHistorico.incompletos_callouts.length - 1"
                  class="h-4 w-px"
            ...
            <DataTable
              :value="resumenHistorico.incompletos"
              class="resumen-tabla text-sm"
              stripedRows
              rowHover
              paginator
            ...
            >
              ...
              <Column
                header="Medidor principal"
                sortable
                :sortField="'veces_medidor_principal_incompleto'"
                style="width: 170px"
              >
                <template #body="{ data }">
                  <span
                    v-if="
                      esCriticoProblema(
                        pctDe(data.veces_medidor_principal_incompleto, data.dias_con_fila),
                      )
                    "
                    class="inline-flex rounded-full px-2.5 py-1 text-xs font-bold"
                    :style="chipEstilo()"
                  >
                    {{ data.veces_medidor_principal_incompleto }} de {{ data.dias_con_fila }} ·
                    {{ pctDe(data.veces_medidor_principal_incompleto, data.dias_con_fila) }}%
                  </span>
              ...
              <Column
                header="Medidor respaldo"
                sortable
                :sortField="'veces_medidor_respaldo_incompleto'"
                style="width: 170px"
              >
                <template #body="{ data }">
                  <span
                    v-if="
                      esCriticoProblema(
                        pctDe(data.veces_medidor_respaldo_incompleto, data.dias_con_fila),
                      )
                    "
                    class="inline-flex rounded-full px-2.5 py-1 text-xs font-bold"
                    :style="chipEstilo()"
                  >
                    {{ data.veces_medidor_respaldo_incompleto }} de {{ data.dias_con_fila }} ·
                    {{ pctDe(data.veces_medidor_respaldo_incompleto, data.dias_con_fila) }}%
                  </span>
                  <span v-else class="text-xs" :style="ESTILO_PLANO">
                    {{ data.veces_medidor_respaldo_incompleto }} de {{ data.dias_con_fila }} ·
              ...
              <Column
                header="Inversores"
                sortable
                :sortField="'veces_solenium_incompleto'"
                style="width: 170px"
              >
                <template #body="{ data }">
                  <span
                    v-if="
                      esCriticoProblema(pctDe(data.veces_solenium_incompleto, data.dias_con_fila))
                    "
                    class="inline-flex rounded-full px-2.5 py-1 text-xs font-bold"
                    :style="chipEstilo()"
                  >
                    {{ data.veces_solenium_incompleto }} de {{ data.dias_con_fila }} ·
                    {{ pctDe(data.veces_solenium_incompleto, data.dias_con_fila) }}%
                  </span>
                  <span v-else class="text-xs" :style="ESTILO_PLANO">
                    {{ data.veces_solenium_incompleto }} de {{ data.dias_con_fila }} ·
                    {{ pctDe(data.veces_solenium_incompleto, data.dias_con_fila) }}%
            ...
            <p
              v-if="!resumenHistorico.incompletos.length"
              class="py-6 text-center text-xs"
              style="color: #9b89b5"
            >
              Sin datos incompletos en este rango.
            </p>
          </div>
        </template>
...
const fecha = ref(
  tabInicial === 0 && route.query.fecha ? parseFechaISO(route.query.fecha) : ayerColombia(),
)
const fechaHistorial = ref(
  tabInicial === 1 && route.query.fecha ? parseFechaISO(route.query.fecha) : ayerColombia(),
)
...
async function cargarResumenHistorico() {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: e.response?.data?.detail || 'No se pudo cargar el resumen histórico.',
      life: 4000,
    })
...
function chartOptionsPara(tipo, items) {
  return {
    ...
    scales: {
      x: {
        ticks: { font: { size: 11, weight: '600' }, color: '#6b5a8a' },
        grid: { display: false },
      },
...
// Semáforo de severidad -- "% de días con un problema" más alto es peor
// (incompletos, revisar manualmente, y el drill-down de fuente).
...
function esCriticoProblema(pct) {
  return pct > 30
}
...
function detalleFiltrado(tipo) {
  ...
  const detalle =
    tipo === 'gen'
      ? resumenHistorico.value?.detalle_fuente_generacion
      : resumenHistorico.value?.detalle_fuente_consumo
...
async function cargarResumen() {
  try {
    const { data } = await api.get('/reporte-energia/resumen', {
...
async function cargarLista(silent = false) {
  ...
  try {
    const { data } = await api.get('/reporte-energia/fronteras', {
      params: { fecha: fechaISO.value },
    })
    filas.value = data
  } catch (e) {
    if (!silent) {
      toast.add({
        severity: 'error',
        summary: 'Error',
        detail: 'No se pudo cargar el reporte de ese día.',
        life: 4000,
      })
      filas.value = []
    }
...
async function cargarEstadoQuoiaActual() {
  try {
    const { data } = await api.get('/reporte-energia/estado-quoia', {
      params: { fecha: fechaISO.value },
    })
...
async function revisarEstadoQuoia() {
  try {
    const { data } = await api.post('/reporte-energia/estado-quoia', null, {
      params: { fecha: fechaISO.value },
      timeout: 180000,
    })
...
async function ejecutarClasificacion() {
  ...
  try {
    ...
    toast.add({
      severity: 'info',
      summary: 'Clasificación iniciada',
      detail:
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: e.response?.data?.detail || 'No se pudo iniciar la clasificación.',
      life: 4000,
    })
...
async function detenerClasificacion() {
  ...
  try {
    await api.post('/reporte-energia/ejecutar/cancelar', null, {
      params: { fecha: fechaISO.value },
    })
    toast.add({
      severity: 'info',
      summary: 'Deteniendo…',
      detail: 'Se detiene después de terminar la frontera en curso, no de inmediato.',
      life: 5000,
    })
  } catch (e) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'No se pudo pedir la detención.',
      life: 4000,
    })
  } finally {
...
async function avisarSiHuboFallidas(fechaSondeada) {
  try {
    const { data } = await api.get('/reporte-energia/ejecutar/estado', {
      params: { fecha: fechaSondeada },
    })
    if (data.error_general) {
      toast.add({
        severity: 'error',
        summary: 'Clasificación interrumpida',
        detail: data.error_general,
        life: 8000,
      })
    } else if (data.cancelado) {
      toast.add({
        severity: 'warn',
        summary: 'Clasificación detenida',
      ...
      toast.add({
        severity: 'warn',
        summary: 'Clasificación terminada con errores',
...
function enviarReporte() {
  ...
  confirm.require({
    message:
      `Se va a enviar el reporte del ${fechaISO.value} a Quoia/ASIC` +
...
async function _enviarReporteConfirmado() {
  enviando.value = true
  try {
    const { data } = await api.post('/reporte-energia/enviar', null, {
      params: { fecha: fechaISO.value },
      timeout: 300000,
    })
    if (data.bloqueado) {
      toast.add({
        ...
        detail: data.motivo_bloqueo,
        life: 5000,
      })
    } else if (data.fallidos.length) {
      toast.add({
        severity: 'warn',
        summary: 'Reporte enviado con fallos',
      ...
      toast.add({
        ...
        detail: `${data.enviados} fronteras enviadas`,
        life: 3000,
      })
    }
    if (!data.bloqueado) {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: e.response?.data?.detail || 'No se pudo enviar el reporte.',
      life: 4000,
    })
  } finally {
