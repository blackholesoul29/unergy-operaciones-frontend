<template>
  <div class="min-h-screen space-y-5 p-5" style="background: #fdfaf7; color: #2c2039">
    <!-- Header -->
    <PageHeader
    ...
    >
      ...
      <template #actions>
        <button
          ...
          "
          v-tooltip.bottom="
            'Empresa responsable de cada PPA. Los contratos de un responsable no relevante se ocultan en toda esta página.'
          "
        >
        ...
        <label
          class="flex cursor-pointer items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold"
          :style="
            verOcultos
          ...
          "
          v-tooltip.bottom="
            'Muestra en TODAS las pestañas los contratos cuyo responsable está marcado como no relevante'
          "
        >
        ...
        <button
          ...
          class="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors"
          style="
            border: 1px solid rgba(214, 68, 85, 0.3);
            color: #d64455;
            background: rgba(214, 68, 85, 0.05);
          "
    ...
    <!-- Tab bar -->
    <div class="-mt-1 flex gap-1 border-b" style="border-color: rgba(44, 32, 57, 0.08)">
      <button
        v-for="(tab, i) in TABS"
        :key="i"
        @click="activeTab = i"
    ...
    <div v-show="activeTab === 1" class="space-y-6">
      ...
      <div class="flex flex-wrap items-end gap-3">
        <div class="flex flex-col gap-1">
          <label class="text-xs font-semibold tracking-wider uppercase" style="color: #915bd8"
            >Año</label
          >
      ...
      <template v-else-if="anualData">
        <div class="flex flex-wrap items-center gap-2 text-sm" style="color: #7a6e8a">
          <span class="text-base font-semibold" style="color: #2c2039">
        ...
        <div class="cv-panel p-4">
          <div ref="chartBox" class="relative select-none" style="width: 100%; height: 360px">
            ...
            >
              ...
              <g v-for="(mes, i) in anualData.meses" :key="i">
                ...
                <text
                  ...
                  font-size="11"
                  :fill="
                    selectedMonthIdx === i ? '#F0C040' : isCurrentMonth(mes) ? '#2C2039' : '#7a6e8a'
                  "
            ...
            >
              <div class="mb-2 font-bold" style="color: #f0c040">
                ...
                <span
                  v-if="anualData.meses[hovered].tipo_datos === 'mes_actual'"
                  class="ml-1 text-xs font-normal"
                  style="color: rgba(59, 186, 220, 0.85)"
                  >mes en curso</span
                >
                <span
                  v-else-if="anualData.meses[hovered].tipo_datos === 'proyeccion_historica'"
                  class="ml-1 text-xs font-normal"
              ...
              <div class="space-y-1">
                ...
                <template v-if="anualData.meses[hovered].tipo_datos === 'mes_actual'">
                  <div class="flex justify-between gap-6">
                    ...
                    <span class="font-mono font-semibold" style="color: #c4a1f0">{{
                      fmtMwh(anualData.meses[hovered].gen_mwh)
                    }}</span>
                  </div>
                  <div
                    v-if="cierreVal(anualData.meses[hovered])"
                  ...
                  >
                    ...
                    <span class="font-mono font-bold" style="color: rgba(59, 186, 220, 0.95)">{{
                      fmtMwh(cierreVal(anualData.meses[hovered]))
                    }}</span>
                  </div>
                  <div
                    v-if="anualData.meses[hovered].dias_restantes != null"
                    class="mt-0.5 text-xs"
                    style="color: rgba(253, 250, 247, 0.4)"
                  >
                    {{ anualData.meses[hovered].dia_actual }}d transcurridos ·
                    {{ anualData.meses[hovered].dias_restantes }}d restantes
                  </div>
                </template>
                ...
                <template v-else>
                  <div class="flex justify-between gap-6">
                    ...
                    <span class="font-mono font-semibold">{{
                      fmtMwh(genVal(anualData.meses[hovered]))
                    }}</span>
                  </div>
                </template>
                <div
                  v-if="anualData.meses[hovered].min_mwh !== null"
                  class="flex justify-between gap-6"
                >
                ...
                <div
                  v-if="anualData.meses[hovered].max_mwh !== null"
                  class="flex justify-between gap-6"
                >
          ...
          <div class="mt-3 flex flex-wrap gap-5 pl-1">
            ...
            <div class="flex items-center gap-2 text-xs" style="color: #7a6e8a">
              <div class="h-4 w-4 rounded-sm" style="background: #915bd8"></div>
              Generación real
            </div>
            <div class="flex items-center gap-2 text-xs" style="color: #7a6e8a">
              <div
      ...
      <div
        ...
        class="cv-panel py-16 text-center"
        style="color: #7a6e8a"
      >
        <i class="pi pi-chart-bar mb-3 block text-4xl" style="color: #915bd8" />
      ...
      <div>
        ...
        <DataTable
          ...
          size="small"
          stripedRows
          class="cv-panel cursor-pointer overflow-hidden"
          @row-click="(e) => selectContrato(e.data.id)"
          :rowClass="
            (row) =>
              row.id === selectedContratoId
                ? 'row-selected'
                : row.id === CONSOLIDADO_ID
                  ? 'row-consolidado'
                  : ''
          "
        >
          <Column header="Contrato" style="min-width: 200px">
            <template #body="{ data: row }">
              <div class="text-sm font-semibold" style="color: #2c2039">
              ...
              <div class="mt-0.5 text-xs" style="color: #7a6e8a">
                {{ row.comprador_nombre
                  ...
                  v-tooltip.top="
                    'Responsable: ' + row.responsable + ' — normalmente oculto en Cumplimiento'
                  "
                  >{{ row.responsable }}</span
                >
              </div>
            </template>
          </Column>
          <Column header="Vigencia" style="width: 190px">
            <template #body="{ data: row }">
              <span class="text-xs" style="color: #7a6e8a"
                >{{ fmtFecha(row.fecha_inicio) }} – {{ fmtFecha(row.fecha_fin) }}</span
              >
            </template>
          </Column>
          <Column header="Mín anual" style="width: 130px; text-align: right">
            <template #body="{ data: row }">
              <span v-if="row.total_min_mwh !== null" class="font-mono text-sm">{{
                fmtMwh(row.total_min_mwh)
              }}</span>
          ...
          <Column header="Máx anual" style="width: 130px; text-align: right">
            <template #body="{ data: row }">
              <span v-if="row.total_max_mwh !== null" class="font-mono text-sm">{{
                fmtMwh(row.total_max_mwh)
              }}</span>
              <span v-else class="text-xs" style="color: #b0a0c0">—</span>
            </template>
          </Column>
          <Column header="Meses" style="width: 90px; text-align: center">
            <template #body="{ data: row }">
              <span class="font-mono text-sm"
                >{{ row.meses_con_compromisos
                }}<span class="text-xs" style="color: #7a6e8a">/12</span></span
              >
            </template>
          </Column>
    ...
    <div v-show="activeTab === 0" class="space-y-5">
      ...
      <div class="flex flex-wrap items-end gap-3">
        ...
        <button v-if="hiddenContratos.size > 0" @click="showAllContratos" class="cv-btn">
          <i class="pi pi-eye text-xs" style="color: #2e7d32" />Mostrar ocultos ({{
        ...
        >
          <i
            class="pi text-xs"
            :class="sortDesc ? 'pi-sort-amount-down' : 'pi-sort-amount-up'"
            style="color: #915bd8"
          />
        ...
        <div class="flex flex-wrap items-center gap-1.5">
          <button
            ...
            :style="
              estadoFiltro === null
                ? 'border-color:#915BD8; background:rgba(145,91,216,0.10); color:#915BD8; font-weight:700;'
                : ''
            "
          >
            Todos
        ...
        <MultiSelect
          ...
          :maxSelectedLabels="2"
          selectedItemsLabel="{0} offtakers"
          class="text-sm"
        ...
        <span class="w-full text-xs" style="color: #9b8fb0"
          >Arrastra las plantas entre contratos para simular</span
        >
      </div>
      ...
      >
        <div class="mb-4 flex items-center gap-2">
          ...
          <span class="text-sm font-bold" style="color: #2c2039">Nuevo PPA nuevo</span>
        </div>
      ...
      <template v-else-if="simData">
          ...
          >
            ...
            >
              <div class="flex min-w-0 items-center gap-2">
                <i
                  class="pi flex-shrink-0 text-xs transition-transform"
                  :class="expandedContratos.includes(c.id) ? 'pi-chevron-down' : 'pi-chevron-right'"
                  style="color: #915bd8"
                />
                <div class="min-w-0">
                  <div class="flex flex-wrap items-center gap-1.5">
                    <span class="text-sm font-bold break-words" style="color: #2c2039">{{
                      c.nombre
                    }}</span>
                    <span
                      v-if="c._ficticio"
                    ...
                    >
                    <span
                      v-if="simResults[c.id]?.plantasEsp != null"
                      class="flex-shrink-0 rounded px-1.5 py-0.5 text-xs font-semibold"
                      :style="estadoBadge(simResults[c.id].estadoPlantas)"
                      v-tooltip="
                        'Plantas inscritas (registradas y despachando vía GESCON) / plantas contrato (exigidas) — este mes'
                      "
                      >{{ simResults[c.id].plantasReg }}/{{
                        simResults[c.id].plantasEsp
                      }}
                      plantas</span
                    >
                    ...
                    >
                    <span
                      v-if="simResults[c.id]?.pct !== null && simResults[c.id]?.pct !== undefined"
                      class="flex-shrink-0 rounded px-1.5 py-0.5 text-xs font-semibold"
              ...
              <div class="mb-3 grid grid-cols-3 gap-x-3">
                <div>
                  ...
                  <div class="mt-1 flex flex-wrap items-baseline gap-1">
                    <span class="font-mono text-xs font-bold" style="color: #2c2039">{{
                      fmtMwh(simResults[c.id].gen)
                    }}</span>
                    <span
                      v-if="simResults[c.id].genDup > 0"
                      ...
                      ><i class="pi pi-shopping-cart" style="font-size: 9px" />{{
                        fmtMwh(simResults[c.id].genDup)
                      }}
                      bolsa</span
                    >
                  </div>
                ...
                <div>
                  ...
                  <div class="mt-1 font-mono text-xs font-bold" style="color: #2c2039">
                    {{ simResults[c.id].min !== null ? fmtMwh(simResults[c.id].min) : '—' }}
                  </div>
                </div>
                <div>
                  <div
                  ...
                  <div class="mt-1 font-mono text-xs font-bold" style="color: #2c2039">
                    {{
                      simResults[c.id].genProy != null && simResults[c.id].genProy > 0
                        ? fmtMwh(simResults[c.id].genProy)
                        : '—'
                    }}
                  </div>
                </div>
              ...
              <div class="space-y-1.5">
                ...
                >
                  ...
                  <template v-if="simResults[c.id].bullet.hasZones">
                    <div
                      v-if="simResults[c.id].bullet.hasMin"
                      class="absolute inset-y-0"
                      :style="{
                        left: '0',
                        width: simResults[c.id].bullet.minPct + '%',
                    ...
                    <div
                      class="absolute inset-y-0"
                      :style="{
                        left: simResults[c.id].bullet.minPct + '%',
                        width:
                          simResults[c.id].bullet.maxPct - simResults[c.id].bullet.minPct + '%',
                        background: 'rgba(46,125,50,0.11)',
                      }"
                    />
                    <div
                      v-if="simResults[c.id].bullet.hasMax"
                      class="absolute inset-y-0"
                      :style="{
                        left: simResults[c.id].bullet.maxPct + '%',
                        right: '0',
                  ...
                  <div
                    ...
                    :style="{
                      width: simResults[c.id].bullet.measurePct + '%',
                      background: estadoColor(simResults[c.id].estado),
                    }"
                  />
                  <!-- Compra en bolsa: tramo amarillo DENTRO de la barra entregada -->
                  <div
                    v-if="simResults[c.id].bullet.dupW"
                    ...
                    "
                    :style="{
                      left: simResults[c.id].bullet.bolsaStartPct + '%',
                      width: simResults[c.id].bullet.dupW + '%',
                    }"
                  ...
                  <div
                    ...
                    :style="{ left: simResults[c.id].bullet.minPct + '%' }"
                    v-tooltip="'Mínimo: ' + fmtMwh(simResults[c.id].min)"
                  />
                  <div
                    v-if="simResults[c.id].bullet.hasMax"
                    class="absolute rounded"
                    style="top: -1px; bottom: -1px; width: 2px; background: #2c2039; opacity: 0.45"
                    :style="{ left: simResults[c.id].bullet.maxPct + '%' }"
                    v-tooltip="'Máximo: ' + fmtMwh(simResults[c.id].max)"
                  />
                  <!-- Proyección de cierre (diamante) -->
                  <div
                    v-if="simResults[c.id].bullet.proyPct != null"
                    class="absolute"
                    style="
                      top: 50%;
                      width: 11px;
                ...
                <div class="flex flex-wrap items-center gap-1.5">
                  ...
                  <span
                    v-if="simResults[c.id].genProy != null && simResults[c.id].genProy > 0"
              ...
              >
                <div class="min-w-0">
                  <span
                    ...
                    style="color: #2c2039; max-width: 128px"
                    >{{ p.nombre }}</span
                  >
                  <span
        ...
        <div
          ...
          :style="
            dragOver === 'none'
              ? 'border-color: #915BD8; box-shadow: 0 0 0 2px rgba(145,91,216,0.18);'
              : ''
          "
    ...
    <div v-show="activeTab === 2" class="space-y-5">
      ...
      <div class="flex flex-wrap items-end gap-4">
            ...
            >
              ...
              >
                ...
                <span
                  ...
                  v-tooltip.bottom="pcFechaCorte ? `Vigentes al ${pcFechaCorte}` : 'Vigentes'"
                ...
                <span
                  ...
                  v-tooltip.bottom="
                    `${pcCountsTerminados[mode.key]} terminaron durante el mes (siguen listados abajo, en rojo)`
                  "
        ...
        <Button
          ...
          v-tooltip.bottom="
            'Descarga TODAS las categorías del mes, incl. plantas externas (todos los contratos y plantas), sin importar el filtro activo'
          "
          style="color: #915bd8; border-color: #915bd8"
        />
      </div>
      ...
      <template v-else-if="pcData">
        ...
        <template v-if="pcMode === 'ppa_venta_ungg'">
          ...
          >
            <div class="flex flex-wrap items-center gap-x-3 gap-y-1">
              ...
              <span
                ...
                style="color: #9a6700"
              >
                <i class="pi pi-shopping-cart" style="font-size: 10px" /><b>{{
                  pcVentaDupInfo.dup
                }}</b>
              ...
              <span
                ...
                style="color: #0369a1"
              >
                <i class="pi pi-sync" style="font-size: 10px" /><b>{{ pcVentaDupInfo.ur }}</b> uso
                del recurso
              </span>
          ...
          <div v-for="c in ppaVentaFiltrado" :key="c.id" class="cv-card">
            <div
              class="cv-row-click flex items-center justify-between px-4 py-3"
              style="
                background: rgba(145, 91, 216, 0.04);
            ...
            >
              <div>
                <span class="text-sm font-bold" style="color: #2c2039">{{ c.nombre }}</span>
              ...
              <div class="flex flex-shrink-0 items-center gap-2">
                ...
                >
                  <i class="pi pi-sync" style="font-size: 9px" />{{
                ...
                <button
                  ...
                  "
                  v-tooltip.left="
                    'Copia la imagen al portapapeles (o la descarga si el navegador no lo permite)'
                  "
                >
                  <i
                    class="pi text-xs"
                    :class="copiadoVentaId === c.id ? 'pi-check' : 'pi-image'"
                  />
            ...
            >
              <div
                ...
                :key="filaKey(p)"
                class="cv-row-click flex items-center justify-between px-4 py-2.5 text-sm"
                :style="
                  p.uso_del_recurso
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span
                    ...
                    v-tooltip="
                      'Salió de esta modalidad durante el mes — su tramo siguiente está en otra piscina'
                    "
                    ><i class="pi pi-sign-out" style="font-size: 9px" />Terminó</span
                  >
                  <span
                    ...
                    style="background: rgba(2, 132, 199, 0.14); color: #0369a1"
                    v-tooltip="
                      'Uso del recurso: la planta está en bolsa y se le paga al cliente su generación a precio bolsa — también listada en c. Compra en Bolsa (UNGG). No genera garantías.'
                    "
                    ><i class="pi pi-sync" style="font-size: 9px" />Uso del recurso</span
                  >
                  <span
                    ...
                    style="background: rgba(240, 192, 64, 0.22); color: #9a6700"
                    v-tooltip="
                      'Duplicado: suministra a este contrato con origen bolsa — también listado en c. Compra en Bolsa (UNGG). Genera garantías.'
                    "
                    ><i class="pi pi-shopping-cart" style="font-size: 9px" />Duplicado</span
                  >
                  <span
                    v-if="p.codigo_sic"
        ...
        <template v-if="pcMode === 'ppa_compra_externa'">
          <Message v-if="externasSinPlantas.length" severity="warn" :closable="false">
            <span v-if="externasSinPlantas.length === 1">
              El contrato <b>{{ externasSinPlantas[0].nombre }}</b> no tiene plantas vinculadas: no
              sabemos a qué planta le estamos comprando. Asóciala en el módulo PPA.
            </span>
            <span v-else>
          ...
          <div v-for="c in pcPools.ppa_compra_externa" :key="c.id" class="cv-card-gold">
            ...
            >
              ...
              <div class="flex flex-shrink-0 items-center gap-2">
                ...
                >
                  <i class="pi pi-exclamation-triangle" style="font-size: 10px" />Sin plantas
                  vinculadas
                </span>
                <span
                  v-else
          ...
          >
            ...
            <div class="flex flex-wrap gap-x-4 gap-y-1">
              <span class="inline-flex items-center gap-1" style="color: #9a6700">
                <i class="pi pi-shopping-cart" style="font-size: 10px" /><b>Duplicado</b> — su
                energía se compra en bolsa (genera garantías).
              </span>
              <span class="inline-flex items-center gap-1" style="color: #0369a1">
                <i class="pi pi-sync" style="font-size: 10px" /><b>Uso del recurso</b> — se le paga
                al cliente a precio bolsa (sin garantías).
              </span>
            </div>
          </div>
            ...
            <div class="divide-y" style="border-color: rgba(44, 32, 57, 0.05)">
              ...
              >
                <div class="flex items-center gap-2">
                  <i
                    class="pi"
                    :class="p.uso_del_recurso ? 'pi-sync' : 'pi-shopping-cart'"
                  ...
                  <span
                    ...
                    style="background: rgba(2, 132, 199, 0.14); color: #0369a1"
                    v-tooltip="
                      'Uso del recurso: la planta está en bolsa y también suministra a un contrato de venta (Venta · UNGG); se le paga al cliente a precio bolsa. No genera garantías.'
                    "
                  ...
                  <span
                    ...
                    style="background: rgba(240, 192, 64, 0.22); color: #9a6700"
                    v-tooltip="
                      'Duplicado: esta planta también suministra a un contrato de venta (Venta · UNGG); su energía a este contrato se compra en bolsa. Genera garantías.'
                    "
                    >Duplicado</span
                  >
                  <span
        ...
        <template v-if="pcMode === 'bolsa_venta_ungg'">
          <div v-if="pcPools.bolsa_venta_ungg.length" class="cv-card">
            <div
              ...
              "
            >
              <span class="text-sm font-bold" style="color: #2c2039">Venta en Bolsa (UNGG)</span>
              <span class="ml-2 text-xs" style="color: #7a6e8a"
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span
                    ...
                    style="background: rgba(214, 68, 85, 0.12); color: #d64455"
                    v-tooltip="
                      'Estuvo libre en bolsa solo este tramo del mes; después entró a otra modalidad'
                    "
                    ><i class="pi pi-sign-out" style="font-size: 9px" />Terminó</span
                  >
                </div>
        ...
        <template v-if="pcMode === 'bolsa_venta_ungc'">
          <div v-if="pcPools.bolsa_venta_ungc.length" class="cv-card">
            <div
              ...
              "
            >
              <span class="text-sm font-bold" style="color: #2c2039">Venta en Bolsa (UNGC)</span>
              <span class="ml-2 text-xs" style="color: #7a6e8a"
            ...
            <div class="divide-y" style="border-color: rgba(44, 32, 57, 0.05)">
              <div
                ...
                :key="filaKey(p)"
                class="cv-row-click px-4 py-2.5 text-sm font-medium"
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span
                    ...
                    class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] font-semibold"
                    style="background: rgba(214, 68, 85, 0.12); color: #d64455"
                    v-tooltip="'Estuvo en esta modalidad solo un tramo del mes'"
                    ><i class="pi pi-sign-out" style="font-size: 9px" />Terminó</span
                  >
                </div>
                <div
                  ...
                  style="color: #7a6e8a"
                  v-tooltip.right="
                    'Vigencia del registro SIC con comprador UNGC que pone la planta en esta modalidad'
                  "
                >
    ...
    <div v-show="activeTab === 3" class="space-y-5">
      ...
      <div class="flex flex-wrap items-end gap-4">
        <div class="flex flex-col gap-1">
          ...
          <Select
            v-model="etYear"
            :options="etYearOptions"
            class="w-24"
            @change="onEtPeriodChange"
          />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs font-semibold tracking-wider uppercase" style="color: #915bd8"
        ...
        >
          <i class="pi pi-history mr-1 text-xs" />histórico local
        </span>
      </div>
      ...
      <template v-else-if="etData">
        <Message v-if="etData.warning" severity="warn" :closable="false">{{
          etData.warning
        }}</Message>
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <div class="cv-card px-4 py-3">
            ...
            <p class="font-mono text-2xl font-bold" style="color: #2c2039">
          ...
          <div class="cv-card px-4 py-3">
            <p class="mb-1 text-xs font-semibold tracking-wider uppercase" style="color: #915bd8">
              Vía PPA
            </p>
            <p class="font-mono text-2xl font-bold" style="color: #915bd8">
          ...
          <div class="cv-card px-4 py-3">
            <p class="mb-1 text-xs font-semibold tracking-wider uppercase" style="color: #2c2039">
              En bolsa
            </p>
            <p class="font-mono text-2xl font-bold" style="color: #2c2039">
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-2 py-2.5">
                  <div class="flex flex-wrap gap-1">
                    ...
                    <span
                      ...
                      class="rounded px-1.5 py-0.5 text-xs font-medium"
                      style="background: rgba(44, 32, 57, 0.08); color: #2c2039"
                      >Bolsa</span
                    >
                ...
                <td class="px-2 py-2.5 text-right font-mono" style="color: #915bd8">
                  {{ p.ppa_mwh !== null ? fmtMwh(p.ppa_mwh) : '—' }}
                </td>
                <td class="px-2 py-2.5 text-right font-mono" style="color: #2c2039">
                  {{ p.bolsa_mwh !== null ? fmtMwh(p.bolsa_mwh) : '—' }}
                </td>
                <td class="px-4 py-2.5 text-right font-mono font-semibold" style="color: #2c2039">
            ...
            <tfoot>
              ...
              >
                ...
                <td></td>
                <td class="px-2 py-3 text-right font-mono font-bold" style="color: #915bd8">
                  {{ fmtMwh(etData.totales.ppa_mwh) }}
                </td>
                <td class="px-2 py-3 text-right font-mono font-bold" style="color: #2c2039">
    ...
    <div v-show="activeTab === 4" class="space-y-4">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-2">
          <Select
            v-model="anualMatrizYear"
            :options="years"
            class="w-28"
            @change="loadAnualMatriz"
          />
          ...
          <MultiSelect
            ...
            :maxSelectedLabels="2"
            selectedItemsLabel="{0} contratos"
            class="text-sm"
          ...
          >
            <template #option="{ option }">
              <div class="flex flex-col leading-tight">
                ...
                <span v-if="option.offtaker" class="text-xs" style="color: #7a6e8a">{{
                  option.offtaker
                }}</span>
              </div>
            </template>
      ...
      <Message v-else-if="anualMatrizError" severity="error" :closable="false">{{
        anualMatrizError
      }}</Message>
      <div
              ...
              <tr class="cv-matriz-contrato cursor-pointer" @click="toggleMatriz(c.id)">
                <td class="sticky-col px-3 py-1.5">
                  <i
                    ...
                    :class="expandedMatriz.includes(c.id) ? 'pi-chevron-down' : 'pi-chevron-right'"
                  />
                  ...
                  <i
                    v-if="c._loading"
                    class="pi pi-spin pi-spinner ml-1 text-xs"
                    style="color: #915bd8"
                  />
                </td>
                <td
    ...
    <div v-show="activeTab === 5" class="space-y-5">
      ...
      <template v-else-if="beData">
        <Message v-if="beData.warning" severity="warn" :closable="false">{{
          beData.warning
        }}</Message>
        ...
        <template v-else>
          ...
          <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            <div class="cv-card px-5 py-4">
              <div class="mb-1 flex items-baseline justify-between">
                <h3 class="text-sm font-bold tracking-wider uppercase" style="color: #2c2039">
                  UNGG · generador
                </h3>
              ...
              <table class="w-full text-sm">
                ...
                <tbody>
                  <tr
                    class="be-linea"
                    :class="{ activa: beFiltro === 'e' }"
                    @click="beAbrirCapa('e')"
                    :title="beTitulo('e')"
                  >
                    <td class="py-2">
                      <span class="font-medium" style="color: #2c2039">Venta en bolsa</span>
                      <span class="be-chip be-chip-alerta"
                        ><i class="pi pi-exclamation-triangle" /> cargos regulatorios</span
                      >
                    </td>
                    <td class="text-right font-mono" style="color: #2c2039">
                      {{ fmtNum1(beB.ungg.venta_bolsa.real) }}
                    </td>
                    <td class="text-right font-mono" style="color: #7a6e8a">
                      {{ fmtNum1(beB.ungg.venta_bolsa.proyectado) }}
                    </td>
                    <td class="text-right font-mono font-semibold" style="color: #2c2039">
                  ...
                  >
                    <td class="py-2 pl-3">
                      ...
                      <span class="be-chip be-chip-alerta"
                        ><i class="pi pi-shield" /> garantías</span
                      >
                    </td>
                    <td class="text-right font-mono" style="color: #2c2039">
            ...
            <div class="cv-card px-5 py-4">
              ...
              >
                <p class="mb-1">
                  <strong style="color: #2c2039">{{ beB.ungg.venta_bolsa.n_plantas }}</strong>
                  plantas venden en bolsa por UNGG ·
                  <strong style="color: #2c2039">{{ beB.ungc.venta_bolsa.n_plantas }}</strong> por
                  UNGC
                </p>
                <p>
                  Balance de
                  <strong style="color: #2c2039">{{ MESES[beMonth - 1] }} {{ beYear }}</strong>
          ...
          >
            ...
            <p class="mb-1 text-xs">
              Su energía ya está comprada por contrato fuera de GESCON, así que contarlas como venta
              en bolsa infla la línea de cargos regulatorios. Marca la casilla de arriba para ver el
              balance sin ellas.
            </p>
          ...
          >
            <p class="text-xs">
              <strong>{{ beData.advertencias.sin_datos.length }}</strong> plantas sin generación en
              el mes, excluidas del balance:
              {{ beData.advertencias.sin_datos.map((a) => a.planta).join(', ') }}
            </p>
          </Message>

          <Message
            v-if="beData.advertencias.tramos_estimados.length"
            severity="secondary"
            :closable="false"
          >
            <p class="text-xs">
          ...
          <div class="flex flex-wrap items-end justify-between gap-3">
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span
                ...
                @click="beFiltro = null"
              >
                {{ BE_CATEGORIAS[beFiltro].label }} <i class="pi pi-times ml-1 text-[10px]" />
              </span>
            </div>
            <div class="flex flex-wrap items-center gap-2">
              <InputText
      ...
      <template v-else-if="revData">
        ...
        <div class="grid gap-3" style="grid-template-columns: repeat(auto-fit, minmax(220px, 1fr))">
          ...
          >
            ...
            <div class="mt-1.5 text-xs font-semibold" style="color: #2c2039">{{ s.titulo }}</div>
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-4 py-2">
                  ...
                  >
                    <span
                      :style="a.estado === 'terminado' ? `color:${ROJO_FIN};` : 'color:#2C2039;'"
                      >{{ a.contrato }}</span
                    >
                    <span
                      v-if="a.codigo_sic"
            ...
            <table class="mt-2 w-full">
              <tbody>
                <tr
                  ...
                  style="border-color: rgba(44, 32, 57, 0.06)"
                >
                  <td class="py-1.5 pr-4 font-medium" style="color: #2c2039; width: 220px">
                    {{ p.nombre }}
                  </td>
        ...
        >
          <div class="px-4 py-2.5" style="background: rgba(2, 132, 199, 0.1)">
            <span class="text-sm font-semibold" style="color: #2c2039">Uso del recurso</span>
            <span class="ml-2 text-xs" style="color: #7a6e8a">
        ...
        >
          ...
          <div class="px-6 pt-4 pb-3" style="border-bottom: 1px solid rgba(44, 32, 57, 0.08)">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                <div class="text-lg font-bold" style="color: #2c2039">
                ...
                <span
                  ...
                  style="background: rgba(145, 91, 216, 0.1); color: #915bd8"
                >
                  <i class="pi pi-calendar text-[10px]" /> {{ MESES[beMonth - 1] }} {{ beYear }} ·
                  {{ bePeriodoLabel }}
                </span>
              </div>
              <div class="flex flex-shrink-0 items-center gap-2">
            ...
            <table class="w-full text-sm">
                ...
                >
                  <td class="py-2 pr-2">
                    ...
                    <span
                      ...
                      >{{ f.planta }}</span
                    >
                    <span v-if="f.contrato" class="block text-[11px]" style="color: #915bd8">{{
                      f.contrato
                    }}</span>
                  </td>
                  ...
                  <td class="py-2 text-right font-mono" style="color: #2c2039">
                    {{ f.mwh_real !== null ? fmtNum1(f.mwh_real) : '—' }}
                  </td>
                  <td class="py-2 text-right font-mono" style="color: #7a6e8a">
                    {{ f.mwh_proyectado !== null ? fmtNum1(f.mwh_proyectado) : '—' }}
                  </td>
                  <td class="py-2 text-right font-mono font-semibold" style="color: #2c2039">
                    {{ f.mwh_total !== null ? fmtNum1(f.mwh_total) : '—' }}
                  </td>
                </tr>
              </tbody>
              <tfoot>
                <tr style="border-top: 2px solid rgba(44, 32, 57, 0.12)">
                  ...
                  <td class="pt-3 text-right font-mono text-base font-bold" style="color: #2c2039">
      ...
      <template v-if="detalleCapa">
        ...
        >
          ...
          <div class="px-6 pt-4 pb-3" style="border-bottom: 1px solid rgba(44, 32, 57, 0.08)">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                <div class="truncate text-lg font-bold" style="color: #2c2039">
                ...
                <span
                  ...
                  style="background: rgba(145, 91, 216, 0.1); color: #915bd8"
                >
                  <i class="pi pi-calendar text-[10px]" /> Período de consulta:
                  {{ periodoSimLabel }}
                </span>
              </div>
              <div class="flex flex-shrink-0 items-center gap-2">
                ...
                >
                  <i
                    class="pi text-xs"
                    :class="copiadoCapaId === detalleCapa.c.id ? 'pi-check' : 'pi-image'"
                  />
            ...
            <div
              ...
              :class="detalleCapa.res.genDup > 0 ? 'grid-cols-4' : 'grid-cols-3'"
            >
              <div>
                <div
                ...
                <div class="mt-0.5 font-mono text-sm font-bold" style="color: #2c2039">
              ...
              <div
                v-if="detalleCapa.res.genDup > 0"
                v-tooltip.bottom="
                  'De la energía entregada, esta parte se suministra con compra en bolsa (cuenta para el contrato, origen bolsa)'
                "
              >
                <div
                  ...
                  style="color: #9a6700"
                >
                  <i class="pi pi-shopping-cart" style="font-size: 9px" />Compra en bolsa
                </div>
          ...
          <!-- Tabla -->
          <div class="px-6 py-4">
            <p class="mb-2 text-xs font-bold tracking-widest uppercase" style="color: #915bd8">
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                ...
                >
                  ...
                  <td
                    ...
                    :style="p.es_duplicado ? 'color:#9a6700' : 'color:#2C2039'"
                  >
                    {{ p.month_mwh != null ? fmtMwh(p.month_mwh * p.pct_despacho) : '—' }}
                  </td>
              ...
              <tfoot>
                <tr style="border-top: 2px solid rgba(44, 32, 57, 0.12)">
                  <td class="pt-3 text-sm font-bold" style="color: #2c2039">
                    Total · {{ detalleCapa.plantas.length }} proyecto{{
                      detalleCapa.plantas.length === 1 ? '' : 's'
                    }}
                  </td>
                  <td></td>
                  <td class="pt-3 text-right font-mono text-base font-bold" style="color: #915bd8">
                    {{ fmtMwh(detalleCapa.res.gen) }}
                  </td>
                  <td class="pt-3 text-right font-mono text-base font-bold" style="color: #915bd8">
                    {{
            ...
            >
              <i
                class="pi mt-0.5"
                :class="veredictoCapa(detalleCapa.res).icon"
    ...
    <Teleport to="body">
      <template v-if="selectedMonthIdx !== null && anualData && anualData.meses[selectedMonthIdx]">
        ...
        >
          ...
          >
            <div>
              <span class="text-base font-bold" style="color: #2c2039"
          ...
          <div class="px-5 py-4">
            <p class="mb-3 text-xs font-semibold tracking-widest uppercase" style="color: #915bd8">
              Desglose por planta
            </p>
            <table class="w-full text-sm">
              <thead>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="px-2 py-2 text-right font-mono" style="color: #2c2039">
                    {{ p.gen_planta_mwh !== null ? fmtMwh(p.gen_planta_mwh) : '—' }}
                  </td>
                  <td
                    class="py-2 pl-2 text-right font-mono font-semibold"
                    :style="p.es_duplicado ? 'color: #9a6700;' : 'color: #915BD8;'"
                  >
              ...
              <tfoot>
                <tr style="border-top: 2px solid rgba(44, 32, 57, 0.12)">
                  <td colspan="3" class="pt-3 text-sm font-semibold" style="color: #2c2039">
                    Total al contrato
                  </td>
                  <td class="pt-3 text-right font-mono font-bold" style="color: #2c2039">
                ...
                <tr v-if="anualData.meses[selectedMonthIdx].exposicion_bolsa_duplicados_mwh">
                  ...
                  <td class="pt-1 text-right font-mono font-bold" style="color: #9a6700">
                    {{ fmtMwh(anualData.meses[selectedMonthIdx].exposicion_bolsa_duplicados_mwh) }}
                  </td>
                </tr>
              </tfoot>
            </table>
        ...
        >
          ...
          <div class="px-6 pt-4 pb-3" style="border-bottom: 1px solid rgba(44, 32, 57, 0.08)">
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                <div class="flex flex-wrap items-center gap-2">
                  <span class="text-lg font-bold" style="color: #2c2039">{{
              ...
              <div class="flex flex-shrink-0 items-center gap-2">
                <a
                  ...
                  class="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold"
                  style="background: rgba(145, 91, 216, 0.1); color: #915bd8"
                  v-tooltip.left="'Abrir la carpeta del contrato'"
                  ><i class="pi pi-folder-open text-xs" />Carpeta</a
                >
                <button
                  class="rounded-lg p-1.5 transition-colors hover:bg-gray-100"
                  style="color: #7a6e8a"
          ...
          <div v-else class="space-y-6 px-6 py-4">
            ...
            <div>
              ...
              <Message v-if="!dcPpa && !dcError" severity="info" :closable="false">
                Este contrato no está registrado en el módulo PPA — solo existe como registro GESCON
                (abajo).
              </Message>
                ...
                <table class="w-full text-sm">
                  ...
                  <tbody>
                    <tr
                      ...
                      "
                    >
                      <td class="px-3 py-1.5 font-mono text-xs" style="color: #2c2039">
                        {{ MESES[r.mes - 1] }} {{ r['año'] }}
                      </td>
                      <td class="px-3 py-1.5 text-right font-mono" style="color: #2c2039">
            ...
            <div>
              ...
              <div
                v-if="dcGesconResumen.length"
                class="mb-3 grid grid-cols-2 gap-x-6 gap-y-3 rounded-lg px-3 py-2.5 sm:grid-cols-4"
                ...
                <table class="w-full text-sm">
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td class="px-2 py-1.5 text-right font-mono text-xs" style="color: #915bd8">
                        {{
          ...
          <div class="space-y-5 px-6 py-4">
            ...
            <div>
              ...
              <div class="divide-y rounded-lg border" style="border-color: rgba(44, 32, 57, 0.1)">
                ...
                >
                  ...
                  >
                    <Checkbox
                      v-model="r.incluir_en_cumplimiento"
                      :binary="true"
                      @change="guardarResponsable(r)"
                    />
                    Relevante
            ...
            <div>
              <div class="mb-2 flex items-center justify-between gap-2">
                <p class="text-xs font-bold tracking-widest uppercase" style="color: #915bd8">
                  Asignar contratos ({{ respContratos.length }})
                </p>
                <div class="flex items-center gap-2">
                  <Select
                    v-model="respAsignarA"
                    :options="respOpcionesAsignar"
                    optionLabel="label"
                    optionValue="value"
                    placeholder="Asignar seleccionados a…"
                    class="text-sm"
...
const dcGesconResumen = computed(() => {
  ...
  if (
    !p ||
    !(p.gescon_codigo || p.gescon_fecha_inicio || p.gescon_precio || p.gescon_cantidades_kwh)
  )
    return []
  return [
    { label: 'Código GESCON', value: p.gescon_codigo || '—', mono: true },
    {
      label: 'Vigencia GESCON',
      value: `${p.gescon_fecha_inicio || '—'} → ${p.gescon_fecha_fin || '—'}`,
      mono: true,
    },
...
async function abrirDetalleContrato(c, modo) {
  ...
  try {
    ...
    if (ppaId) {
      try {
        ppa = (await client.get(`/ppa/${ppaId}`)).data
      } catch {
        ppa = null
      }
    }
    ...
    if (ci) {
      try {
        gescon = (await client.get('/asic', { params: { contrato_interno: ci } })).data
      } catch {
        gescon = []
      }
    }
    if (!gescon.length && c._proyecto_id) {
      try {
        gescon = (await client.get('/asic', { params: { proyecto_id: c._proyecto_id } })).data
      } catch {
        /* sin fallback */
      }
    }
    if (!gescon.length && modo !== 'ppa_compra_externa') {
      ...
      if (sic) {
        try {
          gescon = (await client.get('/asic', { params: { codigo_sic_contrato: sic } })).data
        } catch {
          /* sin fallback */
        }
      }
    ...
    gescon.sort(
      (a, b) =>
        (b.es_version_vigente === true) - (a.es_version_vigente === true) ||
    ...
    if (!ppa) {
      ...
      if (vig) {
        try {
          ppa = (await client.get(`/ppa/${vig.contrato_ppa_id}`)).data
        } catch {
          ppa = null
        }
      }
...
async function etFetch(y, m) {
  const res = await client.get('/cumplimiento/energia-transada', {
    params: { year: y, month: m, incluir_todos: incluirTodos() },
    timeout: 180000,
  })
...
function dualBarLeftX(i) {
  return PAD_L + i * slotW + (slotW - dualBarW * 2 - dualGap) / 2
}
function dualBarRightX(i) {
...
const simResults = computed(() => {
  ...
  for (const c of allContratos.value) {
    ...
    const dupPct = genDup > 0 && (max != null || min != null) ? (genDup / (max ?? min)) * 100 : null
    const proyPct =
...
async function loadAnnualData() {
  ...
  try {
    if (selectedContratoId.value === CONSOLIDADO_ID) {
      ...
      anualData.value = await cachedGet(`/cumplimiento/ppa/${selectedContratoId.value}/anual`, {
...
async function exportarAnualExcel() {
  ...
  try {
    ...
    const { construirContratoAnualAOA, sheetNameSafe, slugify } =
      await import('./cumplimientoAnualExport.js')
    ...
    if (consolidado) {
      ...
      const resultados = await Promise.allSettled(
        realContratos.map((c) => fetchContratoAnualCached(c.id, year)),
      )
      ...
      const built = construirContratoAnualAOA({
        contrato: anualData.value.contrato,
        year,
        meses: anualData.value.meses,
        consolidado: false,
      })
      addSheet(
        built,
        anualData.value.contrato.nombre_interno ||
          anualData.value.contrato.numero_codigo_contrato ||
          'Contrato',
      )
    }
...
function beAbrirCapa(categoria) {
...
async function exportarBalanceExcel() {
  ...
  const linea = (etiqueta, celda, signo = 1) =>
    aoa.push([
      etiqueta,
      '',
      '',
...
.be-linea td:first-child {
  border-left: 2px solid transparent;
  padding-left: 6px;
}
.be-linea.activa td:first-child {
  border-left-color: #915bd8;
}
...
.cv-btn-cta {
  ...
  font-weight: 700;
  cursor: pointer;
  box-shadow: 0 1px 0 rgba(44, 32, 57, 0.04);
  transition:
    filter 0.12s,
    box-shadow 0.12s;
}
...
.cv-card {
  background: #fff;
  border: 1px solid rgba(44, 32, 57, 0.07);
  border-radius: 16px;
  overflow: hidden;
  box-shadow:
    0 1px 2px rgba(44, 32, 57, 0.04),
    0 2px 10px rgba(44, 32, 57, 0.035);
  transition:
    border-color 0.15s,
    box-shadow 0.15s,
    transform 0.15s;
...
.cv-card-dragover {
  ...
  box-shadow: 0 0 0 2px rgba(145, 91, 216, 0.18) !important;
}
...
.cv-card-gold {
  ...
  overflow: hidden;
  box-shadow:
    0 1px 2px rgba(154, 103, 0, 0.05),
    0 2px 10px rgba(154, 103, 0, 0.04);
  transition:
    border-color 0.15s,
    box-shadow 0.15s,
    transform 0.15s;
...
.cv-card-gold:hover {
  ...
  box-shadow: 0 10px 28px rgba(154, 103, 0, 0.12);
  transform: translateY(-2px);
}
