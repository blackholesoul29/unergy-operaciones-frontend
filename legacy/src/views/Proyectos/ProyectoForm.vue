  ...
  <form @submit.prevent="submit" class="space-y-4 pt-2">
    <div class="grid grid-cols-2 gap-4">
      ...
      <div>
        ...
        <Select
          v-model="f.clasificacion_regulatoria"
      ...
      <div class="col-span-2">
        ...
        <InputText
          v-model="f.direccion_vereda"
          class="w-full"
          placeholder="Ej: Vereda El Cerrito, km 4 vía Planeta Rica"
        />
      </div>
    ...
    <div class="space-y-4 rounded-lg border border-gray-200 p-4">
      <p class="text-xs font-semibold tracking-wide text-gray-500 uppercase">
        Generación simulada mensual (kWh)
      </p>

      <!-- P90 -->
      <div>
        <p class="mb-2 text-xs font-medium text-gray-600">P90</p>
        <div class="grid grid-cols-6 gap-2">
          <div v-for="(mes, i) in MESES" :key="'p90-' + i">
            <label class="mb-0.5 block text-center text-[10px] text-gray-400">{{ mes }}</label>
            <InputNumber
              v-model="p90Array[i]"
              :maxFractionDigits="1"
              class="w-full"
      ...
      <!-- P50 -->
      <div>
        <p class="mb-2 text-xs font-medium text-gray-600">P50</p>
        <div class="grid grid-cols-6 gap-2">
    ...
    <div class="flex justify-end gap-2 pt-2">
      <Button
        ...
        @click="$emit('cancel')"
      />
      <Button type="submit" label="Crear proyecto" :loading="guardando" :disabled="!puedeGuardar" />
    </div>
    <p v-if="operadorRedObligatorio && !f.operador_red_id" class="text-right text-xs text-gray-500">
...
function submit() {
  ...
  if (capacidadInstaladaKwp.value !== null)
    infoTecnica.capacidad_instalada_kwp = capacidadInstaladaKwp.value
  if (cantidadTotalPaneles.value !== null)
    infoTecnica.cantidad_total_paneles = cantidadTotalPaneles.value
