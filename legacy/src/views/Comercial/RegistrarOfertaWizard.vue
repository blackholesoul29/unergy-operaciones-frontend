...
<template>
  ...
  >
    ...
    <!-- Pasos -->
    <ol class="mb-5 flex items-center gap-1 text-xs">
      <li v-for="(t, i) in PASOS" :key="t" class="flex items-center gap-1">
        ...
        >
          <span
            ...
            :style="{ background: i <= paso ? '#915BD8' : '#c4b8d4' }"
            >{{ i + 1 }}</span
          >
          {{ t }}
        </button>
        <i
          ...
          style="color: #c4b8d4; font-size: 10px"
        />
      </li>
    </ol>
      ...
      <template v-else>
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div>
            ...
            <Select
              v-model="nuevo.origen_tipo"
              :options="ORIGENES_CLIENTE"
              optionLabel="label"
              optionValue="value"
              showClear
              class="w-full"
    ...
    <div v-else-if="paso === 1" class="flex flex-col gap-3">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div class="sm:col-span-2">
            ...
            <MultiSelect
              ...
              :emptyMessage="cargandoCatalogos ? 'Cargando…' : 'No hay plantas cargadas'"
            >
            ...
            <p class="ayuda" :style="o.proyecto_ids?.length ? '' : 'color:#D64455'">
              <template v-if="o.proyecto_ids?.length">
                {{ o.proyecto_ids.length }} planta(s) vinculadas: la oferta va a traer su ubicación,
                operador de red y ficha técnica.
              </template>
              <template v-else>
          ...
          <div>
            ...
            <DatePicker
              v-model="o.fecha_tentativa_inicio"
              dateFormat="yy-mm-dd"
              showIcon
              class="w-full"
            />
      ...
      />
      <p class="ayuda">
        Una oferta por planta × servicio. Es la unidad del tablero: sin al menos una, el registro no
        aparecería en ninguna vista.
      </p>
    </div>
    ...
    <div v-else class="flex flex-col gap-3">
      <div class="rounded-lg p-3" style="background: #faf8fc; border: 1px solid #e8e0f0">
        <div class="mb-1 text-xs font-semibold" style="color: #7a6e8a">CLIENTE</div>
        <div class="text-sm font-medium" style="color: #2c2039">{{ resumenCliente }}</div>
        <div v-if="modo === 'nuevo'" class="mt-1 text-xs" style="color: #9b89b5">
          Se crea nuevo, con {{ contactosValidos.length }} contacto(s).
        </div>
      </div>

      <div class="rounded-lg p-3" style="background: #faf8fc; border: 1px solid #e8e0f0">
        <div class="mb-2 text-xs font-semibold" style="color: #7a6e8a">
...
async function cargarCatalogos() {
  ...
  if (cl.status === 'fulfilled') clientes.value = cl.value
    ...
    toast.add({ severity: 'warn', summary: 'No se pudo cargar la lista de clientes', life: 4000 })
