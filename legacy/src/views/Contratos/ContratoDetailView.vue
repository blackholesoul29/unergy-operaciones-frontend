<template>
  <div v-if="contrato">
    <DetalleLayout
      :volver="{ to: '/servicios-unificado?vista=servicios&srv=ppa', label: 'Servicios' }"
    ...
    >
      <template #chips>
        ...
        <Tag
          :value="contrato.tipo_contrato === 'compra' ? 'Compra' : 'Venta'"
          :style="
            contrato.tipo_contrato === 'compra'
      ...
      <template #acciones>
        ...
        <a
          ...
          class="cd-head-link"
          v-tooltip.bottom="'Abrir el contrato en Drive'"
        >
      ...
      <template #default="{ tab }">
        ...
        <div v-if="tab === 'datos'" class="space-y-4">
          ...
          <div class="grid grid-cols-2 gap-3 lg:grid-cols-4">
            ...
            >
              ...
              <p class="cd-stat-val" :style="`color:${estadoVigencia.color}`">
                {{ estadoVigencia.label }}
              </p>
              <p class="cd-stat-sub" :style="`color:${estadoVigencia.color};opacity:.7`">
                {{ estadoVigencia.detalle }}
              </p>
            </div>
            ...
            <div class="cd-stat">
              <p class="cd-stat-lbl">
                <i class="pi pi-chart-line" style="font-size: 9px" />Indexación
              </p>
              <p class="cd-stat-val">{{ contrato.indice_indexacion || '—' }}</p>
              <p class="cd-stat-sub">
                {{ contrato.periodicidad_indexacion || 'sin periodicidad'
                }}<template v-if="contrato.periodo_indexacion_base">
                  · base {{ contrato.periodo_indexacion_base }}</template
                >
              </p>
            </div>
            ...
            <div class="cd-stat">
              ...
              <p class="cd-stat-sub">
                {{
                  contrato.tiempo_pago != null
                    ? 'pago a ' + contrato.tiempo_pago + ' días'
                    : 'sin plazo de pago'
                }}
              </p>
            </div>
            ...
            <div class="cd-sec-body">
              ...
              <div v-if="!editandoId" class="cd-grid">
                ...
                <div class="flex flex-col gap-0.5">
                  ...
                  <div>
                    <Tag
                      v-if="contrato.es_comunidad_energetica"
                      severity="success"
                      value="🏘 Sí"
                      class="text-[10px]"
                    />
                    <span v-else class="text-sm" style="color: #2c2039">{{
                      contrato.es_comunidad_energetica === false ? 'No' : '—'
                    }}</span>
                  </div>
                </div>
              </div>
              ...
              <div v-else class="cd-grid">
                <div class="flex flex-col gap-1">
                  ...
                  <InputText
                    v-model="formId.nombre_interno"
                    placeholder="Ej: Terpel 1"
                    class="w-full"
                  />
                </div>
                <div class="flex flex-col gap-1">
                  ...
                  <InputText
                    v-model="formId.numero_codigo_contrato"
                    placeholder="Ej: UNERGY 001-2023"
                    class="w-full"
                  />
                </div>
              </div>
              ...
              <div class="cd-sec-act">
                ...
                <template v-else>
                  <Button
                    ...
                    text
                    severity="secondary"
                    @click="cancelarEdicionPartes"
                  />
                  <Button
                    label="Guardar"
                    icon="pi pi-check"
                    size="small"
                    :loading="guardandoPartes"
                    @click="guardarPartes"
                  />
                </template>
              </div>
              ...
              <div v-if="!editandoPartes" class="cd-partes">
                ...
                <div class="cd-parte">
                  <p class="cd-parte-rol">
                    <i class="pi pi-building" style="font-size: 9px" />Comprador
                  </p>
            ...
            <div class="cd-sec-body">
              <div class="cd-grid">
                ...
                <div class="flex flex-col gap-0.5">
                  ...
                  <div>
                    <Tag
                      v-if="contrato.renovacion_automatica != null"
                      :severity="contrato.renovacion_automatica ? 'success' : 'secondary'"
                      :value="contrato.renovacion_automatica ? 'Sí' : 'No'"
                      class="text-[10px]"
                    />
          ...
          <section class="cd-sec">
            ...
            <div class="cd-sec-body">
              <div class="cd-grid">
                ...
                <InfoField
                  label="Periodicidad indexación"
                  :value="contrato.periodicidad_indexacion"
                />
                <InfoField
                  label="Período base indexación"
                  :value="contrato.periodo_indexacion_base"
                />
                <InfoField
                  label="Valor indexación base"
                  :value="
                    contrato.valor_indexacion_base != null
                      ? String(contrato.valor_indexacion_base)
                      : null
                  "
                />
                <InfoField label="Tarifa base ($/kWh)" :value="tarifaBaseFmt" />
                <InfoField
                  label="Periodicidad facturación"
                  :value="contrato.periodicidad_facturacion"
                />
                <InfoField
                  label="Tiempo de pago (días)"
                ...
                <div v-if="contrato.condiciones_pago" class="cd-ancho flex flex-col gap-0.5">
                  ...
                  <span class="text-sm whitespace-pre-line" style="color: #2c2039">{{
          ...
          <section class="cd-sec">
            <header class="cd-sec-head">
              <span class="cd-ico" style="background: #6366f118"
                ><i class="pi pi-book" style="color: #6366f1"
              /></span>
              ...
              <div class="cd-sec-act">
                ...
                <template v-else>
                  <Button
                    ...
                    text
                    severity="secondary"
                    @click="editandoGescon = false"
                  />
                  <Button
                    label="Guardar"
                    size="small"
                    icon="pi pi-check"
                    :loading="guardandoGescon"
                    @click="guardarGescon"
                  />
                </template>
              </div>
            ...
            <div class="cd-sec-body">
              ...
              <div v-if="!editandoGescon" class="cd-grid">
                ...
                <InfoField
                  label="GESCON inicio"
                  :value="formatFecha(contrato.gescon_fecha_inicio)"
                />
              ...
              <div v-else class="cd-grid">
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <InputNumber
                    v-model="formGescon.gescon_precio"
                    :maxFractionDigits="4"
                    class="w-full"
                  />
                </div>
                <div class="flex flex-col gap-1">
                  ...
                  <DatePicker
                    v-model="formGescon.gescon_fecha_inicio"
                    dateFormat="yy-mm-dd"
                    showIcon
                    class="w-full"
                  />
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <DatePicker
                    v-model="formGescon.gescon_fecha_fin"
                    dateFormat="yy-mm-dd"
                    showIcon
                    class="w-full"
                  />
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <InputNumber
                    v-model="formGescon.gescon_cantidades_kwh"
                    :maxFractionDigits="3"
                    locale="en-US"
                    class="w-full"
                  />
          ...
          <section class="cd-sec">
            <header class="cd-sec-head">
              <span class="cd-ico" style="background: #d9770618"
                ><i class="pi pi-link" style="color: #d97706"
              /></span>
              ...
              <div class="cd-sec-act">
                ...
                <template v-else>
                  <Button
                    ...
                    text
                    severity="secondary"
                    @click="editandoEnlace = false"
                  />
                  <Button
                    label="Guardar"
                    icon="pi pi-check"
                    size="small"
                    :loading="guardandoEnlace"
                    @click="guardarEnlace"
                  />
                </template>
              </div>
            ...
            <div class="cd-sec-body">
              ...
              <div
                v-if="!editandoEnlace"
                class="cd-link"
                :class="{ 'cd-link--vacio': !enlaceContrato }"
              >
                <span class="cd-ico" style="background: #fef3c7">
                  <i class="pi pi-file-pdf" style="color: #d97706" />
                </span>
                <div class="min-w-0 flex-1">
                  <p class="mb-0.5 text-xs font-medium" style="color: #92400e">Contrato en Drive</p>
                  <a
                    v-if="enlaceContrato"
                    :href="enlaceContrato"
                    target="_blank"
              ...
              <!-- Modo edición -->
              <div v-else class="flex max-w-xl flex-col gap-1">
                <label class="cd-lbl">Enlace al contrato (Drive, Dropbox, SharePoint…)</label>
                <InputText
                  v-model.trim="formEnlace.carpeta_link"
                  class="w-full"
                  placeholder="https://drive.google.com/…"
                  @keyup.enter="guardarEnlace"
                />
                <small class="text-[11px]" style="color: #9b89b5">
                  Debe empezar por <span class="font-mono">http://</span> o
                  <span class="font-mono">https://</span>. Déjalo vacío para quitar el enlace.
                </small>
              </div>
            </div>
          </section>
          ...
          <section v-if="tieneDetallesOperacionales" class="cd-sec">
            <header class="cd-sec-head">
              <span class="cd-ico" style="background: #64748b18"
                ><i class="pi pi-list" style="color: #64748b"
              /></span>
            ...
            <div class="cd-sec-body space-y-3">
              ...
              <div v-if="contrato.specific_service_terms" class="flex flex-col gap-0.5">
                ...
                <span class="text-sm whitespace-pre-line" style="color: #2c2039">{{
                  contrato.specific_service_terms
                }}</span>
              </div>
        ...
        <div v-if="tab === 'cantidades'" class="space-y-4">
          <!-- Resumen -->
          <div
            v-if="resumenCantidades && !editandoCantidades"
          ...
          >
            <div class="cd-stat">
              ...
              <p class="cd-stat-sub">
                {{ resumenCantidades.añoMin }} – {{ resumenCantidades.añoMax }}
              </p>
            </div>
            <div class="cd-stat">
              <p class="cd-stat-lbl">
                <i class="pi pi-bolt" style="font-size: 9px" />Compromiso {{ hoyPeriodo.año }}
              </p>
              <p class="cd-stat-val">{{ fmtNum(resumenCantidades.totalAñoActual) }} MWh</p>
              <p class="cd-stat-sub">
                {{
                  resumenCantidades.tieneAñoActual
                    ? 'suma de mínimos del año'
                    : 'sin compromisos este año'
                }}
              </p>
            </div>
            <div class="cd-stat">
              <p class="cd-stat-lbl">
                <i class="pi pi-clock" style="font-size: 9px" />Mes en curso
              </p>
              <p class="cd-stat-val">
                {{
                  resumenCantidades.actual
                    ? fmtNum(resumenCantidades.actual.energia_minima) + ' MWh'
                    : '—'
                }}
              </p>
              <p class="cd-stat-sub">
            ...
            <div class="cd-stat">
              <p class="cd-stat-lbl">
                <i class="pi pi-arrows-h" style="font-size: 9px" />Flexibilidad
              </p>
              <p class="cd-stat-val">
          ...
          <div class="cd-toolbar">
            <SelectButton
              v-if="!editandoCantidades"
              v-model="vistaCantidades"
              :options="VISTAS"
              optionLabel="label"
              optionValue="value"
              size="small"
            ...
            <div class="cd-toolbar-act">
              ...
              <template v-else>
                ...
                <Button
                  ...
                  icon="pi pi-check"
                  size="small"
                  :loading="guardandoCantidades"
                  :disabled="!energiaRows.length"
                  @click="guardarCantidades"
                />
              </template>
            </div>
          </div>
          ...
          <template v-if="editandoCantidades">
            <div class="cd-aviso">
              ...
              <div>
                Copia las columnas <strong>Año · Mes · Mín · Máx · Plantas contrato</strong> desde
                Excel y pégalas abajo (Mín/Máx en MWh/mes; <strong>Plantas contrato</strong> = nº de
                plantas que el contrato exige ese mes). Máx y Plantas contrato son opcionales. Al
                guardar se <strong>reemplazan todos</strong> los compromisos actuales.
              </div>
            </div>
            <Textarea
              ...
              <table class="cd-preview">
                <thead>
                  <tr>
                    <th>Año</th>
                    <th>Mes</th>
                    <th class="cd-der">Mín (MWh)</th>
                    <th class="cd-der">Máx (MWh)</th>
                ...
                <tbody>
                  ...
                  <tr v-if="energiaRows.length > 8">
                    <td colspan="5" class="cd-preview-mas">
                      … y {{ energiaRows.length - 8 }} filas más
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div v-else class="cd-tabla">
              ...
              >
                <Column field="año" header="Año" style="width: 80px">
                  <template #body="{ data }"
                    ><span class="cd-num">{{ data.año }}</span></template
                  >
                </Column>
                ...
                <Column
                  :header="vistaCantidades === 'anual' ? 'Mín (MWh/año)' : 'Mín (MWh/mes)'"
                  headerClass="cd-th-der"
                  bodyClass="cd-der"
                >
                  <template #body="{ data }">
                ...
                <Column
                  :header="vistaCantidades === 'anual' ? 'Máx (MWh/año)' : 'Máx (MWh/mes)'"
                  headerClass="cd-th-der"
                  bodyClass="cd-der"
                >
                  <template #body="{ data }"
                    ><span class="cd-num">{{ fmtNum(data.energia_maxima) }}</span></template
                  >
                </Column>
                <Column style="width: 160px" headerClass="cd-th-der" bodyClass="cd-der">
                  <template #header>
                    <span
                      v-tooltip.top="
                        'Plantas registradas y despachando energía al contrato. La calcula la plataforma vía GESCON.'
                      "
                    >
                      {{
                        vistaCantidades === 'anual'
                          ? 'Plantas inscritas (máx)'
                          : 'Plantas inscritas'
                      }}
                    </span>
                  </template>
                  <template #body="{ data }">
                    <span
                      v-if="data.plantas_inscritas != null"
                      class="cd-pill"
                      :class="plantasClase(data)"
                      >{{ data.plantas_inscritas }}</span
                    >
                ...
                <Column
                  :header="
                    vistaCantidades === 'anual' ? 'Plantas contrato (máx)' : 'Plantas contrato'
                  "
                  style="width: 150px"
                  headerClass="cd-th-der"
                  bodyClass="cd-der"
                ...
                <Column header="Rango" style="width: 110px">
                  <template #body="{ data }">
                    <span
                      v-if="data.energia_minima > 0 && data.energia_maxima != null"
                      class="cd-pill cd-pill--flex"
                    >
        ...
        <div v-if="tab === 'tarifas'" class="space-y-4">
          <!-- Resumen -->
          <div
            v-if="resumenTarifas && !editandoTarifas"
            class="grid grid-cols-2 gap-3 lg:grid-cols-4"
          >
            <div class="cd-stat" style="background: #fffbeb; border-color: #fde68a">
              <p class="cd-stat-lbl" style="color: #b45309">
                <i class="pi pi-dollar" style="font-size: 9px" />
                {{ resumenTarifas.esDelMes ? 'Tarifa del mes' : 'Última tarifa' }}
              </p>
              <p class="cd-stat-val" style="color: #b45309">
                {{ fmtCOP(resumenTarifas.vigente.tarifa) }}
              </p>
              <p class="cd-stat-sub" style="color: #b45309; opacity: 0.75">
                {{ MESES[resumenTarifas.vigente.mes - 1] }} {{ resumenTarifas.vigente.año }} ·
                COP/kWh
              </p>
            </div>
            <div class="cd-stat">
              <p class="cd-stat-lbl">
                <i class="pi pi-chart-line" style="font-size: 9px" />Variación
              </p>
              <p class="cd-stat-val" :style="`color:${varColor(resumenTarifas.varPct)}`">
                {{
                  resumenTarifas.varPct != null
                    ? (resumenTarifas.varPct > 0 ? '+' : '') +
                      resumenTarifas.varPct.toFixed(1) +
                      '%'
                    : '—'
                }}
              </p>
            ...
            <div class="cd-stat">
              <p class="cd-stat-lbl">
                <i class="pi pi-arrows-v" style="font-size: 9px" />Rango histórico
              </p>
              <p class="cd-stat-val">
                {{ fmtCOP(resumenTarifas.min) }} – {{ fmtCOP(resumenTarifas.max) }}
              </p>
          ...
          <div class="cd-toolbar">
            <SelectButton
              v-if="!editandoTarifas"
              v-model="vistaTarifas"
              :options="VISTAS"
              optionLabel="label"
              optionValue="value"
              size="small"
            ...
            <div class="cd-toolbar-act">
              ...
              <template v-else>
                ...
                <Button
                  ...
                  icon="pi pi-check"
                  size="small"
                  :loading="guardandoTarifas"
                  :disabled="!tarifasRows.length"
              ...
              <table class="cd-preview">
                ...
                <tbody>
                  ...
                  <tr v-if="tarifasRows.length > 8">
                    <td colspan="3" class="cd-preview-mas">
                      … y {{ tarifasRows.length - 8 }} filas más
                    </td>
                  </tr>
                </tbody>
              </table>
              ...
              >
                ...
                <Column header="Tarifa (COP/kWh)" headerClass="cd-th-der" bodyClass="cd-der">
                  <template #body="{ data }">
                    ...
                    <span v-if="vistaTarifas === 'anual' && !data._uniforme" class="cd-nulo ml-1"
                      >prom.</span
                    >
                  </template>
                </Column>
                ...
                >
                  <template #body="{ data, index }">
                    <span
                      v-if="
                        index > 0 &&
                        currentTarifas[index - 1]?.tarifa != null &&
                        data.tarifa != null
                      "
                      class="cd-pill"
                      :style="`color:${varColor(varPct(currentTarifas[index - 1].tarifa, data.tarifa))};background:${varBg(varPct(currentTarifas[index - 1].tarifa, data.tarifa))}`"
                    >
        ...
        <div v-if="tab === 'asic'" class="space-y-4">
          <!-- Resumen -->
          <div v-if="!loadingAsic && asicRows.length" class="grid grid-cols-2 gap-3 lg:grid-cols-4">
            <div class="cd-stat">
            ...
            <div class="cd-stat" style="background: #ecfdf5; border-color: #a7f3d0">
              <p class="cd-stat-lbl" style="color: #059669">
                <i class="pi pi-check-circle" style="font-size: 9px" />Vigentes
              </p>
              <p class="cd-stat-val" style="color: #059669">{{ resumenAsic.vigentes }}</p>
              <p class="cd-stat-sub" style="color: #059669; opacity: 0.75">
                con fecha fin en el futuro
              </p>
            </div>
            <div class="cd-stat">
              <p class="cd-stat-lbl">
                <i class="pi pi-verified" style="font-size: 9px" />Publicados
              </p>
            ...
            <div
              class="cd-stat"
              :style="resumenAsic.enProceso ? 'background:#fffbeb;border-color:#fde68a' : ''"
            >
              <p class="cd-stat-lbl" :style="resumenAsic.enProceso ? 'color:#b45309' : ''">
                <i class="pi pi-hourglass" style="font-size: 9px" />En proceso
              </p>
              <p class="cd-stat-val" :style="resumenAsic.enProceso ? 'color:#b45309' : ''">
                {{ resumenAsic.enProceso }}
              </p>
              <p
                class="cd-stat-sub"
                :style="resumenAsic.enProceso ? 'color:#b45309;opacity:.75' : ''"
              >
          ...
          <div class="cd-toolbar">
            ...
            />
            <div class="cd-toolbar-act">
              <span class="cd-toolbar-nota"
                >{{ asicFiltrados.length }} de {{ asicRows.length }} registros</span
              >
            </div>
          </div>
          ...
          <div v-else-if="!asicFiltrados.length" class="cd-vacio">
            <i class="pi pi-book" />
            <p class="cd-vacio-tit">
              Sin registros ASIC {{ vistaAsic === 'vigentes' ? 'vigentes' : '' }}
            </p>
          ...
          <div v-else class="cd-tabla">
            ...
            >
                    ...
                    :severity="
                      {
                        ...
                        rechazado: 'danger',
                        desistido: 'secondary',
                      }[data.estado_solicitud] || 'secondary'
                    "
                  />
                </template>
              </Column>
              <Column field="fecha_inicio" header="Inicio" sortable style="width: 105px">
                <template #body="{ data }"
                  ><span class="cd-num">{{ data.fecha_inicio || '—' }}</span></template
                >
              </Column>
              <Column field="fecha_fin" header="Fin" sortable style="width: 105px">
                <template #body="{ data }"
                  ><span class="cd-num">{{ data.fecha_fin || '—' }}</span></template
                >
              </Column>
              <Column
                field="porcentaje_despacho"
                header="% Despacho"
                style="width: 115px"
                headerClass="cd-th-der"
                bodyClass="cd-der"
              >
                <template #body="{ data }">
                  <span
                    v-if="data.porcentaje_despacho != null"
                    class="cd-pill"
              ...
              <Column field="fecha_solicitud" header="F. solicitud" sortable style="width: 120px">
                <template #body="{ data }"
                  ><span class="cd-num">{{ data.fecha_solicitud || '—' }}</span></template
                >
              </Column>
        ...
        <div v-if="tab === 'proyectos'" class="space-y-4">
          ...
          <div
            v-if="contrato.proyectos?.length"
          ...
          >
            ...
            >
              <span class="cd-ico" style="background: #915bd818"
                ><i class="pi pi-bolt" style="color: #915bd8"
              /></span>
              <div class="min-w-0 flex-1">
          ...
          <div v-else class="cd-vacio">
            ...
            <p class="cd-vacio-sub">
              Usa <strong>Asociar proyecto</strong> para vincular las que despachan a este PPA.
            </p>
          </div>
        </div>
      </template>
    ...
    <Dialog
      v-model:visible="showAsociar"
      header="Asociar proyecto"
      modal
      :style="{ width: '420px' }"
    >
...
async function guardarId() {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Guardado',
      detail: 'Identificación actualizada',
      life: 2500,
    })
  } catch (e) {
    toast.add({
...
async function guardarPartes() {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Guardado',
      detail: 'Partes del contrato actualizadas',
      life: 3000,
    })
  } catch (e) {
    toast.add({
...
async function guardarEnlace() {
  ...
  if (url && !/^https?:\/\//i.test(url)) {
    toast.add({
      ...
      detail: 'Debe empezar por http:// o https://',
      life: 3500,
    })
    return
  }
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Guardado',
      detail: url ? 'Enlace del contrato actualizado' : 'Enlace eliminado',
      life: 2500,
    })
  } catch (e) {
    toast.add({
...
function parseTarifas() {
  tarifasError.value = ''
  const lines = tarifasPaste.value
    .split('\n')
    .map((l) => l.trim())
    .filter(Boolean)
  const rows = []
  for (const [i, line] of lines.entries()) {
    ...
    if (cols.length < 3) {
      tarifasError.value = `Fila ${i + 1}: se esperan 3 columnas`
      tarifasRows.value = []
      return
    }
    ...
    if (isNaN(año) || !mes || isNaN(tarifa)) {
      tarifasError.value = `Fila ${i + 1}: datos inválidos`
      tarifasRows.value = []
      return
    }
...
function parseEnergia() {
  energiaError.value = ''
  const lines = energiaPaste.value
    .split('\n')
    .map((l) => l.trim())
    .filter(Boolean)
  const rows = []
  for (const [i, line] of lines.entries()) {
    const cols = splitRow(line)
    if (cols.length < 3) {
      energiaError.value = `Fila ${i + 1}: se esperan al menos 3 columnas (Año · Mes · Mín)`
      energiaRows.value = []
      return
    }
    const año = parseInt(cols[0].trim(), 10)
    ...
    if (isNaN(año) || !mes || isNaN(min)) {
      energiaError.value = `Fila ${i + 1}: datos inválidos`
      energiaRows.value = []
      return
    }
    rows.push({
...
function onPasteTarifas() {
  setTimeout(parseTarifas, 50)
}
function onPasteEnergia() {
...
async function guardarTarifas() {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Guardado',
      detail: `${data.length} tarifas actualizadas`,
      life: 2500,
    })
  } catch (e) {
    toast.add({
...
async function guardarCantidades() {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Guardado',
      detail: `${data.length} compromisos actualizados`,
      life: 2500,
    })
  } catch (e) {
    toast.add({
...
const resumenCantidades = computed(() => {
  ...
  return {
    ...
    flex: conRango.length
      ? (conRango.reduce((acc, r) => acc + (r.energia_maxima / r.energia_minima - 1), 0) /
          conRango.length) *
        100
      : null,
  }
...
const cantidadesMensuales = computed(() => {
  ...
  return [...contrato.value.compromisos_energia]
    ...
    .map((r) => ({
      ...r,
      plantas_inscritas: plantasInscritasMap.value[`${r.año}-${r.mes}`] ?? null,
    }))
})

const cantidadesAnuales = computed(() => {
  if (!contrato.value?.compromisos_energia) return []
  const base = agregarPorAño(
    cantidadesMensuales.value,
...
async function asociarProyecto() {
  ...
  try {
    await api.post(`/ppa/${contrato.value.id}/proyectos`, {
      proyecto_id: proyectoSeleccionado.value.id,
    })
    contrato.value.proyectos = [...(contrato.value.proyectos ?? []), proyectoSeleccionado.value]
    showAsociar.value = false
    toast.add({
      severity: 'success',
      summary: 'Proyecto asociado',
      detail: proyectoSeleccionado.value.nombre_comercial,
      life: 2500,
    })
  } catch (e) {
    toast.add({
...
async function cargarPlantasInscritas() {
  ...
  try {
    const { data } = await api.get(
      `/cumplimiento/ppa/${contrato.value.id}/plantas-inscritas-por-mes`,
    )
    const map = {}
...
async function cargarAsic(c) {
  ...
  try {
    ...
    toast.add({
      severity: 'warn',
      summary: 'ASIC',
      detail: 'No se pudieron cargar registros ASIC',
      life: 3000,
    })
  } finally {
...
.cd-tabla :deep(.p-datatable-tbody > tr:last-child > td) {
  border-bottom: none;
}
.cd-tabla :deep(.p-datatable-tbody > tr.p-row-odd) {
  background: #fdfcfe;
}
.cd-tabla :deep(.p-datatable-tbody > tr:hover) {
  background: #f7f3fd;
}
...
.cd-tabla :deep(.cd-th-der .p-datatable-column-header-content) {
  justify-content: flex-end;
}
.cd-tabla :deep(td.cd-der),
.cd-der {
  text-align: right;
}
