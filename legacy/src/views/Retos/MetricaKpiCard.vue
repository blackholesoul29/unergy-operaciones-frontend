<template>
  ...
  <div class="rq-card rq-kpi" @click="emit('foco', metrica)">
    ...
    <div class="rq-kpi-head">
      <span class="rq-kpi-nombre" v-tooltip.top="metrica.descripcion || ''">{{
        metrica.nombre
      }}</span>
      <span v-if="!metrica.activa" class="rq-chip rq-chip-neutro">Inactiva</span>
      <span class="rq-chip" :style="estadoBadge(metrica.estado)">{{
        estadoLabel(metrica.estado)
      }}</span>
      <Button
...
const items = computed(() => [
  ...
  { separator: true },
  {
    ...
    command: () => emit('eliminar', props.metrica),
  },
])
...
.rq-kpi:focus-within .rq-kpi-mas {
  opacity: 1;
}
