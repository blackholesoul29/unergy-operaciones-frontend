  ...
  <div class="pc-wrap">
    ...
    <div class="pc-top">
      <div>
        ...
        <div class="sub">
          <span class="periodo-chip">{{
            periodo ? periodoLabel : 'Sin período seleccionado'
          }}</span>
      ...
      <div class="top-actions">
        <div class="fld">
          ...
          <div class="pc-period">
            ...
            <button
              ...
              @click="stepMes(1)"
              v-tooltip.bottom="'Mes siguiente'"
            >
        ...
        <button
          v-if="tab === 'preliquidacion' || tab === 'oficial'"
          class="btn-o"
        ...
        <button
          v-if="tab !== 'diferencia' && tab !== 'clasificacion'"
          class="btn-o"
          :disabled="loading || contrastando"
          @click="verContraste"
        ...
        <button
          ...
          @click="armarPeriodo"
          v-tooltip.bottom="
            'Arma los paneles del período desde la API. NEU y Nitro siguen con su Excel.'
          "
        >
    ...
    <div v-if="contraste" class="pc-aviso">
      ...
      <div
        v-for="(p, i) in contraste.proyectos.filter((x) => x.diferencias && x.diferencias.length)"
        :key="i"
        class="pc-contraste-proy"
      >
      ...
      <div
        v-if="!contraste.proyectos.some((x) => x.diferencias && x.diferencias.length)"
    ...
    <Dialog
      v-model:visible="showPeriodoDialog"
      modal
      :draggable="false"
      :closable="true"
      class="pc-dialog"
    ...
    >
      ...
      <div class="fld" style="margin-top: 14px">
        ...
        <div class="dlg-aviso">
          ...
          <b>{{ dlgTipoCarga.toUpperCase() }}</b> para este período. Los demás se reportarán como
          rechazados.
        </div>
      </div>
      <template #footer>
        ...
        <button
          class="btn"
          :disabled="!dlgMes || !dlgAnio || !dlgTipo || !dlgTipoCarga"
    ...
    <!-- Tabs -->
    <div class="tabs">
      <div class="tab" :class="{ act: tab === 'preliquidacion' }" @click="setTab('preliquidacion')">
        Preliquidación
      </div>
      <div class="tab" :class="{ act: tab === 'oficial' }" @click="setTab('oficial')">Oficial</div>
      <div class="tab" :class="{ act: tab === 'seleccion' }" @click="setTab('seleccion')">
        Selección
      </div>
      <div class="tab" :class="{ act: tab === 'diferencia' }" @click="setTab('diferencia')">
        Diferencia
      </div>
      <div class="tab" :class="{ act: tab === 'clasificacion' }" @click="setTab('clasificacion')">
        Clasificación
      </div>
    </div>
    ...
    <div v-if="rechazados.length" class="banner banner-rechazo">
      <div class="rechazo-h">
        ...
        <b>{{ rechazados.length }} ER no se cargaron</b> — clasificación distinta a la sección
        elegida
      </div>
      ...
      <template v-else>
        ...
        <div v-if="tab === 'seleccion'" class="card">
            ...
            <div class="sel-tipo">
              ...
              <label class="vt-opt" :class="{ on: selTipo === 'preliquidacion' }">
                <input
                  ...
                  v-model="selTipo"
                  @change="cargarPaneles"
                />
                Preliquidación
              </label>
              <label class="vt-opt" :class="{ on: selTipo === 'oficial' }">
                <input type="radio" value="oficial" v-model="selTipo" @change="cargarPaneles" />
                Oficial
              </label>
            </div>
          </div>
          <div class="card-h" style="border-top: 1px solid var(--line); padding-top: 10px">
            <span class="hint" style="padding: 0"
              >Marca qué liquidar; el detalle contable está en las pestañas Preliquidación /
              Oficial.</span
            >
            <div class="pool-actions">
              <button class="mini" @click="selAll('liquidar_ingresos', true)">
                Liq. ingresos todos
              </button>
              <button class="mini" @click="selAll('liquidar_costos', true)">
                Liq. costos todos
              </button>
            ...
            <table class="ptbl">
              ...
              <tbody>
                <tr v-for="p in panelesFiltrados" :key="p.id">
                  ...
                  <td>
                    <span class="pmeta">{{
                      p.inversionistas.map((i) => i.nombre).join(', ')
                    }}</span>
                  </td>
          ...
          <div v-if="tipoDatos === 'oficial'" class="cons-pool">
            <div class="fld">
              ...
              >
                ...
                <button class="mini" @click="usarSiguiente('ing')">
                  usar {{ consInfo.ingresos.siguiente }}
                </button>
              </span>
            </div>
            <div class="fld">
              ...
              >
                ...
                <button class="mini" @click="usarSiguiente('cos')">
                  usar {{ consInfo.costos.siguiente }}
                </button>
              </span>
            </div>
            <div class="hint">
              ...
              <template v-if="consInfo">
                Siguiente libre: Ing <b>{{ consInfo.ingresos.siguiente }}</b> · Cost
                <b>{{ consInfo.costos.siguiente }}</b
                >.</template
              >
            </div>
            <div class="summary">
              <b>{{ nLiqIng }}</b> liq. ingresos · <b>{{ nLiqCost }}</b> liq. costos ·
              <b>{{ nGeneran }}</b> generan mandatos
            </div>
          </div>
          <div v-else class="hint" style="padding: 8px 12px">
            La preliquidación no lleva consecutivos — se asignan en el panel <b>oficial</b> (el
        ...
        <div v-if="tab === 'seleccion' && paneles.length" class="card" style="margin-top: 14px">
          <div class="card-h">
            ...
            <span class="hint" style="padding: 0"
              >Corrige a qué celda del ER apunta un concepto de Ingresos / Comercialización.</span
            >
          </div>
              ...
              <table class="dt">
                <thead>
                  <tr>
                    ...
                    <th class="l">Celda (hoja!celda)</th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  <tr v-if="!lineasMapeables(p).length">
                    <td class="l muted" colspan="2">Sin conceptos mapeables.</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <template v-if="tab === 'preliquidacion' || tab === 'oficial'">
          <div class="sec-title-row">
            ...
            <span class="hint" style="padding: 0"
              >Muestra el 100% del proyecto; expande cada uno para el desglose por
              inversionista.</span
            >
          </div>
          <div v-if="!panelesFiltrados.length" class="empty sm">
            Ningún proyecto coincide con los filtros.
          </div>
          <div
            v-for="p in panelesFiltrados"
            :key="'d' + p.id"
            class="proj"
            :class="{ off: !esActivo(p), open: open[p.id] }"
          >
            <div class="phead" @click="toggle(p.id)">
              ...
              <span v-if="p.liquidar_ingresos && !p.liquidar_costos" class="pill pill-bolsa"
                >solo ingresos</span
              >
              <span v-if="!p.liquidar_ingresos && p.liquidar_costos" class="pill pill-bolsa"
                >solo costos</span
              ...
              <span
                class="pill"
                :class="p.origen === 'api' ? 'pill-api' : 'pill-er'"
                v-tooltip.top="
                  p.origen === 'api'
              ...
              <div class="pcons">
                <span
                  >Ing: <b>{{ tab === 'oficial' ? (p.consecutivo_ingresos ?? '—') : '—' }}</b></span
                >
                <span
                  >Cost: <b>{{ tab === 'oficial' ? (p.consecutivo_costos ?? '—') : '—' }}</b></span
                >
                <button
                  class="btn-er"
                  @click.stop="descargarEr(p)"
            ...
            <div class="body" v-show="open[p.id]">
                ...
                <table class="dt flat">
                  ...
                  <tbody>
                    <template v-for="blk in bloquesMostrados()" :key="blk.key">
                      <tr class="blk-h">
                        <td colspan="5">{{ blk.label }}</td>
                      </tr>
                      <tr
                        v-for="(ln, i) in lineas100Bloque(p, blk)"
                        :key="blk.key + i"
                        :class="{ derivada: ln.derivada }"
                      >
                        <td class="l">
                          {{ ln.concepto }}<span v-if="ln.derivada" class="imp-tag">impuesto</span>
                        </td>
                        <td class="l">
                          <span
                            v-if="ln.fuente"
                            class="fuente-tag"
                            :title="fuenteTitle(ln.fuente)"
                            >{{ fuenteLabel(ln.fuente) }}</span
                          >
                          <span v-else-if="!ln.derivada" class="muted" style="font-size: 11px"
                            >ER</span
                          >
                        </td>
                      ...
                      <tr class="blk-tot">
                        ...
                        <td :class="{ neg: total100Bloque(p, blk) < 0 }">
                          {{ fmt(total100Bloque(p, blk)) }}
                        </td>
                        <td colspan="2"></td>
                      </tr>
                    </template>
              ...
              <button class="expand-inv" @click="toggleInv(p.id)">
                ...
                {{ invOpen[p.id] ? 'Ocultar' : 'Ver' }} desglose por inversionista ({{
                  p.inversionistas.length
                }})
              </button>
                  ...
                  <div class="inv-head">
                    ...
                    <div class="inv-cons">
                      <span
                        >Ing:
                        <b>{{
                          tab === 'oficial' && p.liquidar_ingresos
                  ...
                  <div class="secs">
                    <template v-for="sec in secciones" :key="sec.key">
                      <div
                        ...
                        class="sec-acc"
                        :class="{ open: secOpen[secKey(p.id, invKeyOf(inv), sec.key)] }"
                      >
                        <div class="sec-bar" @click="toggleSec(p.id, invKeyOf(inv), sec.key)">
                          <span class="chev">▶</span>
                          <span class="sec-lbl">{{ sec.label }}</span>
                          <span class="sec-tot" :class="{ neg: totalSec(inv, sec.key) < 0 }">{{
                            fmt(totalSec(inv, sec.key))
                          }}</span>
                        </div>
                        <div
                          class="sec-body"
                          v-show="secOpen[secKey(p.id, invKeyOf(inv), sec.key)]"
                        >
                          <div class="tbl-wrap">
                            <table class="dt">
                              <thead>
                              ...
                              <tbody>
                                <tr
                                  v-for="ln in lineasSec(inv, sec.key)"
                                  ...
                                  <td class="l">
                                    <div v-if="sec.key === 'ingresos'" class="fuente-row">
                                      ...
                                      <button
                                        class="fuente-x"
                                        title="Quitar fuente"
                                        @click="quitarFuente(p, ln)"
                                      >
                                        ✕
                                      </button>
                                    </div>
                                    <div v-else class="cpt">
                                      {{ ln.concepto
                                      }}<span v-if="ln.derivada" class="imp-tag">impuesto</span
                                      ><span
                                        v-if="ln.fuente"
                                        class="fuente-tag"
                                        :title="fuenteTitle(ln.fuente)"
                                    ...
                                    <div v-if="!ln.derivada && ln.fuente" class="origen-wrap">
                                      <span class="origen-mod"
                                        >↳ {{ fuenteOrigen(ln.fuente) }}</span
                                      >
                                    </div>
                                  </td>
                                  <td>
                                ...
                                <tr v-if="sec.key === 'ingresos'">
                                  <td colspan="4">
                                    <button class="fuente-add" @click="agregarFuente(p)">
                      ...
                      <div v-if="sec.key === 'comercializacion'" class="sec-subtotal">
                        ...
                        <span
                          class="sec-tot"
                          :class="{
                            neg: totalSec(inv, 'ingresos') + totalSec(inv, 'comercializacion') < 0,
                          }"
                          >{{
                            fmt(totalSec(inv, 'ingresos') + totalSec(inv, 'comercializacion'))
                          }}</span
                        >
                      </div>
                      <div v-else-if="sec.key === 'costos'" class="sec-subtotal">
                        <span class="sec-lbl">Valor a pagar (Costos Operativos)</span>
                        <span class="sec-tot" :class="{ neg: totalSec(inv, 'costos') < 0 }">{{
                          fmt(totalSec(inv, 'costos'))
                        }}</span>
                      </div>
                      <div v-else-if="sec.key === 'facturas'" class="sec-subtotal">
                        <span class="sec-lbl">Valor a pagar (Facturas de Servicio)</span>
                        <span class="sec-tot" :class="{ neg: totalSec(inv, 'facturas') < 0 }">{{
                          fmt(totalSec(inv, 'facturas'))
                        }}</span>
                      </div>
                    </template>
                    ...
                    <div class="sec-resultado">
                      ...
                      <span class="sec-tot" :class="{ neg: utilidad(inv) < 0 }">{{
                        fmt(utilidad(inv))
                      }}</span>
                    </div>
                  </div>
      ...
      <div class="card">
        <div class="card-h">
          ...
          <div class="pool-actions">
            ...
            <button
              class="btn"
              :disabled="!periodo || clasSaving || !clasDirty"
        ...
        <div class="banner-aviso">
          ...
          La clasificación es <b>por período</b> — un proyecto puede cambiar de tipo entre meses. El
          tipo define cómo se leen los ingresos del ER al cargarlo.
        </div>

        <div v-if="!periodo" class="empty sm">Selecciona un período arriba para clasificar.</div>
        <div v-else-if="clasLoading" class="empty">
          ...
          <table class="ptbl">
            ...
            <tbody>
              <tr v-for="c in clasFiltrados" :key="c.proyecto_id">
                <td class="l">
                  <span class="pname">{{ c.proyecto }}</span>
                </td>
                <td>
    ...
    <template v-else-if="tab === 'diferencia'">
      ...
      <div v-else-if="!diff.tiene_oficial" class="card">
        <div class="empty">
          <i
            class="pi pi-clock"
            style="font-size: 22px; display: block; margin-bottom: 8px; color: var(--p2)"
          />
          ...
          <div class="cards-resumen">
            ...
            <div class="rc">
              ...
              <div class="rc-val" :class="diffClass(proy.utilidad_dif)">
                <span v-if="proy.utilidad_dif != null" class="arrow">{{
                  arrow(proy.utilidad_dif)
                }}</span
                >{{ fmt(proy.utilidad_dif) }}
              </div>
            </div>
          </div>
                ...
                <tbody>
                    ...
                    <template v-if="lineasGrupo(inv, g.key).length">
                      ...
                      <tr v-for="(ln, i) in lineasGrupo(inv, g.key)" :key="g.key + i">
                        ...
                        <td :class="diffClass(ln.diferencia)">
                          <span v-if="ln.diferencia != null && ln.diferencia !== 0" class="arrow">{{
                            arrow(ln.diferencia)
                          }}</span
                          >{{ ln.diferencia != null ? fmt(ln.diferencia) : '—' }}
                        </td>
                        <td :class="diffClass(ln.diferencia)">
                  ...
                  <tr class="tot">
                    ...
                    <td :class="diffClass(inv.utilidad_dif)">
                      <span
                        v-if="inv.utilidad_dif != null && inv.utilidad_dif !== 0"
                        class="arrow"
                        >{{ arrow(inv.utilidad_dif) }}</span
                      >{{ inv.utilidad_dif != null ? fmt(inv.utilidad_dif) : '—' }}
                    </td>
                    <td></td>
                  </tr>
                </tbody>
...
const grupos = [
  { key: 'ingresos', keys: ['ingresos'], label: 'INGRESOS', total: 'Total Ingresos' },
  {
    ...
    keys: ['comercializacion'],
    label: 'COMERCIALIZACIÓN XM',
    total: 'Total Comercialización',
  },
  {
    key: 'costos',
    keys: ['costos', 'facturas'],
    label: 'COSTOS OPERATIVOS',
    total: 'Total Costos Operativos',
  },
]
...
const toggleMapeo = (id) => {
  mapeoOpen[id] = !mapeoOpen[id]
}
const lineasMapeables = (p) =>
  (p.total_100 || []).filter(
...
async function armarPeriodo() {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: `${data.armados} paneles armados`,
    ...
    try {
      ...
      toast.add({
        severity: 'warn',
        summary: 'Paneles armados, vista sin refrescar',
        detail: 'Recarga la página para verlos.',
        life: 6000,
      })
    }
  } catch (e) {
    toast.add({
      severity: 'error',
      summary: 'No se pudo armar el período',
...
async function descargarEr(panel, inversionista = null) {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'No se pudo generar el ER',
      detail: e?.response?.data?.detail || e.message,
      life: 6000,
    })
  }
...
const bloquesMostrados = () =>
  fBloque.value ? bloquesPlano.filter((b) => b.key === fBloque.value) : bloquesPlano
...
const panelesFiltrados = computed(() => {
  ...
  return paneles.value.filter((p) => {
    ...
    if (fEstado.value) {
      ...
      if (fEstado.value === 'solo_cost' && !(!p.liquidar_ingresos && p.liquidar_costos))
        return false
...
const totalGrupo = (inv, grupo) =>
  lineasDe(inv, grupo).reduce((s, l) => s + (Number(l.valor_cop) || 0), 0)
...
const total100Grupo = (p, grupo) =>
  lineas100(p, grupo).reduce((s, l) => s + (Number(l.valor_cop) || 0), 0)
...
const total100Bloque = (p, blk) =>
  lineas100Bloque(p, blk).reduce((s, l) => s + (Number(l.valor_cop) || 0), 0)
// Desglose por inversionista, expandible por proyecto (colapsado por defecto).
const invOpen = reactive({})
const toggleInv = (id) => {
  invOpen[id] = !invOpen[id]
}
...
const total100Sec = (p, sk) =>
  lineas100Sec(p, sk).reduce((s, l) => s + (Number(l.valor_cop) || 0), 0)
...
async function subirSoporte(p, ln, file) {
  ...
  try {
    ...
    aplicarSoporte(p, ln.grupo, ln.concepto, {
...
async function eliminarSoporte(p, ln) {
  ...
  try {
    await api.delete(`/panel-contable/${p.id}/soporte`, {
      params: { grupo: ln.grupo, concepto: ln.concepto },
    })
...
async function cargarPaneles() {
  ...
  try {
    const { data } = await api.get('/panel-contable', {
      params: { periodo: periodo.value, tipo: tipoDatos.value },
    })
    paneles.value = data.paneles || []
    paneles.value.forEach((p, i) => {
      if (open[p.id] === undefined) open[p.id] = i === 0 && esActivo(p)
    })
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'No se pudieron cargar los paneles',
      life: 4000,
    })
  } finally {
    loading.value = false
...
async function cargarClasMap() {
  if (!periodo.value) return
  try {
    const { data } = await api.get('/panel-contable/clasificacion', {
...
async function cargarDiferencia() {
  ...
  loading.value = true
  try {
    const { data } = await api.get('/panel-contable/diferencia', {
      params: { periodo: periodo.value },
    })
    diff.value = data
  } catch (e) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'No se pudo cargar la diferencia',
      life: 4000,
    })
  } finally {
    loading.value = false
...
async function cargarClasificacion() {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: 'No se pudo cargar la clasificación',
      life: 4000,
    })
  } finally {
...
async function guardarClasificacion() {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Clasificación guardada',
      detail: periodoLabel.value,
      life: 3000,
    })
  } catch (e) {
    toast.add({
...
async function onErSelected(e) {
  ...
  if (!periodo.value) {
    toast.add({
      ...
      detail: 'Selecciona el mes/año antes de cargar ER',
      life: 4000,
    })
  ...
  try {
    ...
    if (data.sin_match?.length)
      partes.push(
        `<span style="color:#c0392b">${data.sin_match.length} sin match: ${data.sin_match.join(', ')}</span>`,
      )
    if (data.rechazados?.length)
      partes.push(
        `<span style="color:#d35400">${data.rechazados.length} rechazados por clasificación</span>`,
      )
    if (data.errores?.length)
      partes.push(`<span style="color:#c0392b">${data.errores.length} con error</span>`)
    uploadMsg.value = partes.join(' · ')
    toast.add({
      severity: 'success',
      summary: 'ER procesados',
      detail: `${data.cargados?.length || 0} proyecto(s)`,
      life: 3500,
    })
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: err.response?.data?.detail || 'Fallo al procesar ER',
      life: 5000,
    })
  } finally {
    uploading.value = 0
...
function _aplicarLoteFlags(campos, val, trasReasignar = true) {
  const prev = paneles.value.map((p) => ({
    p,
    old: Object.fromEntries(campos.map((c) => [c, p[c]])),
  }))
  ...
  Promise.all(
    paneles.value.map((p) => {
      ...
      return api
        .patch(`/panel-contable/${p.id}`, payload)
        .then(() => null)
        .catch(() => p.id)
    }),
  ).then((res) => {
    ...
    if (fallidos.size) {
      prev.forEach(({ p, old }) => {
        if (fallidos.has(p.id)) Object.assign(p, old)
      })
      toast.add({
        severity: 'warn',
        summary: 'Algunos no se guardaron',
        detail: `${fallidos.size} panel(es) revertido(s)`,
        life: 3500,
      })
    }
...
async function onFlag(p) {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'No se guardó',
      detail: 'Recargando estado…',
      life: 3000,
    })
    cargarPaneles()
  }
...
async function reasignar(soloFaltantes = true) {
  try {
    ...
    paneles.value.forEach((p) => {
      ...
      if (fresh) {
        p.consecutivo_ingresos = fresh.consecutivo_ingresos
        p.consecutivo_costos = fresh.consecutivo_costos
      }
    })
  } catch (e) {
    toast.add({
      severity: 'warn',
      summary: 'Consecutivos',
      detail: 'No se pudieron reasignar los consecutivos',
      life: 3500,
    })
  }
}
...
async function cambiarCelda(p, ln, texto) {
  ...
  if (!m) {
    toast.add({
      ...
      detail: 'Usa hoja!celda, ej. Sheet1!H35',
      life: 3500,
    })
    return
  }
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Celda actualizada',
      detail: ln.concepto + ' ← ' + hoja + '!' + celda,
      life: 2500,
    })
  } catch (err) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: err.response?.data?.detail || 'No se pudo remapear la celda',
      life: 4500,
    })
  }
}
...
async function renombrarFuente(p, ln, nuevaEtiqueta) {
  if (!ln.hoja || !ln.celda) {
    toast.add({
      severity: 'warn',
      summary: 'Sin celda de origen',
      detail: 'Esta fuente no tiene celda de origen; primero asígnale una',
      life: 4000,
    })
    return
  }
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Fuente renombrada',
      detail: nuevaEtiqueta,
      life: 2500,
    })
  } catch (err) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: err.response?.data?.detail || 'No se pudo renombrar la fuente',
      life: 4500,
    })
  }
}
...
async function agregarFuente(p) {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Fuente agregada',
      detail: String(etiqueta).trim(),
      life: 2500,
    })
  } catch (err) {
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: err.response?.data?.detail || 'No se pudo agregar la fuente',
      life: 4500,
    })
  }
}
...
async function quitarFuente(p, ln) {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Error',
      detail: err.response?.data?.detail || 'No se pudo quitar la fuente',
      life: 4500,
    })
  }
}
...
function exportarExcel() {
  ...
  if (!rows.length) {
    toast.add({
      severity: 'warn',
      summary: 'Nada que exportar',
      detail: 'No hay filas con los filtros actuales',
      life: 3000,
    })
    return
  }
...
.pc-period-btn:hover:not(:disabled) {
  background: #f5f2fa;
}
.pc-period-btn:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
.pc-period-label {
...
.vista-toggle .vt-opt.on {
  border-color: #915bd8;
  background: #eee7fb;
  color: #2c2039;
  font-weight: 600;
}
.vista-toggle .vt-opt input {
  accent-color: #915bd8;
  cursor: pointer;
}
...
.sec-body {
  background: #fff;
  border-top: 1px solid var(--line);
  animation: secReveal 0.18s ease;
}
@keyframes secReveal {
  from {
    opacity: 0;
...
.sec-resultado .sec-lbl {
  color: var(--p2);
}
.sec-resultado .sec-tot {
  color: var(--p2);
  font-size: 14px;
}
.sec-resultado .sec-tot.neg {
  color: var(--red);
}
.sec-subtotal {
  display: flex;
...
.sec-subtotal .sec-tot {
  font-size: 12.5px;
  color: var(--txt2);
  font-weight: 700;
  font-variant-numeric: tabular-nums;
}
.sec-subtotal .sec-tot.neg {
  color: var(--red);
}
.tbl-wrap {
  overflow-x: auto;
...
:deep(.pc-dialog .p-dialog-header) {
  background: #faf8fd;
  border-bottom: 1px solid var(--line);
  padding: 14px 18px;
}
:deep(.pc-dialog .p-dialog-content) {
  padding: 18px;
  background: var(--bg);
}
:deep(.pc-dialog .p-dialog-footer) {
  display: flex;
...
.pc-contraste-tabla td {
  padding: 3px 8px;
  border-top: 1px solid #efeaf7;
}
.pc-contraste-tabla .num {
  text-align: right;
  font-variant-numeric: tabular-nums;
}
.pc-contraste-tabla .neg {
  color: #c0392b;
}
.pc-contraste-tabla .pos {
  color: #1d6f42;
}
</style>
