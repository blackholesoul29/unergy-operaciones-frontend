<template>
  ...
  <div v-else-if="panel" class="space-y-5">
    ...
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
      ...
      <div class="kpi-card">
        <p class="kpi-valor pt-1.5 text-base">{{ fmtFecha(panel.kpis.proximo_vencimiento) }}</p>
    ...
    <section class="seccion">
      ...
      <div class="p-4">
        <p v-if="!panel.plantas.length" class="py-4 text-center text-sm" style="color: #9b89b5">
          Sin plantas vinculadas.
        </p>
        <div v-else class="space-y-2">
          <div
            v-for="p in panel.plantas"
            :key="p.proyecto_id"
            class="flex flex-wrap items-center justify-between gap-2 rounded-lg px-3 py-2.5"
            :style="{
              border: '1px solid #ece7f2',
              background:
                p.semaforo && p.semaforo !== 'vigente' ? SEMAFORO[p.semaforo].bg : 'white',
            }"
          >
            <div class="min-w-0">
              <p class="truncate text-sm font-semibold" style="color: #2c2039">{{ p.nombre }}</p>
              <p class="text-xs" style="color: #6b5a8a">
                ...
                <span v-if="p.participacion_actual !== null">
                  · participación {{ p.participacion_actual }}%</span
                >
              </p>
            </div>
    ...
    <section class="seccion">
      ...
      <div class="space-y-4 p-4">
        <p v-if="!panel.condiciones.length" class="py-2 text-center text-sm" style="color: #9b89b5">
          Sin condiciones registradas.
        </p>
        <div v-else class="overflow-x-auto">
          <table class="w-full text-sm">
            <thead>
              <tr class="text-left text-[10px] tracking-wide uppercase" style="color: #9b89b5">
            ...
            <tbody>
              <tr
                v-for="c in panel.condiciones"
                :key="c.contrato_id"
                class="border-t"
                style="border-color: #f3eefa"
              >
                <td class="py-2 pr-3 font-medium" style="color: #2c2039">
                  {{ fmt(c.proyecto_nombre) }}
                </td>
            ...
            <summary class="flex cursor-pointer list-none items-center justify-between">
              ...
              <span class="flex items-center gap-3">
                <ParticipacionSparkline
                  :puntos="
                    g.filas.map((f) => ({ fecha: f.fecha_inicio, porcentaje: f.porcentaje }))
                  "
                />
                <span class="text-sm font-bold tabular-nums" style="color: #915bd8"
                  >{{ fmt(g.actual) }}<template v-if="g.actual !== '—'">%</template></span
                >
              </span>
            </summary>
    ...
    <section class="seccion">
      ...
      <div class="p-4">
        <p v-if="!panel.contratos.length" class="py-4 text-center text-sm" style="color: #9b89b5">
          Sin contratos registrados.
        </p>
        <div v-else class="space-y-2">
          <div
            v-for="c in panel.contratos"
            :key="`${c.fuente}-${c.id}`"
            class="flex flex-wrap items-center justify-between gap-2 rounded-lg px-3 py-2.5"
            style="border: 1px solid #ece7f2"
          >
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                ...
                <span class="truncate text-sm font-semibold" style="color: #2c2039">{{
                  fmt(c.numero)
                }}</span>
                <span
                  v-if="c.semaforo"
              ...
              <p class="mt-0.5 text-xs" style="color: #6b5a8a">
            ...
            <a
              ...
              target="_blank"
              rel="noopener"
              class="flex shrink-0 items-center gap-1 text-xs font-semibold hover:underline"
              style="color: #915bd8"
            >
              <i class="pi pi-external-link text-xs" /> Abrir contrato
            </a>
            <span v-else class="shrink-0 text-xs italic" style="color: #bba8d4">Sin link</span>
          </div>
        </div>
      </div>
    </section>
