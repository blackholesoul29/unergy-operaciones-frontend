<template>
  <div class="mand-root">
    ...
    <div class="mand-periodo-bar">
      ...
      <span
        v-if="badgeMes === 'correcciones' || badgeMes === 'cerrado'"
        class="mand-badge-mes"
        :class="`mand-badge-mes--${badgeMes}`"
      >
    ...
    <div v-else>
      ...
      <div v-if="hayBanner" class="mand-banner">
        ...
        <div class="mand-banner-txt">
          <strong>Vanessa (revisoría)</strong> reportó {{ correcciones.length }} mandato(s) con
          novedad.
          <span class="mand-banner-obs">{{ obsBanner }}</span>
        </div>
        <button class="mand-banner-btn" disabled title="Disponible al conectar Gmail (Fase B)">
          Ver correo
        </button>
      </div>
      ...
      <div class="mand-metricas">
        <div class="mand-metrica">
          <span class="mand-metrica-num">{{ resumen.total }}</span
          ><span class="mand-metrica-lbl">Total</span>
        </div>
        <div class="mand-metrica mand-metrica--amb">
          <span class="mand-metrica-num">{{ resumen.correcciones }}</span
          ><span class="mand-metrica-lbl">Correcciones</span>
        </div>
        <div class="mand-metrica mand-metrica--ver">
          <span class="mand-metrica-num">{{ resumen.firmados }}</span
          ><span class="mand-metrica-lbl">Firmados</span>
        </div>
        <div class="mand-metrica mand-metrica--mor">
          <span class="mand-metrica-num">{{ resumen.enviados_inversionista }}</span
          ><span class="mand-metrica-lbl">Enviados inv.</span>
        </div>
        <div class="mand-metrica">
          <span class="mand-metrica-num">{{ resumen.pendientes }}</span
          ><span class="mand-metrica-lbl">Pendientes</span>
        </div>
      </div>
      ...
      <div class="mand-subtabs">
        <button
          ...
          :key="t.value"
          class="mand-subtab"
          :class="{ 'mand-subtab--active': subTab === t.value }"
          @click="subTab = t.value"
        >
          {{ t.label
          }}<span v-if="t.value === 'correcciones'" class="mand-subtab-count">{{
            correccionesTabCount
          }}</span>
        </button>
      </div>
      ...
      <div class="mand-filtros">
        <Select
          ...
          placeholder="Estado"
          showClear
          class="mand-filtro-sel"
        />
        <Select
          v-model="filtroTercero"
          :options="tercerosOpciones"
          placeholder="Tercero / inversionista"
          showClear
          filter
          class="mand-filtro-sel"
        />
        <span class="mand-buscar"
          ><i class="pi pi-search" /><input
            v-model="buscarCmu"
            type="text"
            placeholder="Buscar CMU…"
        /></span>
      </div>
      ...
      <div class="mand-tabla-wrap">
        <table class="mand-tabla">
          ...
          <tbody>
            <tr
              v-for="m in mandatosFiltrados"
              :key="m.id"
              :class="{ 'mand-fila-resalt': filaResaltada(m.estado) }"
            >
              ...
              <td>{{ m.periodo }}</td>
              <td>
                <span class="mand-badge" :class="`mand-badge--${estadoMeta(m.estado).cls}`">
                  <i
                    v-if="estadoMeta(m.estado).cls === 'neutro-alerta'"
                    class="pi pi-exclamation-triangle"
                    style="font-size: 10px"
                  />
                  {{ estadoMeta(m.estado).label }}</span
                >
              </td>
              <td class="mand-obs">{{ m.observacion || '—' }}</td>
              <td>
                <span v-if="m.tiene_pdf" class="mand-doc-ok">Disponible</span
                ><span v-else class="mand-guion">—</span>
              </td>
              <td>
                <span v-if="m.fecha_envio_inversionista" class="mand-badge mand-badge--morado">{{
                  m.fecha_envio_inversionista
                }}</span>
                <span v-else-if="m.estado === 'firmado'" class="mand-pend-envio"
                  >Pendiente envío</span
                >
                <span v-else-if="m.estado === 'sin_inversionista'" class="mand-sin-inv"
                  >Sin inversionista</span
                >
                <span v-else class="mand-guion">—</span>
              </td>
              <td>
                <button
                  v-if="m.tiene_pdf_zip || m.tiene_pdf"
                  class="mand-clip-btn"
                  title="Ver PDF"
      ...
      <div class="mand-acciones">
        <button class="mand-btn mand-btn--sec" @click="exportarCsv">
          <i class="pi pi-download" /> Exportar
        </button>
        <button class="mand-btn mand-btn--sec" disabled title="Acción interna (Fase B)">
          <i class="pi pi-send" /> Correo a revisoría
        </button>
        ...
        <label class="mand-btn mand-btn--pri">
          <i :class="subiendoZip ? 'pi pi-spin pi-spinner' : 'pi pi-file-import'" /> Cargar ZIP de
          mandatos
          <input type="file" accept=".zip" class="hidden" @change="abrirDialogoZip" />
        </label>
        <button class="mand-btn mand-btn--pri" disabled title="Acción interna (Fase B)">
          <i class="pi pi-share-alt" /> Enviar a inversionistas
        </button>
      </div>
    </div>
    ...
    <div
      v-if="mostrarDialogoZip"
      class="mand-modal-overlay"
      @click.self="mostrarDialogoZip = false"
    >
      <div class="mand-modal">
        ...
        <div class="mand-modal-acciones">
          <button class="mand-btn mand-btn--sec" @click="mostrarDialogoZip = false">
            Cancelar
          </button>
          <button
            class="mand-btn mand-btn--pri"
            :disabled="subiendoZip || !periodoZip"
            @click="confirmarCargaZip"
          >
    ...
    <div v-if="resumenZip" class="mand-resumen">
      <div class="mand-resumen-head">
        <span
          ><i class="pi pi-check-circle" style="color: #3b6d11" />
          {{ resumenZip.detectados }} mandatos detectados</span
        >
        <span
          ><i class="pi pi-check-circle" style="color: #3b6d11" />
          {{ resumenZip.identificados_auto }} inversionistas identificados</span
        >
        <span v-if="resumenZip.sin_inversionista"
          ><i class="pi pi-exclamation-triangle" style="color: #854f0b" />
          {{ resumenZip.sin_inversionista }} sin inversionista</span
        >
      ...
      <div v-if="resumenZip.sugerencias?.length" class="mand-sugerencias">
        ...
        <div v-for="s in resumenZip.sugerencias" :key="s.mandato_id" class="mand-sug-fila">
          ...
          <span class="mand-sug-txt"
            >"{{ s.nombre_extraido }}" → <strong>{{ s.sugerido_nombre }}</strong> ({{
              Math.round(s.score * 100)
            }}%)</span
          >
          <button class="mand-btn mand-btn--sec mand-sug-btn" @click="asignarSugerencia(s)">
            Asignar
          </button>
        </div>
      </div>
      <p v-if="resumenZip.no_parseables?.length" class="mand-noparse">
        {{ resumenZip.no_parseables.length }} archivo(s) no reconocido(s):
        {{ resumenZip.no_parseables.join(', ') }}
      </p>
      <p class="mand-resumen-nota">
        El cruce con Gmail se activará al conectar la cuenta (Fase B).
      </p>
    </div>
  </div>
</template>
...
async function confirmarCargaZip() {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'No se pudo cargar el ZIP',
...
async function asignarSugerencia(s) {
  try {
    await api.patch(`/mandatos/${s.mandato_id}`, {
      inversionista_id: s.sugerido_id,
      estado: 'pendiente_envio',
    })
    resumenZip.value.sugerencias = resumenZip.value.sugerencias.filter(
      (x) => x.mandato_id !== s.mandato_id,
    )
    await cargar()
...
const correccionesTabCount = computed(
  () =>
    mandatos.value.filter((m) => m.estado === 'con_correcciones' || m.estado === 'corregido')
      .length,
)
...
const mandatosFiltrados = computed(() => {
  ...
  if (subTab.value === 'correcciones')
    lista = lista.filter((m) => m.estado === 'con_correcciones' || m.estado === 'corregido')
  else if (subTab.value === 'firmados') lista = lista.filter((m) => m.estado === 'firmado')
  else if (subTab.value === 'enviados_inv')
    lista = lista.filter((m) => m.estado === 'enviado_inversionista')
...
function estadoMeta(e) {
  return ESTADO_META[e] || { label: e, cls: 'neutro' }
}
function filaResaltada(e) {
  return e === 'con_correcciones' || e === 'corregido'
}
...
.mand-badge-mes--correcciones {
  background: #faeeda;
  color: #854f0b;
}
.mand-badge-mes--cerrado {
  background: #eaf3de;
  color: #3b6d11;
}
.mand-sync {
...
.mand-metrica--amb .mand-metrica-num {
  color: #854f0b;
}
.mand-metrica--ver .mand-metrica-num {
  color: #3b6d11;
}
.mand-metrica--mor .mand-metrica-num {
  color: #534ab7;
}
.mand-subtabs {
  display: inline-flex;
...
.mand-subtab:not(.mand-subtab--active) .mand-subtab-count {
  background: #e1d7f5;
  color: #6d4ae8;
}
.mand-filtros {
...
.mand-tabla tbody td {
  padding: 10px 12px;
  border-bottom: 0.5px solid #f1eef7;
  vertical-align: top;
}
.mand-fila-resalt {
  background: #fffbf4;
...
.mand-modal {
  ...
  width: 380px;
  max-width: 92vw;
  box-shadow: 0 8px 30px rgba(28, 18, 50, 0.2);
}
.mand-modal-titulo {
  font-size: 15px;
  font-weight: 700;
