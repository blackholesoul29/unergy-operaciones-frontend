  ...
  <div class="space-y-4">
    ...
    >
      <template #actions>
        ...
        <span
          v-if="ippVigente"
          class="self-center rounded-lg px-2.5 py-1.5 text-xs whitespace-nowrap"
        ...
        <Button
          ...
          icon="pi pi-bolt"
          size="small"
          :loading="accion === 'liquidar'"
          @click="abrir('liquidar')"
        />
      </template>
    </PageHeader>
    ...
    <Dialog v-model:visible="dialogVisible" :header="cfg.header" modal class="w-full max-w-md">
      <div class="space-y-3 pt-1">
        ...
        <p v-if="progreso" class="flex items-center gap-2 text-[11px] text-gray-500">
    ...
    <Dialog
      ...
      header="Diagnóstico del proyecto"
      modal
      class="w-full max-w-2xl"
    >
      <div class="space-y-3 pt-1">
        <p class="text-xs text-gray-500">
          Por qué <strong>{{ diagProyecto }}</strong> sale o no en el estado de resultados de
          {{ filtros.month }}/{{ filtros.year }}.
        </p>
        <div v-if="diagCargando" class="py-8 text-center text-gray-400">
          <i class="pi pi-spin pi-spinner text-2xl" />
        </div>
        <div v-else-if="diag" class="max-h-96 space-y-1.5 overflow-y-auto">
          <div
            v-for="ch in diag.checks"
            :key="ch.key"
          ...
          >
            <i
              :class="ch.status === 'ok' ? 'pi pi-check-circle' : 'pi pi-times-circle'"
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-4 py-2 whitespace-nowrap">
                <span class="rounded px-1.5 py-0.5 text-[11px]" :style="estiloTipo(row.tipo_dato)">
              ...
              <td
                ...
                :style="row.valor < 0 ? 'color:#D64455' : ''"
              >
                {{ fmtNum(row.valor) }}
              </td>
...
const MODOS = {
  ipp: {
    header: 'Consultar IPP',
    submit: 'Consultar',
    version: false,
    ayuda: 'Trae el IPP del mes desde el DANE. Es inmediato y no se puede enviar uno propio.',
  },
  ftp: {
    header: 'Consultar FTP',
    submit: 'Descargar',
    version: true,
    ayuda:
      'Descarga los ocho archivos del FTP de XM. Requiere los códigos SIC y FRT del proyecto y contratos vigentes en el mes.',
  },
  liquidar: {
    header: 'Liquidar',
    submit: 'Liquidar',
    version: true,
    ayuda:
      'Liquida los contratos del período. Requiere el FTP ya descargado y va antes de repartir los costos de XM.',
  },
}
...
async function ejecutar() {
  if (c.mes == null || c.anio == null || (cfg.value.version && !c.version)) {
    toast.add({
      ...
      detail: 'Completa mes, año' + (cfg.value.version ? ' y versión.' : '.'),
      life: 4000,
    })
    return
  }
  ...
  try {
    if (modo.value === 'ipp') {
      ...
      toast.add({
        severity: 'success',
        summary: `IPP de ${nombreMes(c.mes)} ${c.anio}`,
        detail: `${ipp} · queda guardado y se ve en la cabecera`,
        life: 6000,
      })
    ...
    toast.add({
      severity: 'error',
      summary: `${cfg.value.header} falló`,
...
async function diagnosticar(project) {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'No se pudo diagnosticar',
      detail: e.response?.data?.detail || e.message,
      life: 6000,
    })
  } finally {
