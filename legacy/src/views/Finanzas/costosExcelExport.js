...
const PK_ENTRIES = Object.entries(PROJECT_PK).map(([nombre, pk]) => ({
  pk,
  keys: tokensSig(nombre),
}))
...
export async function generarExcelCostos(periodo) {
  ...
  let omFilas = []
  try {
    omFilas = (await api.get(`/om/calculo/${periodo}`)).data.filas || []
  } catch {
    omFilas = []
  }
  ...
  try {
    starlinkData = (await api.get(`/starlink/factura/${periodo}`)).data
  } catch {
    /* sin datos */
  }
  ...
  let arrFilas = []
  try {
    arrFilas = (await api.get(`/arriendos/calculo/${periodo}`)).data.filas || []
  } catch {
    arrFilas = []
  }
  const leaseByPk = {}
