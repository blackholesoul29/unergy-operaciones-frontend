  ...
  <div class="space-y-4 pt-3">
    ...
    >
      ...
      <div class="flex flex-col gap-1">
        ...
        <select
          ...
          class="w-44 rounded-lg border border-gray-200 bg-white px-2 py-1.5 text-sm"
        >
      ...
      <div class="flex flex-col gap-1">
        ...
        <select
          ...
          class="w-48 rounded-lg border border-gray-200 bg-white px-2 py-1.5 text-sm"
        >
          <option value="todos">Todos</option>
        ...
        <div class="section-collapse" :class="{ open: openSections.has(sec.tipo) }">
          <div class="overflow-x-auto">
            <table
              ...
              style="min-width: 1080px; table-layout: fixed"
            >
              <thead>
                <tr class="border-t border-b border-gray-100 bg-gray-50">
                  ...
                  <th
                    ...
                    style="color: #7c3aed; width: 160px"
                  >
                    Canon Arrendamiento
                  </th>
                  <th
              ...
              <tbody>
                <tr
                  ...
                  :key="fila.id"
                  class="group border-t border-gray-100 transition-colors duration-100 hover:bg-gray-50/70"
                >
                  ...
                  <td
                    ...
                    style="color: #2c2039; overflow: hidden; text-overflow: ellipsis"
                    :title="fila.proyecto"
                  >
                    <div class="flex max-w-full flex-col gap-0.5">
                      ...
                      <span class="inline-flex max-w-full flex-wrap items-center gap-1.5">
                        ...
                        <span
                          v-if="!fila.aplica_este_mes && conContrato(fila)"
                          class="inline-flex flex-shrink-0 items-center gap-1 rounded-full px-1.5 py-0.5 text-[10px] font-normal"
                        ...
                        <span
                          ...
                          class="inline-flex flex-shrink-0 cursor-help items-center gap-1 rounded-full px-1.5 py-0.5 text-[10px] font-normal"
                      ...
                      <span
                        v-if="fila.nombre_arrendador"
                        class="truncate text-[11px] text-gray-400"
                        :title="fila.nombre_arrendador"
                      >
                  ...
                  <!-- Canon calculado -->
                  <td
                    class="bg-purple-50/30 px-4 py-2 text-right tabular-nums"
                    :class="!esFacturable(fila) ? 'opacity-40' : ''"
                  ...
                  >
                    <span class="inline-flex items-center justify-end gap-1">
                      ...
                      <i
                        v-if="fila.canon_calculado != null"
                        class="pi pi-info-circle flex-shrink-0 cursor-help opacity-0 transition-opacity group-hover:opacity-100"
                  ...
                  <td
                    ...
                    :class="!esFacturable(fila) ? 'opacity-40' : ''"
                  >
                    <div class="inline-flex flex-wrap items-center justify-center gap-0.5">
    ...
    >
      ...
      <div class="flex-1 text-xs" style="color: #92400e">
        <p class="mb-1 font-semibold">IPC pendiente para próximos períodos</p>
    ...
    >
      <p class="mb-3 text-sm text-gray-600">
        Estos arriendos son facturables este mes pero los desmarcaste. Indica el motivo de cada
      ...
      <template #footer>
        <button
          ...
          @click="showExclusionDialog = false"
        >
          Cancelar
        </button>
        <button
    ...
    <!-- ── Dialog IPC ─────────────────────────────────────────────────────── -->
    <Dialog
      v-model:visible="showIPCDialog"
      modal
      header="Tasas IPC — Arriendos"
      :style="{ width: '460px' }"
    >
    ...
    <CalculoIpcPopover
      ref="canonPopover"
      :valor-base-anual="
        filaCanon && filaCanon.valor_base != null ? filaCanon.valor_base * 12 : null
      "
...
async function cargarDatos() {
  ...
  try {
    ...
    filas.value.forEach((f) => {
      seleccion[f.id] = f.incluido && f.habilitado
    })
  } catch {
...
const filasFiltradas = computed(() => {
  ...
  return filas.value.filter((f) => {
    ...
    if (
      ...
      (f.estado_contrato || 'con_contrato') !== filtroEstado.value
    )
      return false
    if (
...
function toggleTodosSeccion(items, checked) {
  seccionFacturables(items).forEach((f) => {
    seleccion[f.id] = checked
  })
}
...
const totalSeleccionado = computed(() =>
  filas.value
    .filter((f) => esFacturable(f) && seleccion[f.id])
    .reduce((s, f) => s + (f.canon_a_facturar || 0), 0),
)
const totalIVASeleccionado = computed(() =>
  filas.value
    .filter((f) => esFacturable(f) && seleccion[f.id])
    .reduce((s, f) => s + (f.iva_calculado || 0), 0),
)
...
function toggleTodos(e) {
  filasHabilitadas.value.forEach((f) => {
    seleccion[f.id] = e.target.checked
  })
}
...
watch(periodoActual, (p) => {
  loadDocs(p)
  cargarDatos()
})
onMounted(() => {
  loadDocs(periodoActual.value)
  cargarDatos()
})
</script>
