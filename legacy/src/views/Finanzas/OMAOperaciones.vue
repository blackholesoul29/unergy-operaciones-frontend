<template>
  <div class="space-y-4 pt-3">
    ...
    >
      ...
      <div class="flex items-center gap-2">
        <div class="relative">
          ...
          <div
            v-if="showColMenu"
            class="absolute top-8 right-0 z-50 space-y-1 rounded-xl border border-gray-200 bg-white p-3 shadow-lg"
            style="min-width: 240px"
          >
            <p class="mb-2 text-xs font-semibold text-gray-500">Mostrar columnas</p>
            <label
              v-for="col in columnasOpcionales"
              :key="col.key"
              class="flex cursor-pointer items-center gap-2 rounded px-1 py-0.5 text-xs hover:bg-gray-50"
            >
    ...
    <template v-else>
              ...
              <thead>
                <tr class="border-t border-b border-gray-100 bg-gray-50">
                  <th class="w-10 px-4 py-2.5 text-left">
                    <input
                      type="checkbox"
                      :checked="todosMarcadosSeccion(sec.items)"
                  ...
                  <th
                    v-if="colsVisibles.n_indexaciones"
                  ...
                  <th
                    v-if="colsVisibles.factor_acumulado"
                    class="px-4 py-2.5 text-right text-xs font-medium tracking-wide whitespace-nowrap text-gray-500 uppercase"
                  >
                    Factor Acum.
                  </th>
                  <th
                    v-if="colsVisibles.valor_anual_indexado"
                    class="px-4 py-2.5 text-right text-xs font-medium tracking-wide whitespace-nowrap text-gray-500 uppercase"
                  >
                    Val. Anual Indexado
                  </th>
                  <th
                    v-if="colsVisibles.valor_mes_completo"
                    class="px-4 py-2.5 text-right text-xs font-medium tracking-wide whitespace-nowrap text-gray-500 uppercase"
                  >
                    Mes Completo
                  </th>
                  <th
                    v-if="colsVisibles.prorrateo"
              ...
              <tbody>
                ...
                >
                  <td
                    ...
                    :class="
                      !fila.habilitado || !fila.aplica_este_mes || !conContrato(fila)
                        ? 'opacity-40'
                        : ''
                    "
                  >
                  ...
                  <td class="px-4 py-2 font-medium" style="color: #2c2039; white-space: nowrap">
                    ...
                    <span
                      v-if="!fila.habilitado && conContrato(fila)"
                      class="ml-1.5 inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 align-middle text-[10px] font-normal"
                    ...
                    <span
                      v-else-if="!fila.aplica_este_mes"
                      class="ml-1.5 inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 align-middle text-[10px] font-normal"
                    ...
                    <span
                      v-if="fila.motivo_exclusion"
                      class="ml-1.5 inline-flex cursor-help items-center gap-1 rounded-full px-1.5 py-0.5 align-middle text-[10px] font-normal"
                  ...
                  >
                    <span
                      ...
                      :style="{
                        background: estadoContratoMeta(fila).bg,
                        color: estadoContratoMeta(fila).fg,
                      }"
                    >
                  ...
                  <td
                    class="px-4 py-2 text-xs whitespace-nowrap text-gray-600"
                    :class="
                      !fila.habilitado || !fila.aplica_este_mes || !conContrato(fila)
                        ? 'opacity-40'
                        : ''
                  ...
                  <td
                    v-if="colsVisibles.n_indexaciones"
                    class="px-4 py-2 text-right text-xs text-gray-500"
                    :class="
                      !fila.habilitado || !fila.aplica_este_mes || !conContrato(fila)
                        ? 'opacity-40'
                        : ''
                  ...
                  <td
                    class="px-4 py-2 text-right font-mono text-xs text-gray-600"
                    :class="
                      !fila.habilitado || !fila.aplica_este_mes || !conContrato(fila)
                        ? 'opacity-40'
                        : ''
                  ...
                  <td
                    v-if="colsVisibles.prorrateo"
                    class="px-4 py-2 text-center text-xs text-gray-500"
                    :class="
                      !fila.habilitado || !fila.aplica_este_mes || !conContrato(fila)
                        ? 'opacity-40'
                        : ''
                  ...
                  <td
                    class="group bg-purple-50/30 px-4 py-2 text-right"
                    :class="
                      !fila.habilitado || !fila.aplica_este_mes || !conContrato(fila)
                        ? 'opacity-40'
                        : ''
                  ...
                  >
                    ...
                    <div class="flex items-center justify-end gap-1.5">
                      ...
                      >
                        <i
                          v-if="fila.habilitado"
                          class="pi pi-info-circle cursor-pointer text-[11px]"
                          style="color: #915bd8"
                          title="Ver cálculo"
                      ...
                      <span
                        ...
                        :style="
                          fila.incluido && fila.habilitado ? 'color:#7c3aed' : 'color:#9ca3af'
                        "
                      >
                  ...
                  <td
                    class="bg-purple-50/30 px-4 py-2 text-right font-mono text-xs"
                    :class="
                      !fila.habilitado || !fila.aplica_este_mes || !conContrato(fila)
                        ? 'opacity-40'
                        : ''
                  ...
                  >
                    {{
                      valorEfectivo(fila) != null
                        ? formatCOP(valorEfectivo(fila) * (1 + IVA_TASA))
                        : '—'
                    }}
                  </td>
                  <td
                  ...
                  >
                    <button
                      v-if="conContrato(fila)"
                      type="button"
                      @click="toggleFacturado(fila)"
                      class="inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 text-xs font-medium transition-colors"
                      :class="
                        togglingFacturado[fila.contrato_id] ? 'pointer-events-none opacity-50' : ''
                      "
                      :style="
                        fila.facturado
                    ...
                    >
                      <i
                        :class="fila.facturado ? 'pi pi-check' : 'pi pi-circle'"
                        class="text-[10px]"
                      />{{ fila.facturado ? 'Sí' : 'No' }}
                    </button>
                  ...
                  >
                    <DocumentoIcon
                      :doc="
                        fila.documento_disponible
                          ? { nombre_archivo: fila.documento_nombre || fila.nombre_proyecto }
                          : null
                      "
                      :tooltip="
                        fila.documento_disponible
                          ? fila.documento_nombre || fila.nombre_proyecto
                          : null
                      "
      ...
      >
        ...
        <div class="ml-auto flex items-center gap-6">
          ...
          <div class="text-right">
            <p class="text-[10px] font-medium tracking-wide text-gray-400 uppercase">IVA (19%)</p>
            <p class="text-sm font-semibold tabular-nums" style="color: #2c2039">
              {{ formatCOP(totalIVASeleccionado) }}
            </p>
          </div>
          <div class="text-right">
            <p class="text-[10px] font-medium tracking-wide text-gray-400 uppercase">Total</p>
            <p class="text-base font-bold tabular-nums" style="color: #7c3aed">
              {{ formatCOP(totalConIVASeleccionado) }}
            </p>
          </div>
        </div>
      </div>
    ...
    >
      <i
        class="pi pi-file-pdf flex-shrink-0 text-sm"
      ...
      <button
        v-if="facturaProveedor.tiene_archivo"
        type="button"
      ...
      <a
        v-else-if="facturaProveedor.enlace_pdf"
        :href="facturaProveedor.enlace_pdf"
        target="_blank"
        rel="noopener"
        class="flex flex-shrink-0 items-center gap-1 text-xs font-medium hover:underline"
        style="color: #915bd8"
    ...
    <Dialog
      v-model:visible="showExclusionDialog"
      modal
      header="Motivo de exclusión"
      :style="{ width: '30rem' }"
    >
      <p class="mb-3 text-sm text-gray-600">
        Estos proyectos <b>aplican este mes</b> pero los desmarcaste. Indica el motivo de cada
        exclusión (queda registrado):
      </p>
      ...
      <template #footer>
        ...
        <Button
          label="Guardar"
          :disabled="!exclusionValida"
          @click="confirmarExclusiones"
          style="background: #915bd8; border-color: #915bd8"
        />
      </template>
    </Dialog>
    ...
    <Popover ref="infoPopover">
      <div v-if="filaInfo" class="text-xs" style="min-width: 280px; color: #2c2039">
        ...
        <div class="space-y-1 font-mono">
          ...
          <div
            v-if="filaInfo.prorrateo_label && filaInfo.prorrateo_label !== 'Completo'"
        ...
        <div
          v-if="filaInfo.ipc_incompleto"
          class="mt-2 flex items-start gap-1.5 rounded-md p-2 text-[11px]"
          style="background: #fffbeb; color: #92400e"
        >
          <i class="pi pi-exclamation-triangle mt-0.5 text-[11px]" style="color: #d97706" />
          <span
            >Falta la tasa IPC de algún año del período; la indexación mostrada es parcial. Cárgala
            con el botón IPC.</span
          >
        </div>
        ...
        <div
          v-if="esManual(filaInfo)"
          class="mt-2 flex items-start gap-1.5 rounded-md border-t p-2 pt-2 text-[11px]"
          style="background: #fffbeb; color: #92400e"
        >
          <i class="pi pi-exclamation-triangle mt-0.5 text-[11px]" style="color: #d97706" />
          <div class="flex-1">
    ...
    <Dialog v-model:visible="showIPCDialog" modal header="Tasas IPC" :style="{ width: '420px' }">
      <div class="space-y-3 pt-1">
        <p class="text-xs text-gray-500">
          La tasa del año en que cae cada aniversario del contrato se aplica desde ese aniversario,
          no desde enero. El aniversario se cuenta desde la fecha base — max(suscripción, inicio de
          operación) — y solo indexa una vez que ya se cumplió.
        </p>
        <DataTable :value="ipcTasas" class="text-sm" stripedRows>
          ...
          <Column header="Estado">
            <template #body="{ data }">
              <Tag
                :value="data.confirmado ? 'Confirmado' : 'Pendiente'"
...
const filasHabilitadas = computed(() =>
  filas.value.filter((f) => f.habilitado && f.aplica_este_mes && conContrato(f)),
)
const filasSeleccionadas = computed(
  () => filasHabilitadas.value.filter((f) => seleccion[f.contrato_id]).length,
)
...
const filasFiltradas = computed(() => {
  ...
  return filas.value.filter((f) => {
    ...
    if (
      ...
      return false
    if (
      filtroEstadoContrato.value !== 'todos' &&
      (f.estado_contrato || 'con_contrato') !== filtroEstadoContrato.value
    )
      return false
    return true
  })
...
const estadoContratoMeta = (f) =>
  ESTADO_CONTRATO_META[f.estado_contrato] || ESTADO_CONTRATO_META.con_contrato
...
function toggleTodosSeccion(items, checked) {
  seccionHabilitadas(items).forEach((f) => {
    seleccion[f.contrato_id] = checked
  })
}
...
function confirmarExclusiones() {
  const motivos = {}
  exclusionPendientes.value.forEach((e) => {
    motivos[e.contrato_id] = e.motivo.trim()
  })
...
async function _ejecutarGuardado(motivos) {
  ...
  try {
    const items = filas.value
      .filter((f) => conContrato(f))
      .map((f) => {
        const ov = overrides[f.contrato_id]
        let valor_manual = null
        if (ov && ov.dirty)
          valor_manual = ov.valor // override local (o null si revertido)
...
async function guardarIPC() {
  ...
  const filasAntes = filas.value.map((f) => ({
    contrato_id: f.contrato_id,
    nombre: f.nombre_proyecto,
    valor: f.valor_a_facturar,
  }))
  try {
    ...
    filas.value.forEach((f) => {
      ...
      if (
        antes &&
        antes.valor != null &&
        f.valor_a_facturar != null &&
        antes.valor !== f.valor_a_facturar
      ) {
        ...
        afectados.push({
          contrato_id: f.contrato_id,
          nombre: f.nombre_proyecto,
          valor_anterior: antes.valor,
          valor_nuevo: f.valor_a_facturar,
          diff_cop,
          diff_pct,
        })
      }
  ...
  try {
    ...
    if (e.response?.status === 404) {
      toast.add({
        ...
        detail:
          'La factura de este período ya no está en el servidor. Vuelve a subirla desde la pestaña Proveedor.',
        life: 6000,
      })
    } else {
...
async function descargarDocumento(fila) {
  ...
  try {
    const resp = await api.get(`/om/documento/${fila.periodo}/${fila.contrato_id}`, {
      responseType: 'blob',
    })
    const url = URL.createObjectURL(resp.data)
    const a = document.createElement('a')
    ...
    if (e.response?.status === 404) {
      toast.add({
        ...
        summary: 'Archivo no disponible',
        detail:
          'El documento de este proyecto ya no está en el servidor. Vuelve a subir la factura del período desde Proveedor.',
        life: 6000,
      })
    } else {
