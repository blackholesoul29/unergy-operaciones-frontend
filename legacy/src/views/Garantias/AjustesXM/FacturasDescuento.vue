<template>
  <div class="mt-4 overflow-hidden rounded-xl bg-white shadow-sm" style="border: 1px solid #e8e0f0">
    <div class="flex items-center justify-between px-4 py-2" style="background: #f3f0f7">
      <span class="text-xs font-bold tracking-widest uppercase" style="color: #915bd8"
      ...
      <table class="text-xs" style="table-layout: fixed; width: 100%; min-width: 880px">
        ...
        <tbody>
          ...
          >
            ...
            <td class="truncate px-3 py-1.5" style="color: #2c2039" :title="f.numero ?? ''">
              {{ f.numero ?? '—' }}
            </td>
            ...
            <td class="truncate px-3 py-1.5" style="color: #6b5a8a" :title="f.concepto">
              {{ f.concepto }}
            </td>
            <td class="px-3 py-1.5">
              <div class="flex items-center gap-1">
                <span
                  ...
                  :style="f.signo < 0 ? 'color:#10B981' : 'color:#D64455'"
                  >{{ f.signo < 0 ? '−' : '+' }}</span
                >
                <InputNumber
            ...
            <td class="px-3 py-1.5">
              <input
                type="date"
                v-model="f.vencimiento"
                class="w-full rounded px-2 py-1 text-xs"
                style="border: 1px solid #e8e0f0; box-sizing: border-box"
              />
            </td>
            <td class="px-3 py-1.5 text-center" style="color: #9ca3af">{{ f.pagina }}</td>
            <td class="truncate px-3 py-1.5">
              <span
                v-if="f.warnings && f.warnings.length"
    ...
    <!-- Pie -->
    <div class="grid grid-cols-3 gap-3 border-t px-4 py-3" style="background: #f9f7fd">
      <div class="text-center">
        <p class="text-[10px] font-semibold tracking-wide uppercase" style="color: #6b5a8a">
          Disponible original
        </p>
      ...
      <div class="text-center">
        ...
        <p
          ...
          :style="disponibleAjustado < 0 ? 'color:#D64455' : 'color:#10B981'"
        >
          {{ fmtCOP(disponibleAjustado) }}
        </p>
      </div>
    </div>
  </div>
