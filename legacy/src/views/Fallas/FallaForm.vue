  ...
  <form @submit.prevent="submit" class="ff-form">
      ...
      <div class="ff-grid">
        ...
        <div v-if="usarEstructura" class="ff-field ff-span2 ff-estructura">
          ...
          <div v-if="catActual && catActual.tipo === 'opcion'" class="ff-sub">
            ...
            <Select
              v-model="cls.subtipo"
              :options="catActual.opciones"
              optionLabel="etiqueta"
              optionValue="codigo"
              placeholder="Seleccionar…"
              class="w-full"
              :class="{ 'p-invalid': errors.subtipo }"
            />
            ...
            <div v-if="opcionActual?.pendiente_reclasificar" class="ff-banner ff-banner--warn">
              <i class="pi pi-clock" /> Quedará <strong>pendiente de reclasificar</strong> hasta
            ...
            <div v-if="opcionActual?.requiere_detalle" class="ff-subfield">
              ...
              <Textarea
                ...
                autoResize
                class="w-full"
                :class="{ 'p-invalid': errors.detalle }"
          ...
          <div v-else-if="catActual && catActual.tipo === 'equipo'" class="ff-sub">
            ...
            <div class="ff-flags">
              <label class="ff-check"
                ><input type="checkbox" v-model="cls.afecta_medicion" /> Afecta la medición de la
                frontera</label
              >
              <label class="ff-check"
                ><input type="checkbox" v-model="cls.perdida_comunicacion" /> Pérdida de
                comunicación (datos) de la frontera</label
              >
            </div>
          ...
          <div v-else-if="catActual && catActual.tipo === 'inversores'" class="ff-sub">
            <div v-if="!proyectoUnicoId" class="ff-banner ff-banner--warn">
              <i class="pi pi-info-circle" /> Para reportar inversores selecciona
            ...
            <template v-else>
              ...
              <div
                ...
                class="ff-banner ff-banner--warn"
              >
                Este proyecto no tiene inversores configurados.
                <button type="button" class="ff-link" @click="gestionInversores = true">
                  Configurar
                </button>
              </div>
              <MultiSelect
                v-else-if="inversoresProyecto.length"
                v-model="cls.inversores_ids"
                :options="inversoresProyecto"
                optionLabel="_label"
                optionValue="id"
                placeholder="Seleccionar inversor(es)"
                display="chip"
                class="w-full"
                :class="{ 'p-invalid': errors.inversores }"
              />
              ...
              <div v-if="gestionInversores" class="ff-invmanage">
                <div class="ff-invmanage-head">
                  <span
                    ...
                    Suma inversores:
                    <strong :class="{ 'ff-text-danger': sumaExcede }"
                ...
                <div v-for="inv in inversoresProyecto" :key="inv.id" class="ff-invrow">
                  ...
                  />
                  <InputNumber
                    v-model="inv.potencia_nominal_kw"
                    placeholder="kW"
                    suffix=" kW"
                    :min="0"
                    class="ff-invkw"
                    @blur="guardarInv(inv)"
                  />
                  <button type="button" class="ff-invdel" @click="eliminarInv(inv)">
                    <i class="pi pi-trash" />
                  </button>
                </div>
                <div class="ff-invrow ff-invrow--new">
                  ...
                  />
                  <InputNumber
                    v-model="nuevoInv.potencia_nominal_kw"
                    placeholder="kW"
                    suffix=" kW"
                    :min="0"
                    class="ff-invkw"
                  />
                  <button type="button" class="ff-invadd" @click="agregarInv">
                    <i class="pi pi-plus" />
                  </button>
                </div>
        ...
        <div v-else class="ff-field">
          ...
          <div v-if="!form.tipo_id" class="mt-1">
            <InputText
              v-model="form.tipo_libre"
              placeholder="O escribe el tipo aquí si no está en la lista..."
              class="w-full"
        ...
        <div class="ff-field">
          ...
          />
          <small v-if="errors.fecha_identificacion" class="ff-error">{{
        ...
        <div v-if="esEstadoProgramado" class="ff-field ff-highlight">
          ...
          />
          <small v-if="errors.fecha_programada" class="ff-error">{{
      ...
      <div class="ff-grid">
        <div class="ff-field">
          ...
          <DatePicker
            ...
            showTime
            hourFormat="24"
            :class="{ 'p-invalid': errors.fecha_resolucion }"
          />
          <small v-if="errors.fecha_resolucion" class="ff-error">{{
        ...
        <div class="ff-field">
          ...
          <Select
            v-model="form.tipo_solucion"
            :options="TIPOS_SOLUCION"
            placeholder="Seleccionar tipo"
            showClear
            class="w-full"
          />
    ...
    <div class="ff-section">
      ...
      >
        ...
        <span class="ff-dropzone-text"
          >Arrastra archivos aquí o <span class="ff-dropzone-link">haz clic</span></span
        >
        <span class="ff-dropzone-hint">Imágenes, PDF, Excel, Word, CSV</span>
        <input
          ref="fileInputRef"
          type="file"
          class="ff-hidden-input"
          accept="*"
          multiple
...
const form = ref({
  ...
  nota_inicial: '',
  fecha_programada: props.initial?.fecha_programada
    ? new Date(props.initial.fecha_programada)
    : null,
...
async function submit() {
  ...
  try {
    ...
    if (esEstadoFinal.value && form.value.fecha_resolucion)
      base.fecha_resolucion =
        form.value.fecha_resolucion instanceof Date
          ? form.value.fecha_resolucion.toISOString()
          : form.value.fecha_resolucion
    if (esEstadoFinal.value && form.value.tipo_solucion)
      base.tipo_solucion = form.value.tipo_solucion
    ...
    if (form.value.energia_perdida_kwh != null)
      base.energia_perdida_kwh = form.value.energia_perdida_kwh
