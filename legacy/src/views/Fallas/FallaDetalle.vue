<template>
  <div class="flex h-full flex-col space-y-4 overflow-y-auto pb-6">
    <!-- Status strip -->
    <div class="flex flex-wrap items-center gap-2">
      <Tag :value="falla.estado?.etiqueta" :style="tagStyle(falla.estado)" />
      <Tag
        v-if="falla.prioridad"
        :value="falla.prioridad?.etiqueta"
        :style="prioTagStyle(falla.prioridad)"
      />
      <span class="ml-auto text-xs font-bold" :style="{ color: diasColor(falla.dias_abierta) }">
        {{ falla.dias_abierta ?? '—' }} días abierta
      </span>
    </div>

    <!-- Info principal -->
    <div class="space-y-3 rounded-xl border p-4" style="border-color: #e8e0f0">
    ...
    <div
      v-if="falla.tipo_falla?.accion_sugerida"
      class="rounded-xl border-l-4 px-4 py-3 text-sm"
      style="border-color: #915bd8; background: #f9f6fe; color: #4b3b72"
    >
      <p class="mb-1 text-[10px] font-bold tracking-wide uppercase" style="color: #915bd8">
        Acción sugerida
      </p>
    ...
    <div class="space-y-3 rounded-xl border p-4" style="border-color: #e8e0f0">
      <p class="text-xs font-bold tracking-wide uppercase" style="color: #915bd8">
        Actualizar estado
      </p>
      <div class="grid grid-cols-2 gap-2">
        ...
        <div>
          ...
          <Select
            v-model="editState.prioridad_id"
        ...
        <div>
          ...
          <Select
            v-model="editState.tipo_resolucion_id"
            :options="catalogos.resoluciones"
            option-value="id"
            option-label="etiqueta"
            class="w-full"
            show-clear
        ...
        <div>
          ...
          <InputNumber
            v-model="editState.energia_perdida_kwh"
            class="w-full"
      ...
      <div v-if="falla.fotos?.length" class="grid grid-cols-3 gap-2">
        ...
        >
          ...
          >
            ...
            <span class="mt-1 w-full truncate px-1 text-center text-[9px]" style="color: #9b89b5">{{
              foto.nombre_archivo
            }}</span>
          </div>
          <!-- Overlay delete -->
          ...
          >
            <a
              :href="foto.url"
              target="_blank"
              class="rounded-lg bg-white p-1.5 text-xs"
              style="color: #2c2039"
              title="Ver"
            >
              <i class="pi pi-eye" />
            ...
            <button
              class="rounded-lg bg-red-500 p-1.5 text-xs text-white"
              title="Eliminar"
          ...
          <span
            v-if="foto.etapa"
            class="absolute top-1 left-1 rounded px-1.5 py-0.5 text-[9px] font-bold"
            style="background: #2c2039; color: #fff; opacity: 0.85"
          >
            {{ foto.etapa }}
          </span>
        </div>
      </div>
      <p v-else class="py-3 text-center text-xs" style="color: #9b89b5">Sin fotos adjuntas</p>
    </div>
    ...
    <div class="space-y-3 rounded-xl border p-4" style="border-color: #e8e0f0">
      ...
      <div class="max-h-64 space-y-3 overflow-y-auto pr-1">
        <div
          v-for="seg in [...(falla.seguimientos || [])].reverse()"
          :key="seg.id"
          class="border-l-2 py-1 pl-3"
          style="border-color: #e0d4f5"
        >
          <div class="mb-1 flex items-center gap-2">
            ...
            <Tag
              v-if="seg.estado_nuevo"
              :value="seg.estado_nuevo"
        ...
        <p
          v-if="!falla.seguimientos?.length"
      ...
      <!-- Nueva nota -->
      <div class="space-y-2 border-t pt-2" style="border-color: #f0eaf8">
        <div class="grid grid-cols-2 gap-2">
          <div>
            <label class="field-label"
              >Nuevo estado <span style="color: #9b89b5">(opcional)</span></label
            >
            <Select
              v-model="newSeg.estado_nuevo"
              :options="catalogos.estados"
              option-value="codigo"
              option-label="etiqueta"
              class="w-full"
...
const proyectoNombre = computed(
  () => props.falla.proyecto?.nombre_comercial || props.falla.proyecto_nombre_raw || '—',
)
...
<style scoped>
.field-label {
  @apply mb-1 block text-[10px] font-semibold tracking-wide uppercase;
  color: #9b89b5;
}
</style>
