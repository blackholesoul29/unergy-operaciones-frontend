  ...
  <div class="fmv-wrap">
    ...
    <div class="fmv-header">
      <div class="fmv-header-left">
        ...
        <span class="text-sm font-bold text-gray-800">Mapa de fallas</span>
        <span class="hidden text-xs text-gray-400 sm:inline">· Proyectos con fallas activas</span>
      </div>
    ...
    <div class="fmv-legend">
      <span class="fmv-legend-item"
        ><span class="fmv-dot" style="background: #16a34a"></span> Sin fallas</span
      >
      <span class="fmv-legend-item"
        ><span class="fmv-dot" style="background: #d97706"></span> 1–2 fallas</span
      >
      <span class="fmv-legend-item"
        ><span class="fmv-dot" style="background: #dc2626"></span> 3+ fallas</span
      >
    ...
    <div v-else-if="loading && !mapListo" class="fmv-empty">
      ...
      <p class="mt-3 text-sm text-gray-500">Cargando mapa…</p>
    </div>
    ...
    <transition name="fmv-slide">
      <div v-if="proyectoSel" class="fmv-panel">
        <div class="fmv-panel-header">
          <span class="flex-1 truncate text-sm font-bold text-gray-800">{{
            proyectoSel.nombre_comercial
          }}</span>
          <button class="fmv-close" @click="proyectoSel = null">
        ...
        <div class="fmv-panel-body">
          ...
          <div class="fmv-stat">
            <span
              class="fmv-stat-num"
              :style="{ color: colorPorN(fallasPorId[proyectoSel.id] || 0) }"
            >
              {{ fallasPorId[proyectoSel.id] || 0 }}
            </span>
            <span class="fmv-stat-lbl"
              >falla{{ (fallasPorId[proyectoSel.id] || 0) !== 1 ? 's' : '' }} activa{{
                (fallasPorId[proyectoSel.id] || 0) !== 1 ? 's' : ''
              }}</span
            >
          </div>
          ...
          <div class="fmv-info-row" v-if="proyectoSel.municipio">
            <i class="pi pi-map-marker" /> {{ proyectoSel.municipio
            }}{{ proyectoSel.departamento ? ', ' + proyectoSel.departamento : '' }}
          </div>
          <div class="fmv-info-row" v-if="proyectoSel.potencia_instalada_kwp">
            <i class="pi pi-sun" />
            {{ Number(proyectoSel.potencia_instalada_kwp).toLocaleString('es-CO') }} kW AC
          </div>
    ...
    <transition name="fmv-slide">
      <div v-if="subSel" class="fmv-panel fmv-panel--sub">
        <div class="fmv-panel-header">
          ...
          <span class="flex-1 truncate text-sm font-bold text-gray-800">{{ subSel.name }}</span>
          <button class="fmv-close" @click="subSel = null">
            <i class="pi pi-times text-xs" />
          </button>
        </div>
        <div class="fmv-panel-body">
          ...
          <div class="fmv-info-row">
            <i class="pi pi-share-alt" /> {{ subSel.circuit_count }} circuito{{
              subSel.circuit_count !== 1 ? 's' : ''
            }}
          </div>
        </div>
      </div>
...
function prioColor(c) {
  return PRIO[c] || '#9ca3af'
}
...
async function cargar() {
  ...
  try {
    const [proyRes, mapaRes] = await Promise.allSettled([
      ...
      operadorSel.value
        ? api.get('/mapa', { params: { operator: operadorSel.value } })
        : Promise.resolve(null),
    ])

    proyectos.value =
      proyRes.status === 'fulfilled'
        ? (proyRes.value.data.items ?? []).filter((p) => p.latitud && p.longitud)
        : []

    const mapaData = mapaRes.status === 'fulfilled' && mapaRes.value ? mapaRes.value.data : null
...
function agregarCapasOR(mapaData) {
  ...
  map.addLayer({
    ...
    paint: {
      ...
      'line-width': ['interpolate', ['linear'], ['get', 'tension_level'], 13.2, 1.5, 115, 2.5],
    },
  })
  ...
  map.on('mouseenter', 'circuits-line', (e) => {
    ...
    popup
      .setLngLat(e.lngLat)
      .setHTML(
        `<div style="font-size:12px;font-weight:600">${p.circuit_name}</div><div style="font-size:11px;color:#6b5a8a">${p.tension_level} kV · ${p.substation_name || ''}</div>`,
      )
      .addTo(map)
  })
...
function agregarSubestaciones(substations) {
  ...
  for (const s of substations) {
    ...
    `
    el.innerHTML =
      '<svg width="8" height="8" viewBox="0 0 8 8"><polygon points="4,0 8,8 0,8" fill="#fff"/></svg>'
...
.fmv-btn-icon:hover:not(:disabled) {
  background: #e9e0f5;
}
.fmv-btn-icon:disabled {
  opacity: 0.4;
  cursor: not-allowed;
}
...
.fmv-panel {
  ...
  box-shadow: 0 8px 24px rgba(28, 18, 50, 0.14);
  z-index: 10;
  overflow: hidden;
}
.fmv-panel--sub {
...
/* Transición panel */
.fmv-slide-enter-active,
.fmv-slide-leave-active {
  transition: all 0.2s;
}
.fmv-slide-enter-from,
.fmv-slide-leave-to {
  opacity: 0;
