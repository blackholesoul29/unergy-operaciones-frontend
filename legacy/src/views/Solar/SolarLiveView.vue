        ...
        <div class="sl-header-right">
          ...
          <div class="sl-refresh-wrap">
            ...
            <div class="sl-auto-wrap">
              <button
                class="sl-auto-btn"
                :class="autoInterval && 'sl-auto-btn--on'"
                @click="toggleAutoMenu"
                :title="autoInterval ? `Auto: ${autoLabel}` : 'Auto-actualizar'"
              >
                <i class="pi pi-clock" />
              ...
              <div v-if="autoMenuOpen" class="sl-auto-menu">
                <button
                  class="sl-auto-option"
                  :class="!autoInterval && 'sl-auto-option--active'"
                  @click="setAuto(0)"
                >
                  Desactivado
                </button>
                <button
                  ...
                  :key="opt.ms"
                  class="sl-auto-option"
                  :class="autoInterval === opt.ms && 'sl-auto-option--active'"
            ...
            <template v-else>
              ...
              <div v-if="getDiffPct(proy.proyecto_id) !== null" class="sl-diff-row">
                ...
                <span
                  :class="[
                    'sl-diff-badge',
                    Math.abs(getDiffPct(proy.proyecto_id)) > 5 ? 'sl-diff-warn' : 'sl-diff-ok',
                  ]"
                >
              ...
              <div class="sl-genhoy">
                <div class="sl-genhoy-head">
                  ...
                  <div class="sl-genhoy-vals">
                    ...
                    <span
                      ...
                      class="sl-genhoy-badge"
                      title="Dato de inversores"
                      >INV</span
                    >
                    <span
                      v-else-if="getGenHoy(proy.proyecto_id).fuente === 'medidor'"
                      class="sl-genhoy-badge sl-genhoy-badge--med"
                      title="Dato de medidor de frontera"
                      >MED</span
                    >
                    <span
                      v-else
                      class="sl-genhoy-badge sl-genhoy-badge--nd"
                      title="Sin dato disponible"
                      >S/D</span
                    >
                  </div>
                </div>
                <div class="sl-genhoy-track">
                  <div
                    class="sl-genhoy-fill"
                    :style="{
                      width:
                        getGenHoy(proy.proyecto_id).p90 > 0
                          ? Math.min(
                              100,
                              (getGenHoy(proy.proyecto_id).real / getGenHoy(proy.proyecto_id).p90) *
                                100,
                            ) + '%'
                          : '0%',
                      background:
...
function onFiltroComplete(e) {
  const nombres = [...new Set((proyectos.value || []).map((p) => p.nombre).filter(Boolean))].sort(
...
.sl-filter-ac :deep(input:focus) {
  border-color: #915bd8;
}
.sl-filter-ac :deep(input::placeholder) {
  color: #9ca3af;
}
.sl-filter-clear {
  position: absolute;
...
.sl-auto-menu {
  ...
  overflow: hidden;
  z-index: 50;
  box-shadow: 0 8px 24px rgba(28, 18, 50, 0.12);
}
.sl-auto-option {
  display: block;
  width: 100%;
...
.sl-auto-option:hover {
  background: #f3f1f8;
}
.sl-auto-option--active {
  background: rgba(145, 91, 216, 0.1);
  color: #7c3aed;
  font-weight: 600;
}
