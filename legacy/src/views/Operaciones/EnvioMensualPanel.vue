    ...
    <header class="em-header">
      ...
      <div class="em-header-controls">
        <div class="em-search-wrap">
          ...
          <button v-if="busqueda" class="em-search-clear" @click="busqueda = ''" title="Limpiar">
            ✕
          </button>
        </div>
      ...
      <div class="em-main">
        ...
        <div v-if="drawerInf" class="em-compact">
          <div class="em-compact-header">
            <span class="em-compact-count"
              >{{ filtrados.length }} informe{{ filtrados.length !== 1 ? 's' : '' }}</span
            >
          </div>
          ...
          <div v-else class="em-compact-list">
            ...
            >
              <span
                class="em-compact-dot"
                :style="{ background: estadoColor(pipelineEstado(inf)) }"
              />
              <div class="em-compact-info">
                ...
                <div class="em-compact-meta">
                  ...
                  <span
                    :class="['em-state-pill', `em-pill-${pipelineEstado(inf)}`]"
        ...
        <div v-else class="em-table-wrap">
          ...
          <table v-else class="em-table">
            ...
            <tbody>
              <template v-for="row in filasAgrupadas" :key="row._group || row.id">
                ...
                <tr
                  v-else
                  :class="['em-row', { 'em-row--active': drawerInf?.id === row.id }]"
                ...
                >
                  ...
                  <td class="em-td-proj">
                    ...
                    <div class="em-proj-sub" v-if="row.tipo === 'port'">
                  ...
                  <td class="em-td-fecha">
                    ...
                    <div class="em-fecha-sub" v-if="row.editado_por_nombre">
                  ...
                  <td class="em-td-fecha">
                    <div v-if="row.correo_enviado">
                      <span class="em-mail-ok"
                        >📧
                        {{ row.correo_enviado_en ? formatFecha(row.correo_enviado_en) : '✓' }}</span
                      >
                      <div class="em-fecha-sub" v-if="row.enviado_por_nombre">
                  ...
                  <td class="em-td-acciones" @click.stop>
                    ...
                    <button
                      ...
                      :class="{
                        ...
                        'em-btn-coms-resolved':
                          comentariosTotales(row) > 0 && comentariosPendientes(row) === 0,
                    ...
                    <button
                      ...
                      class="em-icon-btn em-btn-verify"
                    ...
                    >
                      <i
                        :class="enviandoIds.has(row.id) ? 'pi pi-spin pi-spinner' : 'pi pi-send'"
                      />
                    </button>
                    <button
                      v-if="row.estado !== 'aprobado' && !row.correo_enviado"
                      class="em-icon-btn em-btn-del"
          ...
          <div v-if="!loading && faltantes.length > 0" class="em-faltantes-wrap">
            <button class="em-faltantes-toggle" @click="showFaltantes = !showFaltantes">
              ...
              >
              <i
                :class="showFaltantes ? 'pi pi-chevron-up' : 'pi pi-chevron-down'"
                style="margin-left: auto; font-size: 11px; color: #6b5a8a"
              />
            </button>
        ...
        <aside v-if="drawerInf" class="em-detail">
          ...
          <div class="em-detail-panel">
            ...
            <header class="em-drawer-head">
              <div class="em-drawer-head-info">
                ...
                <div class="em-drawer-sub">
                  ...
                  <span>·</span>
                  <span>{{
                    drawerInf.periodo_display || formatPeriodo(drawerInf.periodo_desde)
                  }}</span>
                  <span>·</span>
            ...
            <div class="em-drawer-tabs">
              <button
                class="em-drawer-tab"
                :class="{ 'em-drawer-tab--on': drawerTab === 'preview' }"
            ...
            <div class="em-drawer-body">
                  ...
                  <div class="em-preview-toolbar">
                    ...
                    <div class="em-preview-actions">
                      ...
                      <Button
                        ...
                        v-tooltip.bottom="'Imprimir / exportar PDF'"
                      />
                    </div>
                  </div>
              ...
              <div v-else-if="drawerTab === 'comentarios'" class="em-coms-wrap">
                <div
                  ...
                  class="em-state em-state-empty em-state-coms-empty"
                >
                ...
                <div v-else class="em-coms-list">
                  <div
                    ...
                    :key="c.id"
                    class="em-com"
                    :class="{ 'em-com--resuelto': c.resuelto }"
                  >
                    ...
                    <div v-if="resolviendoId === c.id" class="em-resolver-form">
                      ...
                      <div class="em-resolver-actions">
                        <button class="em-btn-sm em-btn-ghost" @click="resolviendoId = null">
                          Cancelar
                        </button>
                        <button
                          class="em-btn-sm em-btn-resolver"
                ...
                <div v-if="drawerInf.estado !== 'aprobado'" class="em-com-add">
                  ...
                  <div class="em-com-add-actions">
                    <Button
                      ...
                      size="small"
                      :disabled="
                        !permisoVerificar || !nuevoComentario.trim() || agregandoComentario
                      "
              ...
              <div v-else-if="drawerTab === 'verificar'" class="em-verify-wrap">
                <div v-if="!permisoVerificar" class="em-verify-blocked">
                  ...
                  <p class="em-state-sub">
                    Sólo <b>Juan José Pacheco</b> ({{ EMAIL_VERIFICADOR }}) o un administrador
                    pueden verificar informes.
                  </p>
                </div>
                <template v-else>
                  <div class="em-verify-check">
                    <div
                      class="em-check-row"
                      :class="{ 'em-check-row--err': comentariosPendientes(drawerInf) > 0 }"
                    >
                      <span>{{ comentariosPendientes(drawerInf) > 0 ? '⚠️' : '✅' }}</span>
                      <span>
                        <b>{{ comentariosPendientes(drawerInf) }}</b>
                        comentario{{ comentariosPendientes(drawerInf) !== 1 ? 's' : '' }} sin
                        subsanar
                        {{
                  ...
                  <div class="em-verify-actions">
                    <Button
                      ...
                      class="em-btn-verify-big"
                    />
                    <Button
                      v-else
                      label="↩ Reabrir para corrección"
                      outlined
                      severity="warn"
                      size="small"
                      @click="reabrir"
      ...
      <div v-if="confirmEnvio" class="em-modal-bd" @click.self="cerrarConfirmEnvio">
        <div class="em-modal">
          <header class="em-modal-head">
            ...
            <button class="em-close" :disabled="enviandoBatch" @click="cerrarConfirmEnvio">
              ✕
            </button>
          </header>
          <div class="em-modal-body">
            <div v-if="!enviandoBatch && !resultadoBatch" class="em-modal-list">
              <div v-for="(inf, i) in puedeEnviarBatch" :key="inf.id" class="em-modal-row">
                ...
                <div class="em-modal-info">
                  <div class="em-modal-info-nombre">
            ...
            <div v-else class="em-modal-result">
              <div class="em-result-big">
                <span class="em-result-ok" v-if="resultadoBatch.ok"
                  >✅ {{ resultadoBatch.ok }}</span
                >
                <span class="em-result-err" v-if="resultadoBatch.err"
...
const resumen = computed(() => {
  ...
  informes.value.forEach((i) => {
    r[pipelineEstado(i)] = (r[pipelineEstado(i)] || 0) + 1
  })
  return r
})
  ...
  try {
    ...
    if (inf.tipo === 'port') {
      ...
      for (const el of secciones) {
        ...
        if (!sp) continue
        try {
          await api.patch(`/informes/${inf.id}/seccion`, {
            sub_project: sp,
      ...
      if (bloqueadas.length)
        toast('⚠️ Guardado parcial — secciones bloqueadas: ' + bloqueadas.join(' | '), true)
...
async function eliminarInforme(inf) {
  ...
  if (
    !confirm(
      `¿Eliminar el ${esPort ? 'portafolio' : 'informe'} de "${inf.proyecto_nombre || inf.sub_project}"?${aviso}`,
    )
  )
    return
  try {
...
async function resolverComentario(c) {
  ...
  try {
    const { data } = await api.patch(
      `/informes/${drawerInf.value.id}/comentarios/${c.id}/resolver`,
...
async function verificarYAprobar() {
  ...
  try {
    if (drawerInf.value.estado === 'borrador') {
      const { data } = await api.patch(`/informes/${drawerInf.value.id}/estado`, {
        estado: 'revisado',
      })
      const idx = informes.value.findIndex((i) => i.id === drawerInf.value.id)
      if (idx >= 0) Object.assign(informes.value[idx], data)
    ...
    const { data: d2 } = await api.patch(`/informes/${drawerInf.value.id}/estado`, {
      estado: 'aprobado',
    })
    const idx = informes.value.findIndex((i) => i.id === drawerInf.value.id)
...
async function reabrir() {
  if (!drawerInf.value) return
  if (!confirm('¿Reabrir este informe? Volverá a borrador y se podrán agregar nuevos comentarios.'))
    return
  try {
    const { data } = await api.patch(`/informes/${drawerInf.value.id}/estado`, {
      estado: 'borrador',
    ...
    toast(
      '⚠️ ' +
        ...
        ' — usa "Devolver a borrador" en el editor.',
      true,
    )
  }
}
...
.em-batch-inline:hover:not(:disabled) {
  background: #15803d;
}
.em-batch-inline:disabled {
  opacity: 0.6;
  cursor: not-allowed;
}
.em-batch-inline i {
...
.em-month-nav:hover:not(:disabled) {
  background: #e9defc;
}
.em-month-nav:disabled {
  opacity: 0.35;
  cursor: not-allowed;
}
.em-month-input {
...
.em-btn-send :deep(.p-button),
:deep(.em-btn-send) {
  background: #16a34a !important;
  border-color: #16a34a !important;
}
.em-btn-send :deep(.p-button:hover),
:deep(.em-btn-send:hover) {
...
.em-icon-btn:hover:not(:disabled) {
  background: #f4f1fa;
  border-color: #c7a8f0;
}
.em-icon-btn:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}
.em-btn-edit {
...
.em-btn-verify:hover:not(:disabled) {
  background: #f0fdf4;
}
.em-btn-send {
  color: #2563eb;
...
.em-btn-send:hover:not(:disabled) {
  background: #eff6ff;
}
.em-btn-del {
  color: #dc2626;
...
.em-btn-coms-on:hover:not(:disabled) {
  background: #fee2e2;
}
.em-btn-coms-resolved {
  color: #2563eb;
...
@media (min-width: 1024px) {
  ...
  .em-content--split .em-detail-backdrop {
    display: none;
  }
...
.em-btn-resolver:hover:not(:disabled) {
  background: #15803d;
  border-color: #15803d;
}
.em-btn-borrar {
...
.em-btn-borrar:hover:not(:disabled) {
  background: #fef2f2;
}
.em-btn-ghost {
  background: #fff;
...
.em-btn-verify-big :deep(.p-button),
:deep(.em-btn-verify-big) {
  background: #16a34a !important;
  border-color: #16a34a !important;
  font-weight: 800 !important;
}
.em-btn-verify-big :deep(.p-button:hover),
:deep(.em-btn-verify-big:hover) {
  background: #15803d !important;
}
.em-verify-hint {
  font-size: 11px;
