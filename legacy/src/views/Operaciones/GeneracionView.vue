<template>
  <div class="gen-page">
    ...
    <div class="gen-sticky-header">
      ...
      <div class="gen-titlebar">
        <div class="flex items-center gap-2.5">
          <div
            ...
            style="background: rgba(145, 91, 216, 0.12)"
          >
          ...
          <span class="hidden text-xs text-gray-500 sm:inline"
            >· Compara la energía generada por proyecto</span
          >
        </div>
      ...
      <div class="gen-filterbar">
        ...
        <Select
          ...
          class="w-28"
          size="small"
          @update:modelValue="aplicarModo"
        />
        <!-- Mensual · Intervalo de meses (un solo picker de rango) -->
        <DatePicker
          v-else-if="granularidad === 'mensual' && modo === 'intervalo'"
          v-model="rango"
          selectionMode="range"
          view="month"
          dateFormat="mm/yy"
        ...
        <DatePicker
          v-else-if="granularidad === 'diaria' && modo === 'intervalo'"
          v-model="rango"
          selectionMode="range"
          :numberOfMonths="2"
          dateFormat="dd/mm/yy"
        ...
        <DatePicker
          v-else-if="granularidad === 'horaria' && modo === 'intervalo'"
          v-model="rango"
          selectionMode="range"
          :numberOfMonths="2"
          dateFormat="dd/mm/yy"
        ...
        <Button
          ...
          size="small"
          class="gen-consultar"
          :class="{ 'gen-consultar--pendiente': pendiente && proyectosSel.length && !rangoError }"
      ...
      <div v-if="proyectos.length" class="gen-datahint">
        <i class="pi pi-bolt" />
        <span
          >{{ proyectos.length }} proyectos disponibles · generación en vivo desde la API de
          Unergy</span
        >
      </div>
    </div>
    ...
    <div v-if="!proyectosSel.length" class="gen-empty">
      <i class="pi pi-info-circle mb-3 text-3xl" style="color: #915bd8" />
    ...
    <div v-else-if="loading" class="gen-loading">
      ...
      <p class="mt-3 text-sm text-gray-500">Cargando datos de generación…</p>
    </div>
    ...
    <div v-else-if="error" class="gen-error">
      ...
      <div class="flex-1">
        ...
        <p class="mt-0.5 text-sm text-gray-600">{{ error }}</p>
      </div>
    ...
    <div v-else-if="!hasQueried" class="gen-empty">
      <i class="pi pi-search mb-3 text-3xl" style="color: #915bd8" />
    ...
    <div v-else-if="!datasets.length || datasets.every((d) => !d.points.length)" class="gen-empty">
      <i class="pi pi-database mb-3 text-3xl text-gray-300" />
      <p class="text-base font-semibold text-gray-700">Sin datos para el rango seleccionado</p>
      <p class="mt-1 text-sm text-gray-500">
        Los proyectos seleccionados no tienen generación registrada en este intervalo. Prueba con un
        rango más amplio o fechas anteriores.
      </p>
      <Button
        label="Ver el año en curso (mensual)"
        icon="pi pi-calendar"
        outlined
        size="small"
        class="mt-3"
        @click="verEsteAnioMensual"
      />
    </div>
    ...
    <template v-else>
      ...
      <div class="gen-kpis">
        <div class="gen-kpi">
          <div class="gen-kpi-icon" style="background: rgba(212, 160, 23, 0.12); color: #d4a017">
            <i class="pi pi-bolt" />
          </div>
          <div>
        ...
        <div class="gen-kpi">
          <div class="gen-kpi-icon" style="background: rgba(145, 91, 216, 0.12); color: #915bd8">
            <i class="pi pi-chart-bar" />
          </div>
          <div>
            <div class="gen-kpi-val">
              {{
        ...
        <div class="gen-kpi">
          <div class="gen-kpi-icon" style="background: rgba(16, 185, 129, 0.12); color: #10b981">
        ...
        <div class="gen-kpi">
          <div class="gen-kpi-icon" style="background: rgba(59, 130, 246, 0.12); color: #3b82f6">
      ...
      <section class="gen-card">
        <header class="gen-card-head">
          ...
          <div class="ml-auto flex items-center gap-2">
            <button
              ...
              :class="{ 'gen-toggle-btn--active': tipoGrafico === t }"
              @click="tipoGrafico = t"
            >
        ...
        <div class="gen-legend">
          <button
            ...
            :key="ds.proyectoId"
            class="gen-legend-item"
            :class="{ 'gen-legend-item--off': ds.hidden }"
        ...
        <div class="gen-chart-wrap" ref="chartWrapRef">
          ...
          <div
            v-if="hover"
            class="gen-tooltip"
            :class="{ 'gen-tooltip--flip': hover.flip }"
      ...
      <section class="gen-card">
        <header class="gen-card-head">
          ...
          <div class="ml-auto flex flex-wrap items-center gap-1.5">
            ...
            <span v-if="fallasGenCount" class="gen-fchip gen-fchip--red"
              >{{ fallasGenCount }} afectan generación</span
            >
        ...
        <div v-else class="overflow-x-auto">
          <DataTable
            :value="fallasDelPeriodo"
            stripedRows
            rowHover
            class="gen-table text-sm"
            :rows="10"
            paginator
            :rowsPerPageOptions="[10, 20, 50]"
            :alwaysShowPaginator="fallasDelPeriodo.length > 10"
            :rowClass="fallaRowClass"
          ...
          >
            ...
            >
              <template #body="{ data }">
                <div
                  class="gen-falla-stripe"
                  :class="{ 'gen-falla-stripe--gen': involucraGeneracion(data) }"
                />
              </template>
            </Column>
            <Column header="Fecha" field="fecha_identificacion" sortable style="width: 96px">
              <template #body="{ data }">
                <span class="font-medium text-gray-800">{{
                  fmtFechaCorta(data.fecha_identificacion)
                }}</span>
              </template>
            </Column>
            ...
            <Column header="Falla" style="min-width: 240px">
              <template #body="{ data }">
                ...
                <div class="gen-falla-desc text-xs text-gray-500">{{ data.descripcion }}</div>
              </template>
            </Column>
            <Column header="Prioridad" style="width: 96px">
              <template #body="{ data }">
                <span class="gen-prio-pill" :style="prioPillStyle(data.prioridad?.codigo)">{{
            ...
            <Column header="Estado" style="width: 120px">
              <template #body="{ data }">
                <Tag
                  :value="data.estado?.etiqueta || '—'"
                  :style="estadoPillStyle(data.estado?.color_hex)"
                />
              </template>
            </Column>
...
.gen-segmented--modo .gen-seg-btn--active {
  color: #2563eb;
}
...
.gen-project-picker :deep(.p-multiselect) {
  width: 100%;
}
...
@keyframes gen-pulse {
  0%,
  100% {
    box-shadow: 0 0 0 0 rgba(145, 91, 216, 0.45);
  }
  50% {
    box-shadow: 0 0 0 4px rgba(145, 91, 216, 0.18);
  }
}
...
.gen-table :deep(.p-datatable-tbody > tr) {
  cursor: pointer;
}
...
.gen-falla-stripe--gen {
  background: #dc2626;
}
.gen-falla-row--gen :deep(td) {
  background: #fff7f7;
}
:deep(.gen-falla-row--gen:hover td) {
