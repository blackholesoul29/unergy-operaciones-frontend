<template>
  <div class="space-y-4" v-if="reg">
    ...
    <div class="overflow-hidden rounded-full" style="height: 10px; background: #ece7f2">
      <div
        :style="`height:100%;width:${Math.min(100, reg.avance_pct)}%;background:linear-gradient(90deg,#915BD8,#6E3FB8);transition:width .5s;`"
      ></div>
    </div>
    ...
    <div class="mapa-card">
      <div class="mapa-header">
        ...
        <div class="mapa-leyenda">
          ...
          <span
            ><i class="dot" style="background: #f3eefb; border: 2px solid #d9c9f0"></i
            >Pendiente</span
          >
          <span
            ><i class="dot" style="background: #fde8e8; border: 2px solid #e05252"></i
            >Bloqueada</span
          >
        </div>
      </div>
        ...
        <svg viewBox="0 0 1040 320" class="mapa-svg" preserveAspectRatio="xMidYMid meet">
          ...
          <g v-for="n in mapaNodos" :key="n.etapa" class="nodo" @click="seleccionar(n.etapa)">
            ...
            <circle
              ...
              :cy="n.y"
              r="26"
              :fill="fillNodo(n.status)"
              :stroke="strokeNodo(n.status)"
              stroke-width="3"
            />
            <text
              :x="n.x"
          ...
          <g>
            ...
            <text
              ...
              text-anchor="middle"
              class="nodo-label"
              style="fill: #2c2039; font-weight: 700"
            >
              Energización
            </text>
          </g>
      ...
      <div v-if="etapaSel" class="etapa-panel">
        <div class="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <div class="text-xs" style="color: #9b89b5">
              Estado:
              <span class="font-semibold" style="color: #6e3fb8">{{
          ...
          <div class="flex gap-2">
            ...
            />
            <Button
              v-if="seleccionada === 'ETAPA_6_FRONTERA' || seleccionada === 'ETAPA_7_REGISTRO_ASIC'"
              label="Equipos y documentos"
              icon="pi pi-box"
              size="small"
              text
              @click="activeTab = 3"
        ...
        <div class="mt-3 flex flex-wrap items-center gap-1">
          ...
          <span v-if="!siguientesEstados(etapaSel).length" class="text-xs" style="color: #9b89b5"
            >— sin transiciones disponibles</span
          >
        </div>

        <div class="mt-3">
          <div class="mb-1 text-xs font-semibold" style="color: #2c2039">Hitos de esta etapa</div>
          <div v-for="h in hitosDeEtapa(seleccionada)" :key="h.hito" class="hito-linea">
            <span
              class="font-mono font-semibold"
              :style="`color:${h.completado ? '#3fb984' : '#9b89b5'}`"
              >{{ h.completado ? '✓' : '○' }} {{ h.codigo }}</span
            >
            <span class="flex-1" style="color: #2c2039">{{ h.descripcion }}</span>
            <span style="color: #6e3fb8; font-weight: 600">{{ h.peso_pct }}%</span>
          </div>
          <div v-if="!hitosDeEtapa(seleccionada).length" class="text-xs" style="color: #9b89b5">
      ...
      <div v-if="reg.bloqueos?.length" class="bloqueos">
        ...
        <span v-for="b in reg.bloqueos" :key="b.etapa" style="color: #2c2039">
          {{ b.etiqueta }} ({{ b.motivo }});</span
        >
      </div>
    </div>
      ...
      <TabView v-model:activeIndex="activeTab">
        ...
        <TabPanel header="General">
          <div class="grid gap-3 p-1 md:grid-cols-3">
            <label class="text-xs" style="color: #2c2039"
              >N° expediente <InputText v-model="general.numero_expediente" class="w-full"
            /></label>
            <label class="text-xs" style="color: #2c2039"
              >ID requerimiento OR <InputText v-model="general.id_requerimiento_or" class="w-full"
            /></label>
            <label class="text-xs" style="color: #2c2039"
              >N° solicitud appweb
              <InputText v-model="general.numero_solicitud_appweb" class="w-full"
            /></label>
            <label class="text-xs" style="color: #2c2039"
              >Fecha conexión estimada
              <input
                type="date"
                v-model="general.fecha_conexion_estimada"
                class="w-full rounded border px-2 py-1"
                style="border-color: #e8e0f0"
            /></label>
            <label class="text-xs" style="color: #2c2039"
              >Vigencia CREG 174 / ámbito
              <input
                type="date"
            ...
            /></label>
            <label class="text-xs" style="color: #2c2039"
              >Tipo visita protecciones
              <Select
                v-model="general.tipo_visita_protecciones"
                :options="cat.tipos_visita"
                showClear
                placeholder="—"
                class="w-full"
            /></label>
            <label class="col-span-2 text-xs" style="color: #2c2039"
              >Punto de conexión (texto)
              <InputText v-model="general.punto_conexion_texto" class="w-full"
            /></label>
            <label class="col-span-3 text-xs" style="color: #2c2039"
              >Notas <Textarea v-model="general.notas" rows="2" class="w-full"
            /></label>
            <div class="col-span-3 flex gap-4">
              ...
              >
              <label class="flex items-center gap-2 text-sm" style="color: #2c2039">
                <Checkbox v-model="general.comercializador_es_or" :binary="true" /> Comercializador
                es el OR</label
              >
            </div>
          </div>
            ...
            <Column field="codigo" header="Hito" style="width: 80px">
              <template #body="{ data }">
                <span class="font-mono font-semibold"
                  >{{ data.completado ? '✓ ' : '' }}{{ data.codigo }}</span
                >
              </template>
            </Column>
            ...
            <Column header="Estado" style="width: 110px">
              <template #body="{ data }">
                <Tag
                  ...
                  :severity="data.completado ? 'success' : 'secondary'"
                  class="text-xs"
                />
              </template>
            </Column>
        ...
        <TabPanel header="Parámetros 9.3">
          <div class="grid gap-4 p-1 md:grid-cols-2">
            <div>
              <div class="mb-2 text-sm font-semibold" style="color: #2c2039">
                Parámetros técnicos
              </div>
              <div class="grid grid-cols-2 gap-2">
                <label v-for="f in campos93" :key="f.k" class="text-xs" style="color: #2c2039">
                  ...
                  <input
                    type="number"
                    step="any"
                    v-model.number="params[f.k]"
                    class="mt-0.5 w-full rounded border px-2 py-1"
                    style="border-color: #e8e0f0"
                  />
                </label>
              </div>
            ...
            <div>
              ...
              <div v-if="validacion && validacion.resultados.length" class="space-y-1">
                ...
                >
                  ...
                  <Tag
                    :value="r.severidad"
                    :severity="sevColor(r.severidad)"
                    class="text-xs"
                    :title="r.mensaje"
                  />
                </div>
              </div>
              <p v-else class="text-xs" style="color: #9b89b5">
          ...
          <div class="space-y-5 p-1">
            <div>
              ...
              <DataTable :value="equipos" class="text-sm" rowHover>
                <template #empty
                  ><div class="py-4 text-center text-xs" style="color: #9b89b5">
                    Sin equipos.
                  </div></template
                >
            ...
            <div>
              ...
              <DataTable :value="documentos" class="text-sm" rowHover>
                ...
                <Column header="" style="width: 90px">
                  <template #body="{ data }">
                    <a v-if="data.url_drive" :href="data.url_drive" target="_blank" class="mr-1"
                      ><i class="pi pi-external-link" style="color: #915bd8"
                    /></a>
                    <Button
                      icon="pi pi-trash"
                      text
          ...
          <div class="p-1">
            ...
            <div v-if="alertas.length" class="space-y-2">
              <div
                ...
                style="border-left: 3px solid #f6b73c; background: #fffbf0"
              >
        ...
        <TabPanel header="Correos">
          <div class="space-y-2 p-1">
            <div class="text-sm" style="color: #2c2039">
              Genera un borrador de correo tipo (se rellena con los datos del proyecto):
            </div>
            <div class="flex flex-wrap gap-2">
              <Button
                label="Firmas al OR (9.1/9.7)"
                size="small"
                outlined
                @click="generarCorreo('SOLICITUD_FIRMAS_OR')"
              />
              <Button
                label="Creación en MDC (XM)"
                size="small"
                outlined
                @click="generarCorreo('CREACION_MDC_XM')"
              />
              <Button
                label="Documentación (Solenium)"
                size="small"
                outlined
                @click="generarCorreo('DOC_FRONTERA_SOLENIUM')"
              />
            </div>
          </div>
        </TabPanel>
    ...
    <!-- Dialogo equipo -->
    <Dialog
      v-model:visible="equipoDialog"
      modal
      header="Nuevo equipo de frontera"
      :style="{ width: '30rem' }"
    >
      <div class="grid grid-cols-2 gap-3">
        <label class="col-span-2 text-xs" style="color: #2c2039"
          >Tipo *
          <Select
            v-model="equipoForm.tipo"
            :options="cat.tipos_equipo"
            placeholder="Tipo…"
            class="w-full"
          />
        </label>
        <label class="text-xs" style="color: #2c2039"
          >Marca<InputText v-model="equipoForm.marca" class="w-full"
        /></label>
        <label class="text-xs" style="color: #2c2039"
          >Modelo<InputText v-model="equipoForm.modelo" class="w-full"
        /></label>
        <label class="text-xs" style="color: #2c2039"
          >Serial<InputText v-model="equipoForm.serial" class="w-full"
        /></label>
        <label class="text-xs" style="color: #2c2039"
          >Venc. calibración<input
            type="date"
        ...
        /></label>
        <label class="text-xs" style="color: #2c2039"
          >Solicitud Solenium<input
            type="date"
            v-model="equipoForm.fecha_solicitud_solenium"
            class="w-full rounded border px-2 py-1"
            style="border-color: #e8e0f0"
        /></label>
        <label class="text-xs" style="color: #2c2039"
          >Envío al OR<input
            type="date"
            v-model="equipoForm.fecha_envio_or"
            class="w-full rounded border px-2 py-1"
            style="border-color: #e8e0f0"
        /></label>
      </div>
    ...
    <Dialog v-model:visible="docDialog" modal header="Nuevo documento" :style="{ width: '30rem' }">
      <div class="grid grid-cols-2 gap-3">
        ...
        <label class="text-xs" style="color: #2c2039"
          >Radicado<InputText v-model="docForm.radicado" class="w-full"
        /></label>
        <label class="text-xs" style="color: #2c2039"
          >Firmado por<InputText v-model="docForm.firmado_por" class="w-full"
        /></label>
        <label class="text-xs" style="color: #2c2039"
          >Emisión<input
            type="date"
            v-model="docForm.fecha_emision"
            class="w-full rounded border px-2 py-1"
            style="border-color: #e8e0f0"
        /></label>
        <label class="text-xs" style="color: #2c2039"
          >Vencimiento<input
            type="date"
            v-model="docForm.fecha_vencimiento"
            class="w-full rounded border px-2 py-1"
            style="border-color: #e8e0f0"
        /></label>
        <label class="col-span-2 text-xs" style="color: #2c2039"
          >Enlace Drive<InputText v-model="docForm.url_drive" class="w-full"
        /></label>
      </div>
      <template #footer>
    ...
    <!-- Dialogo correo -->
    <Dialog
      v-model:visible="correoDialog"
      modal
      header="Borrador de correo"
      :style="{ width: '42rem' }"
    >
      <div v-if="correo" class="space-y-2 text-sm">
        <div>
          <span class="text-xs font-semibold" style="color: #9b89b5">Para:</span>
          {{ (correo.para || []).join(', ') || '—' }}
        </div>
        <div>
          <span class="text-xs font-semibold" style="color: #9b89b5">CC:</span>
          {{ (correo.cc || []).join(', ') }}
        </div>
        <div>
          <span class="text-xs font-semibold" style="color: #9b89b5">Asunto:</span>
          {{ correo.asunto }}
        </div>
        <div v-if="correo.adjuntos?.length">
          <span class="text-xs font-semibold" style="color: #9b89b5">Adjuntos:</span>
...
const CAMPOS_GENERAL = [
  ...
  'fecha_visita_protecciones',
  'tipo_visita_protecciones',
  'exporta',
  'comercializador_es_or',
...
function fillNodo(s) {
  return { completa: '#915BD8', progreso: '#ffffff', pendiente: '#f3eefb', bloqueada: '#fde8e8' }[s]
}
function strokeNodo(s) {
  return { completa: '#6E3FB8', progreso: '#915BD8', pendiente: '#d9c9f0', bloqueada: '#e05252' }[s]
}
function textNodo(s) {
  return { completa: '#ffffff', progreso: '#6E3FB8', pendiente: '#9b89b5', bloqueada: '#b91c1c' }[s]
}
...
function sevColor(s) {
  return (
    { OK: 'success', ERROR: 'danger', ADVERTENCIA: 'warn', PENDIENTE: 'secondary' }[s] ||
    'secondary'
  )
}
...
async function hacerTransicion(etapa, aEstado) {
  ...
  try {
    const { data } = await api.post(`/registros-cnd/${regId.value}/transicion`, {
      etapa,
      a_estado: aEstado,
    })
...
async function guardarParams() {
  ...
  try {
    ...
    for (const f of campos93)
      if (params.value[f.k] !== undefined && params.value[f.k] !== '')
        payload[f.k] = params.value[f.k]
...
async function recomputar() {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: `Alertas: ${data.alertas.length} (${data.creadas} nuevas)`,
      life: 3000,
    })
  } catch (e) {
    toast.add({
      severity: 'error',
      summary: 'Error al recomputar',
      detail: e.response?.data?.detail ?? '',
      life: 5000,
    })
  } finally {
...
async function crearEquipo() {
  try {
    const payload = Object.fromEntries(
      Object.entries(equipoForm.value).filter(([, v]) => v !== '' && v != null),
    )
...
function abrirDoc() {
  docForm.value = { estado: 'BORRADOR' }
  docDialog.value = true
}
async function crearDoc() {
  try {
    const payload = Object.fromEntries(
      Object.entries(docForm.value).filter(([, v]) => v !== '' && v != null),
    )
...
onMounted(async () => {
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'No se pudo abrir el proyecto',
      detail: e.response?.data?.detail ?? '',
      life: 6000,
    })
  }
...
.mapa-card {
  ...
  box-shadow: 0 1px 3px rgba(44, 32, 57, 0.06);
}
.mapa-header {
  display: flex;
  align-items: center;
...
.mapa-leyenda .dot.ring {
  background: #fff;
  border: 2px solid #915bd8;
}
.mapa-wrap {
  overflow-x: auto;
}
