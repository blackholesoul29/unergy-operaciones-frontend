<template>
  <div class="space-y-5">
    ...
    >
      <h2 class="mb-3 text-sm font-bold" style="color: #2c2039">Resultado del Fix</h2>
      <div
        v-for="(a, i) in fixResult.actions"
        :key="i"
        class="border-b py-1 text-xs"
        style="border-color: #f0e8f8"
      >
        <Tag :value="a.action" :severity="actionSev(a.action)" class="mr-2 text-xs" />
        <b>{{ a.contrato }}</b>
        <span v-if="a.planta"> → {{ a.planta }}</span>
        <span v-if="a.sub_project" class="ml-1 font-mono" style="color: #059669"
          >({{ a.sub_project }})</span
        >
    ...
    <template v-if="data && !loading">
      ...
      <div class="rounded-xl bg-white p-4 shadow-sm" style="border: 1px solid #e8e0f0">
        <h2 class="mb-3 text-sm font-bold" style="color: #2c2039">
      ...
      >
        <div
          ...
          :style="
            'border-bottom: 1px solid #e8e0f0; background: ' +
            (c.n_plantas_activas === 0 ? '#fef2f2' : '#f8f5fc')
          "
        >
          <span class="text-sm font-bold" style="color: #2c2039">{{
            c.nombre_interno || '(sin nombre)'
          }}</span>
          <span
          ...
          <Tag
            :value="c.tipo"
            :severity="c.tipo === 'compra' ? 'info' : 'secondary'"
            class="text-xs"
          />
          <span
        ...
        <div v-if="c.gescon_raw?.length" class="px-4 py-3">
          <p class="mb-2 text-xs font-bold tracking-wide uppercase" style="color: #6b5a8a">
              ...
              <tbody>
                ...
                >
                  ...
                  <td
                    ...
                    style="border: 1px solid #e8e0f0; color: #2c2039"
                  >
                    {{ r.planta || '—' }}
                  </td>
                  <td
                    ...
                    style="border: 1px solid #e8e0f0"
                    :style="
                      r.sub_project ? 'color: #059669;' : 'color: #dc2626; font-weight: bold;'
                    "
                  >
                    {{ r.sub_project || 'NULL' }}
                  </td>
        ...
        <!-- Resolved plants -->
        <div
          v-if="c.gescon_resolved?.length"
          class="px-4 py-3"
          style="border-top: 1px solid #e8e0f0"
        >
          <p class="mb-2 text-xs font-bold tracking-wide uppercase" style="color: #059669">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            >
              {{ p.planta }}
              <span class="ml-1 font-mono opacity-70">({{ p.sub_project || 'SIN API ID' }})</span>
              <span v-if="p.pct_despacho != null" class="ml-1 opacity-60"
        ...
        <!-- No plants warning -->
        <div
          v-if="!c.gescon_raw?.length && !c.gescon_resolved?.length"
          class="px-4 py-6 text-center"
        >
          <i class="pi pi-exclamation-triangle mb-2 block text-2xl" style="color: #dc2626" />
          <p class="text-xs font-semibold" style="color: #dc2626">
            Sin registros GESCON —
            {{
...
async function fixEnlaces() {
  ...
  try {
    ...
    fixResult.value = {
      actions: [{ action: 'error', reason: e.response?.data?.detail || e.message, contrato: '—' }],
    }
  } finally {
    fixing.value = false
  }
