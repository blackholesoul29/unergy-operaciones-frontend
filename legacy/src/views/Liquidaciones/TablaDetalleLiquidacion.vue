<template>
  <table class="w-full border-t border-gray-200 text-xs">
    <thead>
      <tr class="bg-gray-100 text-gray-600">
        <th class="w-44 px-3 py-1.5 text-left">Inversionista</th>
        <th class="w-24 px-3 py-1.5 text-left">Doc. contable</th>
        <th class="px-3 py-1.5 text-left">Concepto</th>
        <th class="w-36 px-3 py-1.5 text-right">Total</th>
        <th class="w-36 px-3 py-1.5 text-left">Ref. Factura / Soporte</th>
        <th class="w-24 px-3 py-1.5 text-left">Consecutivo</th>
      </tr>
    </thead>
    <tbody>
      <!-- Fila TOTAL del proyecto (solo si no es modo inversionista) -->
      <template v-if="!modoInversionista">
        <tr class="border-b border-indigo-200 bg-indigo-50 font-semibold">
          <td class="px-3 py-1.5 font-bold text-indigo-800" colspan="2">
            Total
            <span
              class="ml-2 rounded bg-indigo-200 px-1.5 py-0.5 text-[9px] font-semibold tracking-wide text-indigo-800"
            >
              100% PROYECTO
            </span>
          </td>
        ...
        <tr
          v-for="fila in filasTotal"
          :key="fila.key"
          :class="bgFila(fila.doc)"
        ...
        >
          ...
          <td
            ...
            :class="fila.negativo ? 'bg-red-50 text-red-600' : ''"
          >
            {{ fmt(fila.valor) }}
          </td>
          <td class="px-3 py-1.5">
            ...
            >
              <i class="pi pi-file-pdf text-xs text-red-500" />{{ fila.ref }}
            </a>
      ...
      <template v-for="inv in inversionistas" :key="inv.inversionista_id">
        <tr class="border-t-2 border-gray-600 bg-gray-800 text-white">
          <td class="max-w-[176px] truncate px-3 py-1.5 font-semibold" :title="inv.nombre">
            {{ inv.nombre }}
          </td>
