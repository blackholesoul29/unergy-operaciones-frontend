<template>
  <div class="overflow-hidden rounded-xl border bg-white shadow-sm" style="border-color: #e8e0f0">
    <div class="flex items-center gap-2 border-b px-3 py-2" style="border-color: #f0ebf6">
      ...
      <span v-if="mesesHist" class="ml-auto text-[10px]" style="color: #9b8fb0"
        >promedio de {{ mesesHist }} mes(es) anteriores</span
      >
    </div>

    <ProgressSpinner v-if="loading" style="width: 32px; height: 32px" class="mx-auto my-6 block" />
      ...
      <div class="grid grid-cols-2 gap-2 md:grid-cols-5" :class="showChart ? 'mb-3' : ''">
        <div v-for="k in kpis" :key="k.label" class="rounded-lg p-2" :style="{ background: k.bg }">
          ...
          <p
            ...
            :style="{ color: k.delta >= 0 ? '#10B981' : '#D64455' }"
          >
...
const chartOptions = {
  ...
  scales: {
    ...
    y: {
      ticks: { font: { size: 9 }, color: '#9ca3af', callback: (v) => fmtCompact(v) },
