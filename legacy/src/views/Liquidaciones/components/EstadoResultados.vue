<template>
  <div
    class="flex h-full flex-col overflow-hidden rounded-xl border bg-white shadow-sm"
  ...
  >
    <div class="flex items-center gap-2 border-b px-4 py-2.5" style="border-color: #f0ebf6">
      ...
      <span class="ml-auto text-[10px] font-semibold tracking-wide uppercase" style="color: #bba8d4"
        >Concepto · Soporte · Valor</span
      >
    </div>
    ...
    <div class="flex-1">
      <template v-for="g in grupos" :key="g.key">
        <!-- Encabezado de grupo -->
        <div
          class="flex items-center justify-between gap-3 border-t px-4 py-1.5"
          style="border-color: #f0ebf6; background: #faf7ff"
        >
          <span class="text-[11px] font-bold tracking-wide uppercase" style="color: #6e3fb8">{{
            g.label
          }}</span>
          <span
            class="font-mono font-bold whitespace-nowrap tabular-nums"
            :style="{ color: g.sign < 0 ? '#D64455' : '#2C2039' }"
          >
        ...
        <!-- Líneas del grupo -->
        <div
          v-for="(l, i) in g.lineas"
          :key="g.key + '_' + i"
          class="flex flex-wrap items-center gap-x-3 gap-y-1 border-t px-4 py-1.5"
          style="border-color: #f7f3fc"
        >
          <span class="min-w-[120px] flex-1 pl-3 text-xs" style="color: #5b5470">{{
            l.label
          }}</span>
          <a
            v-if="l.soporte_url"
            ...
            :title="l.referencia || 'Ver soporte'"
          >
            <i class="pi pi-paperclip text-[10px]" />{{ l.refCodigo || 'Soporte' }}
          </a>
          <span
            v-else-if="l.refCodigo"
            class="inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] font-medium"
            style="background: #f5f1fa; color: #9b8fb0"
            :title="l.referencia"
          >
            <i class="pi pi-hashtag text-[10px]" />{{ l.refCodigo }}
          </span>
          <span
            v-else-if="l.requiereSoporte"
            class="text-[10px] italic"
            style="color: #cdbfe2"
            title="Sin soporte adjunto"
            >— sin soporte</span
          >
          <span class="font-mono text-xs whitespace-nowrap tabular-nums" style="color: #6b5a8a">{{
            fmtCOP(l.valor)
          }}</span>
        </div>
      </template>
    ...
    <!-- INGRESO NETO -->
    <div
      class="flex items-center justify-between gap-3 border-t-2 px-4 py-2.5"
    ...
    >
      ...
      <span
        class="font-mono text-base font-extrabold whitespace-nowrap tabular-nums"
        style="color: #915bd8"
        >{{ fmtCOP(neto) }}</span
      >
    </div>

    <p class="border-t px-4 py-2 text-[10px] italic" style="color: #9b8fb0; border-color: #f0ebf6">
      Neto = valor a pagar (mandato de ingresos) − mandato de costos − facturas de servicio (CGM,
