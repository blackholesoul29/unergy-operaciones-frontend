<template>
  <div class="liqpdf-wrapper">
    ...
    <transition name="fade">
      <div v-if="toastMsg" :class="['liqpdf-toast', toastErr ? 'is-err' : 'is-ok']">
        {{ toastMsg }}
      </div>
    </transition>
    ...
    <div class="liqpdf-toolbar">
      <div class="flex items-center gap-2">
        ...
        <div>
          <div class="text-sm font-bold" style="color: #2c2039">
            Informe PDF — Estado de Resultados
          </div>
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <Button
          ...
          icon="pi pi-file-pdf"
          size="small"
          style="background: #f6ff72; border-color: #f6ff72; color: #2c2039"
          @click="descargar"
        />
        <Button
    ...
    <div v-if="editMode" class="liqpdf-hint">
      ✏️ Haz clic en cualquier texto para editarlo. Al terminar pulsa <b>Guardar</b>. El PDF respeta
      saltos de página: las capas no se cortan y el pie va siempre al final de cada hoja.
    </div>
...
const _cop2 = new Intl.NumberFormat('es-CO', {
  style: 'currency',
  currency: 'COP',
...
function volver() {
  ...
  if (editMode.value && !window.confirm('Tienes cambios sin guardar. ¿Salir de todos modos?'))
    return
...
async function loadTarifas() {
  ...
  try {
    const { data } = await api.get('/contratos-servicio', {
      params: { proyecto_id: liq.value.proyecto_id },
    })
...
async function loadComparativo() {
  ...
  try {
    ...
    for (const entry of data.periodos || []) {
      ...
      if (proy)
        byMes[entry.periodo] = {
          ingresos: proy.ingresos_cop,
          costosOp: proy.costos_cop,
          facturas: 0,
          neto: proy.valor_a_pagar_total,
        }
    }
  ...
  try {
    ...
    if (data?.proyecto_id) {
      ...
      try {
        ...
        if (per) {
          const { data: rp } = await api.get('/liquidaciones/resumen-panel', {
...
async function guardar() {
  ...
  try {
    ...
    const { data } = await api.put(`/liquidaciones/${route.params.id}/informe`, {
...
function descargar() {
  ...
  if (!w) {
    toast('Permite las ventanas emergentes para descargar el PDF', true)
    return
  }
...
.liqpdf-toolbar {
  ...
  gap: 10px;
  flex-wrap: wrap;
  box-shadow: 0 2px 10px rgba(44, 32, 57, 0.04);
  margin-bottom: 12px;
}
.liqpdf-sel {
  max-width: 260px;
...
.liqpdf-canvas .liq-doc {
  ...
  border: 1px solid #e8e0f0;
  border-radius: 10px;
  box-shadow: 0 6px 30px rgba(44, 32, 57, 0.1);
  padding: 18mm 14mm;
  min-height: 60vh;
}
.liq-doc[contenteditable='true'] {
  outline: 2px dashed rgba(145, 91, 216, 0.35);
  outline-offset: 4px;
}
.liq-doc :focus {
  outline: none;
