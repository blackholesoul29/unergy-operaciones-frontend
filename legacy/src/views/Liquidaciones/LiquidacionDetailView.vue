<template>
  <div class="space-y-3 p-3" style="background: #fdfaf7; min-height: 100vh">
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <Tag
          v-if="liq"
          :value="liq.estado"
          :severity="estadoSeverity(liq.estado)"
          class="text-xs"
        />
      </div>
      <div class="ml-auto flex flex-wrap items-center gap-2">
        <a
          ...
          target="_blank"
          class="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-semibold transition-opacity hover:opacity-80"
    ...
    <ProgressSpinner v-if="loading" class="mx-auto block" />
    ...
    <template v-if="!loading && liq">
      ...
      >
        ...
        <button
          class="ml-auto flex shrink-0 items-center gap-1.5 text-xs font-semibold hover:opacity-70"
          style="color: #915bd8"
      ...
      <div
        class="flex flex-wrap gap-x-4 gap-y-1 rounded-lg border bg-white px-3 py-2 text-[11px] shadow-sm"
        style="color: #2c2039; border-color: #e8e0f0"
      >
        <span
        ...
        >
        <span
          ><span class="text-gray-400">Consec. Ingresos:</span>
          <strong class="ml-1">{{ liq.consecutivo_inicial_ingresos ?? '—' }}</strong></span
        >
        <span
          ><span class="text-gray-400">Consec. Costos:</span>
          <strong class="ml-1">{{ liq.consecutivo_inicial_costos ?? '—' }}</strong></span
        >
        <span
          ><span class="text-gray-400">Tasa cambio:</span>
    ...
    <Dialog
      ...
      header="Editar resumen financiero"
      modal
    ...
    >
      <div class="grid grid-cols-2 gap-3 py-2">
        ...
        <div class="flex flex-col gap-1">
          ...
          <DatePicker
            v-model="resumenForm.fecha_inicio_proceso"
            dateFormat="yy-mm-dd"
            class="w-full"
          />
        </div>
        ...
        <div class="flex flex-col gap-1">
          ...
          <InputText
            v-model="resumenForm.estado_resultados_url"
            class="w-full"
            placeholder="https://..."
          />
        </div>
...
async function load() {
  ...
  if (liq.value?.proyecto_id) {
    try {
      ...
      console.error(
        '[LiquidacionDetail] Error cargando inversionistas:',
        e?.response?.status,
        e?.response?.data ?? e,
      )
    }
    ...
    try {
      ...
      console.error(
        '[LiquidacionDetail] Error cargando Panel ER:',
        e?.response?.status,
        e?.response?.data ?? e,
      )
      panelER.value = null
