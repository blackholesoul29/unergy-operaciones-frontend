<template>
  <div class="fac space-y-4 p-4 sm:p-5">
    ...
    <template v-else>
      ...
      <template v-if="sub === 'facturacion'">
        ...
        <div v-if="noFacturables.length" class="fac-card">
          <p class="fac-note">
            <i class="pi pi-info-circle" /> No facturables por esta vía ({{
      ...
      <template v-else-if="sub === 'facturas'">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <p class="flex-1 text-[11px]" style="color: #9b8fb0; min-width: 260px">
          ...
          <div class="flex shrink-0 items-center gap-2">
            ...
            <button
              ...
              @click="guardarOrden"
            >
              <i :class="guardandoOrden ? 'pi pi-spin pi-spinner' : 'pi pi-save'" class="text-xs" />
              Guardar orden
            </button>
        ...
        >
          <div class="fac-fac-head" @click="toggleFac(f.factura)">
            ...
            />
            <i
              :class="abiertas.has(f.factura) ? 'pi pi-chevron-down' : 'pi pi-chevron-right'"
              class="text-xs"
              style="color: #9b8fb0"
            />
            <span class="proj">{{ f.factura }}</span>
            <span v-if="f.sin_ppa" class="tag" style="background: #fbeede; color: #c9701a"
              >sin PPA · XM (bolsa)</span
            >
            <span
              v-else-if="f.personalizada"
              class="tag"
              style="background: #e6f6ef; color: #1f9d6b"
              >dividida</span
            >
            ...
            <span
              v-else-if="f.emitida"
              class="tag"
              style="background: #e6f6ef; color: #1f9d6b; cursor: pointer"
              @click.stop="abrirNumero(f)"
              v-tooltip.top="'Agregar el N° de factura'"
              >facturada · N°?</span
            >
            <span class="fac-acts" @click.stop>
              <button
                ...
                v-tooltip.top="'Copiar el mensaje'"
              >
              ...
              <button
                ...
                v-tooltip.top="'Copiar como imagen'"
              >
          ...
          <div v-if="abiertas.has(f.factura)" class="fac-fac-body">
            ...
            <div class="fac-div-row">
              ...
              >
                <i
                  :class="guardandoDiv ? 'pi pi-spin pi-spinner' : 'pi pi-arrow-right'"
                  class="text-xs"
                />
                Mover seleccionados
              </button>
      ...
      <template v-else-if="sub === 'cumplimiento'">
        ...
        <div class="fac-kpis">
          ...
          <div class="fac-kpi">
            ...
            <p class="v" :style="{ color: cumpl.resumen.bajo_minimo || 0 ? '#c0392b' : undefined }">
          ...
          <div class="fac-kpi">
            ...
            <p
              class="v"
              :style="{ color: cumpl.resumen.faltante_kwh || 0 ? '#c0392b' : undefined }"
            >
        ...
        <div class="fac-card">
            ...
            <table class="dt">
              ...
              <tbody>
                <tr v-for="f in cumpl.filas" :key="f.ppa || f.numero_contrato">
                  ...
                  <td
                    :style="{
                      color: f.estado === 'bajo_minimo' ? '#c0392b' : '#2C2039',
                      fontWeight: 600,
                    }"
                  >
                    {{ f.pct != null ? f.pct + '%' : '—' }}
                  ...
                  <td
                    :style="{
                      color: f.faltante_kwh > 0 ? '#c0392b' : '#9b8fb0',
                      fontWeight: f.faltante_kwh > 0 ? 600 : 400,
                    }"
                  >
                    {{ f.faltante_kwh > 0 ? fmtNum(f.faltante_kwh) : '—' }}
                  </td>
                  <td class="l">
                    <span class="tag" :style="cumplEstiloEstado(f.estado)">{{
                      MOTIVOS_CUMPL[f.estado] || f.estado
                    }}</span>
                    <span
                      v-if="f.unidad_sospechosa"
                      class="tag"
                      style="background: #fdecea; color: #a13527"
          ...
          <p class="fac-note">
            <i class="pi pi-info-circle" /> Compromiso por contrato marco (PPA), en MWh; despacho
          ...
          <div class="fac-desp-filtro">
            <span class="p-input-icon-left" style="position: relative">
              ...
              <input
                v-model="despFiltro"
                class="fac-in"
                style="width: 280px; padding-left: 30px"
                ...
                <template v-for="d in despachoFiltrado" :key="d.contrato">
                  ...
                  <tr v-if="diasAbiertos.has(d.contrato)" class="fac-desp-dias">
                    ...
                    <td class="l" colspan="4">
                      <div v-if="dias[d.contrato] === 'loading'" class="muted py-1 text-[11px]">
                        <i class="pi pi-spin pi-spinner text-[10px]" /> Cargando días…
                      </div>
                      <div
                        v-else-if="!dias[d.contrato] || !dias[d.contrato].length"
      ...
      <template v-else-if="sub === 'ipp'">
        <div class="fac-card p-4">
          ...
          <p class="mb-3 text-[11px]" style="color: #9b8fb0">
          ...
          <div class="flex flex-wrap items-end gap-2">
            ...
            <button class="fac-btn-sec" :disabled="trayendoDane" @click="traerIppDane">
              <i
                :class="trayendoDane ? 'pi pi-spin pi-spinner' : 'pi pi-cloud-download'"
                class="text-xs"
              />
              Traer del DANE
            </button>
            <button class="fac-btn" :disabled="guardandoIpp || !ippInput" @click="guardarIpp">
              <i :class="guardandoIpp ? 'pi pi-spin pi-spinner' : 'pi pi-save'" class="text-xs" />
              Guardar
            </button>
            <span v-if="ippActual" class="ml-1 text-[11px]" style="color: #2c7a3f"
          ...
          <div
            v-if="ippDane != null && ippActual != null && Number(ippActual) !== Number(ippDane)"
            class="mt-3 flex items-center gap-2 rounded-lg px-3 py-2 text-[11px]"
            style="background: #fff8e6; border: 1px solid #f5e3b3; color: #7a5c00"
          >
            <i class="pi pi-exclamation-triangle" />
            Lo guardado (<b>{{ ippActual }}</b
            >) no coincide con lo que reporta el DANE (<b>{{ ippDane }}</b
            >). Las tarifas de este período están indexadas con el guardado.
          </div>
        </div>
        ...
        <div class="fac-card p-4">
          ...
          <div class="flex flex-wrap items-end gap-2">
            ...
            <button class="fac-btn" :disabled="guardandoBolsa" @click="guardarBolsa">
              <i :class="guardandoBolsa ? 'pi pi-spin pi-spinner' : 'pi pi-save'" class="text-xs" />
              Guardar
            </button>
            <span
              class="ml-1 text-[11px]"
              :style="{ color: bolsa.vigente != null ? '#2C7a3f' : '#c0392b' }"
            >
              {{
                bolsa.vigente != null
    ...
    <Dialog
      v-model:visible="numModal.open"
      modal
      :closable="!numModal.saving"
      :draggable="false"
      :style="{ width: '380px' }"
      :header="numModal.modo === 'editar' ? 'N° de factura' : 'Marcar como facturada'"
    >
      <div class="space-y-3">
        <p class="text-xs" style="color: #6b5a8a">
          <b>{{ numModal.factura }}</b
          ><br />
      ...
      <template #footer>
        <button class="fac-link" :disabled="numModal.saving" @click="numModal.open = false">
          Cancelar
        </button>
...
async function load() {
  ...
  try {
    const [fac, desp, ipp, blz, cmp] = await Promise.all([
      api
        .get('/facturacion', { params: { periodo: per.value } })
        .then((r) => r.data)
        .catch(() => ({})),
      api
        .get('/facturacion/despacho', { params: { periodo: per.value } })
        .then((r) => r.data)
        .catch(() => ({ contratos: [] })),
      api
        .get('/ppa/ipp/mensual')
        ...
        .catch(() => []),
      api
        .get('/facturacion/bolsa', { params: { periodo: per.value } })
        .then((r) => r.data)
        .catch(() => ({})),
      api
        .get('/facturacion/cumplimiento', { params: { periodo: per.value } })
        .then((r) => r.data)
        .catch(() => ({ resumen: {}, filas: [] })),
    ])
    ...
    diasAbiertos.clear()
    for (const k in dias) delete dias[k] // el día a día es por mes
...
async function subirDespacho(file) {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Despacho cargado',
      detail: `${data.contratos} contratos · ${(data.kwh_total / 1000).toFixed(0)} MWh`,
      life: 4000,
    })
    await load()
  } catch (e) {
...
function toggleFac(k) {
  abiertas.has(k) ? abiertas.delete(k) : abiertas.add(k)
}
function selDe(k) {
  if (!sel[k]) sel[k] = reactive(new Set())
  return sel[k]
}
function toggleProy(k, contrato) {
  const s = selDe(k)
...
async function moverSeleccionados(k) {
  ...
  if (!s.size) {
    toast.add({ severity: 'warn', summary: 'Selecciona contratos', life: 3000 })
    return
  }
  if (!nombre) {
    toast.add({ severity: 'warn', summary: 'Escribe el nombre de la factura', life: 3000 })
    return
  }
  ...
  if (crudo) {
    ...
    if (!Number.isFinite(pct) || pct <= 0 || pct > 100) {
      toast.add({
        ...
        detail: 'Debe ser un número entre 0 y 100.',
        life: 4000,
      })
      return
    }
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Factura dividida',
      detail: `${rows.length} contratos → "${nombre}"${comoPct}`,
      life: 3500,
    })
    s.clear()
    nuevoNombre[k] = ''
...
async function guardarOrden() {
  ...
  try {
    ...
    toast.add({
      ...
      detail: 'Se aplica también a los próximos meses.',
      life: 3500,
    })
  } catch (e) {
    toast.add({
...
async function restablecerOrden() {
  try {
    ...
    toast.add({
      severity: 'success',
      summary: 'Orden restablecido',
      detail: 'Vuelve a ordenarse por valor.',
      life: 3000,
    })
  } catch (e) {
    toast.add({
...
async function confirmarNumero() {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: yaEmitida ? 'N° actualizado' : 'Factura marcada',
      detail: num || 'Sin N°',
      life: 2500,
    })
  } catch (e) {
...
async function toggleDias(contrato) {
  ...
  try {
    const { data } = await api.get('/facturacion/despacho/dias', {
      params: { periodo: per.value, contrato },
    })
...
async function copiarMensaje(f) {
  ...
  if (!texto) {
    toast.add({ severity: 'warn', summary: 'Sin datos para el mensaje', life: 3000 })
    return
  }
  try {
  ...
  if (f.tarifa_mixta) {
    toast.add({
      severity: 'warn',
      summary: 'Copiado — revisa la tarifa',
      life: 5000,
...
async function copiarImagen(f) {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'No se pudo generar la imagen',
      detail: e.message,
      life: 5000,
    })
    return
  ...
  canvas.toBlob(async (blob) => {
    ...
    try {
      ...
      toast.add({
        severity: 'info',
        summary: 'Imagen descargada',
        detail: 'El navegador no permite copiarla al portapapeles.',
        life: 4000,
      })
    }
  }, 'image/png')
...
async function traerIppDane() {
  ...
  try {
    ...
    toast.add({
      severity: 'success',
      summary: `IPP del DANE — ${formatPeriodo(props.periodo)}`,
      detail: `${valor}. Revísalo y dale Guardar.`,
      life: 5000,
    })
  } catch (e) {
    toast.add({
      severity: 'warn',
      summary: 'El DANE no tiene ese período',
      detail:
...
async function guardarIpp() {
  ...
  try {
    await api.put('/ppa/ipp/mensual', [
      { año: añoMes.value.a, mes: añoMes.value.m, valor: Number(ippInput.value) },
    ])
    toast.add({
      severity: 'success',
      summary: 'IPP guardado',
      detail: `${formatPeriodo(props.periodo)} = ${ippInput.value}`,
      life: 3500,
    })
    await load()
  } catch (e) {
...
async function guardarBolsa() {
  ...
  try {
    await api.put('/facturacion/bolsa', {
      periodo: per.value,
      valor: bolsaInput.value ? Number(bolsaInput.value) : null,
    })
...
.fac-btn-sec:hover:not(:disabled) {
  background: #f6f1fc;
}
.fac-btn-sec:disabled {
  opacity: 0.6;
  cursor: default;
}
.fac-lbl {
...
.fac-drop-antes {
  box-shadow: inset 0 3px 0 0 #915bd8;
}
.fac-drop-despues {
  box-shadow: inset 0 -3px 0 0 #915bd8;
}
