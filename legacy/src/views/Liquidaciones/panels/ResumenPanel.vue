  ...
  <div class="space-y-4 p-4 sm:p-5">
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">
        ...
        >
          <div class="min-w-0">
            ...
            <p class="mt-1 truncate text-xl font-bold" style="color: #2c2039">{{ kpi.value }}</p>
            <p
              v-if="kpi.sub"
              class="mt-0.5 text-[11px]"
              :style="{ color: kpi.subColor || '#915BD8' }"
            >
              {{ kpi.sub }}
            </p>
          </div>
      ...
      <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div class="rounded-xl border bg-white p-4 shadow-sm" style="border-color: #e8e0f0">
          <h3 class="mb-3 text-sm font-bold" style="color: #2c2039">
            Ingresos por tipo de proyecto
          </h3>
          <div v-if="porTipo.length" class="space-y-2.5">
            <div v-for="t in porTipo" :key="t.tipo">
              <div class="mb-1 flex justify-between text-xs" style="color: #2c2039">
                <span class="font-medium capitalize">{{ t.tipo }}</span>
                <span class="font-mono" style="color: #6b5a8a"
              ...
              <div class="h-2.5 overflow-hidden rounded-full bg-gray-100">
                <div
                  ...
                  :style="{ width: barPct(t.ingresos) + '%', background: '#915BD8' }"
                />
              </div>
            </div>
          </div>
          <p v-else class="py-4 text-center text-xs text-gray-400">Sin paneles en el período.</p>
        </div>

        <!-- ── Pipeline (firmado / pendiente) ─────────────────────── -->
        <div class="rounded-xl border bg-white p-4 shadow-sm" style="border-color: #e8e0f0">
          <h3 class="mb-3 text-sm font-bold" style="color: #2c2039">Estado del período</h3>
      ...
      <div class="rounded-xl border bg-white p-4 shadow-sm" style="border-color: #e8e0f0">
        ...
        <div style="height: 240px">
          ...
          <p v-else class="py-8 text-center text-xs text-gray-400">
      ...
      <div
        ...
        style="border-color: #e8e0f0"
      >
        <div class="flex items-center gap-2 border-b px-4 py-2.5" style="border-color: #f0ebf6">
        ...
        >
          ...
          <Column header="Tipo">
            <template #body="{ data }"
              ><span class="text-xs capitalize">{{ data.tipo_proyecto || '—' }}</span></template
            >
          </Column>
          <Column header="Estado">
            <template #body="{ data }">
              <Tag
                :value="estadoFlujoPanel(data, tipo).label"
                :severity="estadoFlujoPanel(data, tipo).sev"
                class="text-[10px]"
              />
            </template>
          </Column>
          <Column header="Ingresos" style="width: 130px">
            <template #body="{ data }"
              ><span class="font-mono text-xs">{{ fmtCompact(data.ingresos_cop) }}</span></template
            >
          </Column>
          <Column header="Costos" style="width: 120px">
            <template #body="{ data }"
              ><span class="font-mono text-xs text-red-600">{{
                fmtCompact(data.costos_cop)
              }}</span></template
            >
          </Column>
          <Column header="Valor a pagar" style="width: 140px">
            <template #body="{ data }"
              ><span class="font-mono text-xs font-semibold" style="color: #915bd8">{{
                fmtCompact(data.valor_a_pagar_total)
              }}</span></template
            >
          </Column>
          ...
          <template #expansion="{ data }">
            <div class="px-4 py-3" style="background: #faf8fd">
              <p class="mb-2 text-[11px] font-semibold" style="color: #6b5a8a">Por inversionista</p>
              <table class="w-full text-xs">
                ...
                <tbody>
                  <tr
                    v-for="inv in data.inversionistas"
                    :key="inv.proyecto_inversionista_id || inv.nombre"
                    class="border-t"
                    style="border-color: #f0ebf6"
                  >
                    <td class="py-1.5" style="color: #2c2039">
                      {{ inv.cliente_nombre || inv.nombre || '—' }}
                    </td>
                    <td class="py-1.5 font-mono" style="color: #6b5a8a">
                      {{ inv.porcentaje != null ? inv.porcentaje.toFixed(2) + '%' : '—' }}
                    </td>
                    <td class="py-1.5 text-right font-mono font-semibold" style="color: #915bd8">
