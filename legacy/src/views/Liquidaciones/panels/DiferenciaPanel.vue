  ...
  <div class="space-y-4 p-4 sm:p-5">
    ...
    <template v-else>
      ...
      <div
        ...
        class="rounded-lg px-3 py-3 text-center text-xs"
        style="background: #faf7ff; border: 1px solid #e3d5f5; color: #6b5a8a"
      >
        <i
          class="pi pi-clock"
          style="font-size: 20px; display: block; margin-bottom: 6px; color: #915bd8"
        />
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          >
            <p class="text-[11px] font-semibold tracking-wide uppercase" style="color: #6b5a8a">
              {{ k.label }}
            </p>
            <p class="mt-1 text-xl font-bold" :style="{ color: k.color }">
        ...
        >
          ...
          <div
            class="flex cursor-pointer items-center gap-2 border-b px-4 py-2.5 select-none hover:bg-gray-50"
            style="border-color: #f0ebf6"
            @click="toggle(proy.proyecto_id)"
          >
            <i
              :class="abiertos.has(proy.proyecto_id) ? 'pi pi-chevron-down' : 'pi pi-chevron-right'"
              class="text-xs"
              style="color: #9b8fb0"
            />
            ...
            <span class="ml-auto font-mono text-xs">
              ...
              <span style="color: #9b8fb0" class="ml-2">Oficial</span>
              {{ proy.tiene_oficial ? fmtCompact(proy.utilidad_oficial) : '—' }}
              <span class="ml-2 font-semibold" :style="{ color: colorDif(proy.utilidad_dif) }"
          ...
          <div v-if="abiertos.has(proy.proyecto_id)" class="px-3 py-2">
            ...
            <div class="mb-2 flex justify-end">
              <div class="dif-toggle">
                <button
                  class="dif-toggle-btn"
                  :class="{ on: vista(proy.proyecto_id) === '100' }"
                  @click="setVista(proy.proyecto_id, '100')"
                >
                  100%
                </button>
                <button
                  class="dif-toggle-btn"
                  :class="{ on: vista(proy.proyecto_id) === 'inv' }"
                  @click="setVista(proy.proyecto_id, 'inv')"
                >
                  Por inversionista
                </button>
              </div>
            ...
            <div v-else>
              <div
                ...
                class="mb-3"
              >
                <p class="mb-1 text-[11px] font-semibold" style="color: #6b5a8a">
...
function toggle(pid) {
  abiertos.has(pid) ? abiertos.delete(pid) : abiertos.add(pid)
}
const vista = (pid) => vistas[pid] || '100'
function setVista(pid, v) {
  vistas[pid] = v
}
...
function lineas100(proy) {
  ...
  for (const inv of proy.inversionistas || []) {
    for (const ln of inv.lineas || []) {
      ...
      let row = map.get(k)
      if (!row) {
        row = { grupo: ln.grupo, concepto: ln.concepto, preliquidacion: null, oficial: null }
        map.set(k, row)
      }
  ...
  return [
    {
      ...
      value: r.utilidad_estimada || 0,
      color: '#2C2039',
      arrowVal: 0,
    },
    {
      label: 'Utilidad real (oficial)',
      value: r.utilidad_real || 0,
      color: '#2C2039',
      arrowVal: 0,
    },
    {
      label: 'Diferencia (se liquida)',
      value: r.diferencia || 0,
      color: colorDif(r.diferencia || 0),
      arrowVal: r.diferencia || 0,
    },
  ]
})
...
const DiffTabla = {
  ...
  setup(p) {
    ...
    const th = (t, extra = {}) =>
      h('th', { class: 'px-2 py-1 font-medium', style: { color: '#9b8fb0', ...extra } }, t)
    return () => {
      const rows = []
      for (const g of p.grupos) {
        ...
        rows.push(
          h('tr', {}, [
            h(
              ...
              {
                colspan: 5,
                class: 'px-2 pt-2 pb-0.5 font-bold uppercase tracking-wide',
                style: { color: '#6E3FB8', fontSize: '10px' },
              },
              g.label,
            ),
          ]),
        ...
        for (const ln of ls) {
          rows.push(
            h('tr', { class: 'border-t', style: { borderColor: '#f7f3fc' } }, [
              h('td', { class: 'px-2 py-1', style: { color: '#5b5470' } }, ln.concepto),
              h(
                'td',
                { class: 'px-2 py-1 text-right font-mono', style: { color: '#6b5a8a' } },
                cell(ln.preliquidacion),
              ),
              h(
                'td',
                { class: 'px-2 py-1 text-right font-mono', style: { color: '#6b5a8a' } },
              ...
              h(
                'td',
                {
                  class: 'px-2 py-1 text-right font-mono font-semibold',
                  style: { color: colorDif(ln.diferencia) },
                },
                arrow(ln.diferencia) + cell(ln.diferencia),
              ),
              h(
                'td',
                {
      ...
      rows.push(
        h(
          ...
          {
            class: 'border-t-2',
            style: { borderColor: '#915BD8', background: 'rgba(145,91,216,0.06)' },
          },
          [
            h(
              'td',
              { class: 'px-2 py-1.5 font-extrabold', style: { color: '#2C2039' } },
              'Valor a pagar (utilidad)',
            ),
            h(
              'td',
              { class: 'px-2 py-1.5 text-right font-mono font-bold', style: { color: '#2C2039' } },
              cell(u.pre),
            ),
            h(
              'td',
            ...
            h(
              'td',
              {
                class: 'px-2 py-1.5 text-right font-mono font-extrabold',
                style: { color: colorDif(u.dif) },
              },
              arrow(u.dif) + cell(u.dif),
            ),
            h('td', {}),
          ],
        ),
      ...
      return h('div', { class: 'overflow-x-auto' }, [
        h('table', { class: 'w-full text-xs' }, [
          h('thead', {}, [
            h('tr', { class: 'text-left', style: { background: '#faf7ff' } }, [
              th('Concepto'),
              th('Preliquidación', { textAlign: 'right' }),
              th('Oficial', { textAlign: 'right' }),
              th('Diferencia', { textAlign: 'right' }),
              th('%', { textAlign: 'right' }),
            ]),
          ]),
          h('tbody', {}, rows),
