<template>
  <div class="space-y-4" :class="{ 'p-4 sm:p-5': embedded }">
    ...
    <div class="flex flex-wrap items-center gap-3">
      <span v-if="tipoLabel" class="text-sm font-semibold" style="color: #915bd8">{{
        tipoLabel
      }}</span>
      <RouterLink
        v-if="tipoLabel"
    ...
    >
      <div class="flex items-center gap-2 border-b px-4 py-2.5" style="border-color: #f0ebf6">
        <h3 class="text-sm font-bold" style="color: #2c2039">
          Proyectos · {{ formatPeriodo(periodo) }}
        </h3>
        <span
      ...
      >
        ...
        <Column v-if="tipo === 'oficial'" header="Consec. Ing." style="width: 100px">
          <template #body="{ data }"
            ><span class="font-mono text-xs">{{ data.consecutivo_ingresos ?? '—' }}</span></template
          >
        </Column>
        <Column v-if="tipo === 'oficial'" header="Consec. Cos." style="width: 100px">
          <template #body="{ data }"
            ><span class="font-mono text-xs">{{ data.consecutivo_costos ?? '—' }}</span></template
          >
        </Column>
        ...
        <Column header="" style="width: 56px">
          <template #body="{ data }">
            ...
            <Button
              ...
              rounded
              size="small"
              v-tooltip.left="'Crear detalle operativo'"
              :loading="creandoDesde === data.proyecto_id"
        ...
        <template #expansion="{ data }">
          <div class="px-4 py-3" style="background: #faf8fd">
            ...
            <p
              v-if="!data.inversionistas?.length"
              class="py-2 text-center text-[11px]"
              style="color: #9b8fb0"
            >
    ...
    <Dialog
      ...
      header="Nueva liquidación (detalle operativo)"
      modal
      class="w-full max-w-md"
    >
      <div class="space-y-3 py-2">
        <p class="text-xs" style="color: #9b8fb0">
          Crea el registro operativo (mandatos/facturas/XM). Los valores contables viven en el Panel
          Contable.
        </p>
        <div>
          <label class="field-label">Proyecto</label>
          <Select
            v-model="nueva.proyecto_id"
            :options="proyectosOpcionesFiltradas"
