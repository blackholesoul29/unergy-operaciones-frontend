/* =========================================================================
   gaiaSnapshotToFasorial.ts — Adaptador de datos
   -------------------------------------------------------------------------
   El módulo fasorial.ts espera un payload de medidor con estos campos:
   ...
   ========================================================================= */
import type { FasorialMedidorDatos } from '~/features/fallas/utils/fasorial'

export interface GaiaSnapshot {
  vp1?: number | string | null
  vp2?: number | string | null
  vp3?: number | string | null
  cp1?: number | string | null
  cp2?: number | string | null
  cp3?: number | string | null
  ap1?: number | string | null
  ap2?: number | string | null
  ap3?: number | string | null
  rp1?: number | string | null
  rp2?: number | string | null
  rp3?: number | string | null
  pf1?: number | string | null
  pf2?: number | string | null
  pf3?: number | string | null
  pf_avg?: number | string | null
  last_time?: string | null
}

export interface GaiaSnapshotInfo {
...
const CAMPOS_REQUERIDOS: Array<keyof GaiaSnapshot> = ['vp1', 'vp2', 'vp3', 'cp1', 'cp2', 'cp3']
 ...
 */
export function validarSnapshot(snapshot: GaiaSnapshot | null | undefined): ValidacionSnapshot {
  ...
  const tieneActiva = (['ap1', 'ap2', 'ap3'] as const).some((k) => snapshot[k] != null)
  if (faltantes.length) {
    return {
      ok: false,
      faltantes,
...
export function edadLecturaMin(snapshot: GaiaSnapshot | null | undefined): number | null {
...
function parseLastTime(snapshot: GaiaSnapshot | null | undefined): number | null {
 ...
 */
export function gaiaSnapshotToFasorial(
  snapshot: GaiaSnapshot | null | undefined,
  info: GaiaSnapshotInfo = {},
): FasorialMedidorDatos {
  const s = snapshot || {}
  ...
  const apVals = (['ap1', 'ap2', 'ap3'] as const).map((k) => Math.abs(n(s[k])))
  const maxAp = Math.max(...apVals, 0)
  const apDivisor = maxAp > 5000 ? 1000 : 1

  const datos: FasorialMedidorDatos = {
  ...
  for (let f = 1; f <= 3; f++) {
    const rp = n(s[`rp${f}` as keyof GaiaSnapshot])
    if (rp >= 0) {
      datos[`irpp${f}` as keyof FasorialMedidorDatos] = Math.abs(rp) // importa reactivo → atrasa
      datos[`erpp${f}` as keyof FasorialMedidorDatos] = 0
    } else {
      datos[`irpp${f}` as keyof FasorialMedidorDatos] = 0
      datos[`erpp${f}` as keyof FasorialMedidorDatos] = Math.abs(rp) // exporta reactivo → adelanta
    }
  }
...
function unixSecondsFrom(snapshot: GaiaSnapshot): number | null {
