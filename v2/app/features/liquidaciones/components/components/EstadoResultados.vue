<template>
  <div class="bg-white rounded-xl shadow-sm border overflow-hidden h-full flex flex-col" style="border-color:#e8e0f0">
    <div class="px-4 py-2.5 flex items-center gap-2 border-b" style="border-color:#f0ebf6">
      ...
      <h3 class="text-sm font-bold" style="color:var(--color-unergy-deep)">Estado de Resultados</h3>
    ...
    <div class="flex-1">
      <template v-for="g in grupos" :key="g.key">
        ...
        <div class="flex items-center justify-between gap-3 px-4 py-1.5 border-t" style="border-color:#f0ebf6; background:#faf7ff">
          <span class="font-bold text-[11px] uppercase tracking-wide" style="color:var(--color-unergy-purple-dark)">{{ g.label }}</span>
          <span class="font-bold font-mono tabular-nums whitespace-nowrap" :style="{ color: g.sign < 0 ? '#D64455' : 'var(--color-unergy-deep)' }">
        ...
        <div v-for="(l, i) in g.lineas" :key="g.key + '_' + i"
          ...
          <a v-if="l.soporte_url" :href="l.soporte_url" target="_blank" rel="noopener"
            ...
            style="background:#F1EAF9; color:var(--color-unergy-purple-dark)" :title="l.referencia || 'Ver soporte'">
    ...
    <!-- INGRESO NETO -->
    <div class="flex items-center justify-between gap-3 px-4 py-2.5 border-t-2" style="border-color:var(--color-unergy-purple); background:rgba(145,91,216,0.07)">
      <span class="font-extrabold" style="color:var(--color-unergy-deep)">INGRESO NETO (Valor a pagar)</span>
      <span class="font-extrabold font-mono tabular-nums text-base whitespace-nowrap" style="color:var(--color-unergy-purple)">{{ fmtCOP(neto) }}</span>
    </div>
