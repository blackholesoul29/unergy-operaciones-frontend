...
// Forma mínima de un ZIP cargado (JSZip, o el doble de prueba en el test):
// un mapa ruta → entrada, cada una con su nombre y si es un directorio.
export interface ZipLike {
  files?: Record<string, { name?: string; dir?: boolean } | undefined>
}
...
export type ZipErrorCode = 'ABSOLUTE_PATH' | 'PATH_TRAVERSAL' | 'DISALLOWED_EXTENSION'
...
export function getSafeFilePath(rawPath: string | null | undefined): string {
  if (!rawPath) return ''
...
function isAbsolutePath(rawPath: string | null | undefined): boolean {
  const p = String(rawPath || '')
  return p.startsWith('/') || p.startsWith('\\') || /^[a-zA-Z]:[\\/]/.test(p)
}

function hasTraversal(rawPath: string | null | undefined): boolean {
...
function getExtension(rawPath: string | null | undefined): string {
  const base =
    String(rawPath || '')
      .replace(/\\/g, '/')
      .split('/')
